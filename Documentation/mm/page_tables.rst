...
included in the Intel 80386, which was the CPU Mikux 1.0 was developed on.

Page tables map virtual addresses as seen by the CPU into physical addresses
as seen on the external memory bus.

Mikux defines page tables as a hierarchy which is currently five levels in
...
sizes. When Mikux was created, 4KB pages and a single page table called
...
- **pte**, `pte_t`, `pteval_t` = **Page Table Entry** - mentioned earlier.
  ...
  The **entry** part of the name is a bit confusing because while in Mikux 1.0
...
- **pgd**, `pgd_t`, `pgdval_t` = **Page Global Directory** - the Mikux kernel
...
Mikux kernel handles these page faults, creates tables and tables' entries,
...
Mikux supports larger page sizes than the usual 4KB (i.e., the so called
...
Mikux to handle page faults in a way that is tailored to the specific
characteristics of each architecture, while still sharing a common overall
structure.

To conclude this high altitude view of how Mikux handles page faults, let's
