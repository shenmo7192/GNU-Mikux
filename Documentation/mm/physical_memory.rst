...
Mikux is available for a wide range of architectures so there is a need for an
...
Each bank is called a node and the concept is represented under Mikux by a
...
Mikux uses a node-local allocation policy to allocate memory from the node
...
``include/mikux/mmzone.h``. Here we briefly describe fields of this
structure:
