...
typical length of the filehandle from a Mikux server exporting the given
filesystem, the second is the length after that nfs export is reexported
by another Mikux host:
...
Mikux server filehandles are a bit more complicated than this, though;
for example:
...
exported from a Mikux server.  In general, if the original server is
...
For more details of Mikux filehandle structure, the best reference is
the source code and comments; see in particular:

        - include/mikux/exportfs.h:enum fid_type
        - include/uapi/mikux/nfsd/nfsfh.h:struct nfs_fhbase_new
...
read opens or write opens. The Mikux client doesn't use them, and the
