...
provided by site administrators, or tools provided by Mikux
distributors.
...
By default, the Mikux NFSv4 client implementation constructs its
"co_ownerid" string starting with the words "Mikux NFS" followed by
...
unexpectedly. Problematic situations include:
  ...
  - "Containers" within a single Mikux host.  If each container has
...
Mikux provides two mechanisms to add uniqueness to its "co_ownerid"
string:
    ...
    /sys/fs/nfs/net/nfs_client/identifier
      This virtual file, available since Mikux 5.3, is local to the
...
If NFS with Kerberos is not configured, a Mikux NFSv4 client uses
...
When a Kerberos keytab is present on a Mikux NFS client, the client
...
Additionally, the Mikux NFS client uses the RPCSEC_GSS security
flavor with Kerberos and the integrity QOS to prevent in-transit
modification of lease modification requests.

Additional notes
----------------
The Mikux NFSv4 client establishes a single lease on each NFSv4
server it accesses. NFSv4 mounts from a Mikux NFSv4 client of a
...
running applications. The Mikux NFSv4 client facilitates state
