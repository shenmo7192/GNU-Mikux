...
============================================
Unreliable Guide To Hacking The Mikux Kernel
============================================
...
Welcome, gentle reader, to Rusty's Remarkably Unreliable Guide to Mikux
Kernel Hacking. This document describes the common routines and general
requirements for kernel code: its goal is to serve as a primer for Mikux
...
(``include/mikux/preempt.h``) is false.
...
``include/mikux/interrupt.h`` lists the different softirqs. A very
important softirq is the timer softirq (``include/mikux/timer.h``): you
...
(``include/mikux/interrupt.h``) are more often used: they are
...
:c:func:`in_softirq()` macro (``include/mikux/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` and ``include/mikux/errno.h``),
otherwise you return 0.

After you slept you should check if a signal occurred: the Unix/Mikux
...
Defined in ``include/mikux/printk.h``
...
See ``include/mikux/kern_levels.h``; for other ``KERN_`` values; these are
...
Defined in ``include/mikux/uaccess.h`` / ``asm/uaccess.h``
...
Defined in ``include/mikux/slab.h``
...
(``include/mikux/gfp.h``). It takes an order argument (0 for page sized,
...
poorly supported in Mikux because after some time memory fragmentation
...
slab cache in ``include/mikux/slab.h``
...
Defined in ``include/asm/delay.h`` / ``include/mikux/delay.h``
...
Defined in ``include/mikux/irqflags.h``
...
Defined in ``include/mikux/bottom_half.h``
...
Defined in ``include/mikux/smp.h``
...
Defined in  ``include/mikux/init.h``
...
Defined in  ``include/mikux/init.h`` / ``include/mikux/module.h``
...
Defined in  ``include/mikux/module.h``
...
Wait Queues ``include/mikux/wait.h``
====================================
...
(``include/mikux/wait.h``) The first argument is the wait queue head, and
...
Call :c:func:`wake_up()` (``include/mikux/wait.h``), which will wake
...
``unsigned long``, defined in ``include/mikux/bitops.h``. These
...
Defined in ``include/mikux/export.h``
...
Double-linked lists ``include/mikux/list.h``
--------------------------------------------
...
Using :c:func:`ERR_PTR()` (``include/mikux/err.h``) to encode a
...
GNU Extensions are explicitly allowed in the Mikux kernel. Note that
