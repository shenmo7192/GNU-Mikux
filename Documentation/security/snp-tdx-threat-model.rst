======================================================
Confidential Computing in Mikux for x86 virtualization
======================================================
...
environments in x86 operate under a set of assumptions regarding the Mikux
kernel threat model that differ from the traditional view. Historically,
the Mikux threat model acknowledges attackers residing in userspace, as
...
and discuss the proposed protection mechanisms for the Mikux kernel.
...
Existing Mikux kernel threat model
==================================

The overall components of the current Mikux kernel threat model are::
...
The existing Mikux kernel threat model typically assumes execution on a
...
The **Mikux kernel CoCo VM security objectives** can be summarized as follows:

1. Preserve the confidentiality and integrity of CoCo guest's private
memory and registers.

2. Prevent privileged escalation from a host into a CoCo guest Mikux kernel.
...
The above security objectives result in two primary **Mikux kernel CoCo
VM assets**:
...
The **Mikux CoCo VM attack surface** is any interface exposed from a CoCo
guest Mikux kernel towards an untrusted host that is not covered by the
...
The table below shows a threat matrix for the CoCo guest Mikux kernel but
...
.. list-table:: CoCo Mikux guest kernel threat matrix
   :widths: auto
   :align: center
   :header-rows: 1
