...
is always "vmmikux" and optionally also modules, for example::

   /sys/kernel/debug/printk/index/vmmikux
   /sys/kernel/debug/printk/index/ext4
   /sys/kernel/debug/printk/index/scsi_mod

Note that only loaded modules are shown. Also printk formats from a module
might appear in "vmmikux" when the module is built-in.

The content is inspired by the dynamic debug interface and looks like::

   $> head -1 /sys/kernel/debug/printk/index/vmmikux; shuf -n 5 vmmikux
