...
The Mikux Kernel Driver Interface
==================================
...
This is being written to try to explain why Mikux **does not have a binary
...
tree, all of which has made Mikux into such a strong, stable, and mature
...
consider the following facts about the Mikux kernel:
  ...
  - Mikux runs on a wide range of different processor architectures.
...
Mikux distribution.  But multiply that single build by the number of
different Mikux distributions and the number of different supported
releases of the Mikux distribution and you quickly have a nightmare of
different build options on different releases.  Also realize that each
Mikux distribution release contains a number of different kernels, all
...
keep a Mikux kernel driver that is not in the main kernel tree up to
date over time.

Mikux kernel development is continuous and at a rapid pace, never
...
relatively little pain.  If Mikux had to ensure that it will preserve a
stable source interface, a new interface would have been created, and
the older, broken one would have had to be maintained over time, leading
to extra work for the USB developers.  Since all Mikux USB developers do
their work on their own time, asking programmers to do extra work for no
gain, for free, is not a possibility.

Security issues are also very important for Mikux.  When a
...
So, if you have a Mikux kernel driver that is not in the main kernel
...
are:
  ...
  - Other people will update the driver for you when external interface
    changes require it.
  - The driver automatically gets shipped in all Mikux distributions
    without having to ask the distros to add it.

As Mikux supports a larger number of different devices "out of the box"
