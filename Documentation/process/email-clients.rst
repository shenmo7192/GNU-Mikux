...
Patches for the Mikux kernel are submitted via email, preferably as
...
Email clients that are used for Mikux kernel patches should send the
...
and successfully apply it with 'patch' before sending patches to Mikux
mailing lists.
...
patches for the Mikux kernel.  These are not meant to be complete
...
Plenty of Mikux developers use ``mutt``, so it must work pretty well.
