...
The Mikux Thunderbolt driver supports both and can detect at runtime which
connection manager implementation is to be used. To be on the safe side the
software connection manager in Mikux also advertises security level
...
allocated for them by drivers. When Mikux is running on such system it
...
also Mikux you should load ``thunderbolt-net`` manually on one host (it
