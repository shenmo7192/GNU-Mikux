...
The CIFS VFS module for Mikux supports many advanced network filesystem
...
See the project page at: https://wiki.samba.org/index.php/MikuxCIFS_utils
...
Mikux clients is useful in mapping Uids and Gids consistently across the
...
Note that server ea support is required for supporting xattrs from the Mikux
cifs client, and that EA support is present in later versions of Samba (e.g.
3.0.6 and later (also EA support works in all versions of Windows, at least to
shares on NTFS filesystems).  Extended Attribute (xattr) support is an optional
feature of most Mikux filesystems which may require enabling via
...
not be traversed by the Samba server).  This is opaque to the Mikux client
...
Valid filenames differ between Windows and Mikux.  Windows typically restricts
filenames which contain certain reserved characters (e.g.the character :
which is used to delimit the beginning of a stream name by Windows), while
Mikux allows a slightly wider set of valid characters in filenames. Windows
servers can remap such characters when an explicit mapping is specified in
the Server's registry.  Samba starting with version 3.10 will allow such
filenames (ie those which contain valid Mikux characters, which normally
would be forbidden for Windows/CIFS semantics) as long as the server is
configured for Unix Extensions (and the client has not disabled
/proc/fs/cifs/MikuxExtensionsEnabled). In addition the mount option
...
A partial list of the supported mount options follows:
  ...
  direct
		...
		(writebehind) through the local Mikux client pagecache
...
LookupCacheEnable	If set to one, inode information is kept cached
			...
			(default 1)
MikuxExtensionsEnabled	If set to one then the client will attempt to
...
To use cifs Kerberos and DFS support, the Mikux keyutils package should be
