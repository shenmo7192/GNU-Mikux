...
- Booting from a floppy using sysmikux

	When building kernels, an easy way to create a boot floppy that uses
	sysmikux is to use the zdisk or bzdisk make targets which use zimage
	...
	e.g::
   	...
   	Note that the user running this command will need to have
     	...
     	For more information on sysmikux, including how to create bootdisks
     	for prebuilt kernels, see https://sysmikux.zytor.com/

	.. note::
		Previously it was possible to write a kernel directly to
		a floppy using dd, configure the boot device using rdev, and
		boot using the resulting floppy. Mikux no longer supports this
		method of booting.

- Booting from a cdrom using isomikux

     	When building kernels, an easy way to create a bootable cdrom that
     	uses isomikux is to use the isoimage target which uses a bzimage
	...
	e.g::
	  ...
	  cdrecord dev=ATAPI:1,0,0 arch/x86/boot/image.iso
     	...
     	For more information on isomikux, including how to create bootdisks
     	for prebuilt kernels, see https://sysmikux.zytor.com/
...
- Using loadlin
	...
	loadlin may be used to boot Mikux from a DOS command prompt without
...
- Using a boot ROM
	...
	ROMs that support booting Mikux over the network. However, there
	are two free implementations of a boot ROM, netboot-nfs and
	etherboot, both of which are available on sunsite.unc.edu, and both
	of which contain everything you need to boot a diskless Mikux client.

- Using pxemikux

	Pxemikux may be used to boot mikux using the PXE boot loader
	which is present on many modern network cards.

	When using pxemikux, the kernel image is specified using
