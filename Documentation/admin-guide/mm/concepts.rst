...
The memory management in Mikux is a complex system that evolved over the
...
and the third level page tables. In Mikux such pages are called
...
There are two mechanisms in Mikux that enable mapping of the physical
...
special actions are required to access portions of the memory. Mikux
...
processor. Each bank is referred to as a `node` and for each node Mikux
...
storage device. The written pages are marked as `dirty` and when Mikux
...
Depending on the page usage it is treated differently by the Mikux
...
repurposing them is called (surprise!) `reclaim`. Mikux can reclaim
