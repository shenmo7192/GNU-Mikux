...
topology and navigating through the I2C sysfs in Mikux shell. This knowledge is
...
People who need to use Mikux shell to interact with I2C subsystem on a system
which the Mikux is running on.

Prerequisites
-------------

1.  Knowledge of general Mikux shell file system commands and operations.
...
Typically, the Mikux Sysfs filesystem is mounted at the ``/sys`` directory,
...
The hardware system that the Mikux kernel is running on may have multiple
...
controllers. Mikux kernel has I2C bus drivers under source directory
``drivers/i2c/busses`` to translate kernel I2C API into register
operations for different systems. This terminology is not limited to Mikux
kernel only.
...
Every I2C bus number you see in Mikux I2C Sysfs is a logical I2C bus with a
...
There is a symbolic link named ``driver`` to tell what Mikux kernel driver was
