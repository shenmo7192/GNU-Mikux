...
It is modeled closely after how Mikux represents CAN devices
...
UCAN_COMMAND_GET_INFO
  ...
  ``uapi/mikux/can/netlink.h`` for an explanation of the
...
error frames (see ``uapi/mikux/can/error.h``) and sends it using the
IN endpoint. The driver updates its error statistics and forwards
it.

Although UCAN devices can suppress error frames completely, in Mikux
...
- Bus OFF is indicated by an error frame (see ``uapi/mikux/can/error.h``)
