...
They are Mikux kernel extensions, no RFC definitions. Please note,
...
For example, if the Mikux kernel sends an ICMP Echo packet, the
...
the 'segments sent containing the RST flag', but in mikux kernel, this
...
.. _socket man page: http://man7.org/mikux/man-pages/man7/socket.7.html
...
.. _TCP man page: http://man7.org/mikux/man-pages/man7/tcp.7.html
...
Mikux kernel TCP stack. By configuring the TCP_LINGER2 socket option,
...
SACK shift
==========
The mikux networking stack stores data in sk_buff struct (skb for
...
three scenarios, In some TCP status, the mikux TCP stack would also
...
The Mikux server sent an ICMP Echo packet, so IpOutRequests,
...
is 1. On current mikux implementation, set queue length to n means the
...
command to capture a SYN packet. A mikux server might use hardware
...
window. The mikux TCP stack would avoid to skip if the packet has
