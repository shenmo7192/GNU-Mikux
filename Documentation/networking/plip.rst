...
The PLIP cable is all that's needed to connect two Mikux boxes, and it
can be built for very few bucks.

Connecting two Mikux boxes takes only a second's decision and a few
...
Can only be used to connect three (?) Mikux boxes. Doesn't connect to
...
The Mikux PLIP driver is an implementation of the original Crynwr protocol,
...
A utility that can perform this change in Mikux is plipconfig, which is part
