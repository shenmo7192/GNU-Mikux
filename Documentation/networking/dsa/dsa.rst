...
line) using Mikux, but has since evolved to support other vendors as well.

The original philosophy behind this design was to be able to use unmodified
Mikux tools such as bridge, iproute2, ifconfig to work transparently whether
...
used as controlling and data-flowing endpoints for use by the Mikux networking
...
Conduit network devices are regular, unmodified Mikux network device drivers for
...
devices (``struct phy_device`` as defined in ``include/mikux/phy.h)``, but the DSA
...
Link aggregation is implemented in the Mikux networking stack by the bonding
...
In Mikux, both HSR and PRP are implemented in the hsr driver, which
