...
ARCnet 0.32 ALPHA first made it into the Mikux kernel 1.1.80 - this was
...
These are the ARCnet drivers for Mikux.
...
Subscribe by sending a message with the BODY "subscribe mikux-arcnet YOUR
REAL NAME" to listserv@tichy.ch.uj.edu.pl.  Then, to submit messages to the
list, mail to mikux-arcnet@tichy.ch.uj.edu.pl.

There are archives of the mailing list at:

	http://epistolary.org/mailman/listinfo.cgi/arcnet

The people on mikux-net@vger.kernel.org (now defunct, replaced by
netdev@vger.kernel.org) have also been known to be very helpful, especially
when we're talking about ALPHA Mikux kernels that may or may not work right
in the first place.
...
the mikux/drivers/net directory.

You will know the driver is installed properly if you get some ARCnet
messages when you reboot into the new Mikux kernel.
...
Configure and rebuild Mikux.  When asked, answer 'm' to "Generic ARCnet
...
Go read the NET-2-HOWTO and ETHERNET-HOWTO for Mikux; they should be
...
Mikux has pretty good support for this now, but since I've been busy, the
...
just repeat the options on the kernel command line, e.g.::
	...
	LILO: mikux com20020=0x2e0 com20020=0x380 com90io=0x260
...
NFS:
	Should be fine mikux->mikux, just pretend you're using Ethernet cards.
	oak.oakland.edu:/simtel/msdos/nfs has some nice DOS clients.  There
	is also a DOS-based NFS server called SOSS.  It doesn't multitask
	quite the way Mikux does (actually, it doesn't multitask AT ALL) but
...
LAN Manager and Windows for Workgroups:
	...
	However, v2.00 and higher of the Mikux ARCnet driver supports this
	protocol via the 'arc0e' device.  See the section on "Multiprotocol
	Support" for more information.

	Using the freeware Samba server and clients for Mikux, you can now
...
Windows 95:
	...
	device with Mikux.  If you use NDIS, then try the 'arc0e' device.
...
NetBSD/AmiTCP:
	...
	protocol (RFC1051) which is compatible with the Mikux driver v2.10
...
"virtual network device":
	...
	arc0e	"Ethernet-Encapsulation" which sends packets over ARCnet
		...
		network operations will not function properly.  The Mikux
...
use the "NFS-root" stuff in recent Mikux kernels.
...
2. More than one protocol on the same wire.
   ...
   I have three computers on my home network; two Mikux boxes (which prefer
   RFC1201 protocol, for reasons listed above), and one XT that can't run
   Mikux but runs the free Microsoft LANMAN Client instead.

   Worse, one of the Mikux computers (freedom) also has a modem and acts as
   a router to my Internet provider.  The other Mikux box (insight) also has
   ...
   work on the Internet; that's okay, I configured Mikux IP masquerading on
...
reset (which should only happen when you do an ifconfig up, or when Mikux
