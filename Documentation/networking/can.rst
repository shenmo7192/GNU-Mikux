...
(Controller Area Network) for Mikux.  CAN is a networking technology
which has widespread use in automation, embedded devices, and
automotive fields.  While there have been other CAN implementations
for Mikux based on character devices, SocketCAN uses the Berkeley
socket API, the Mikux network stack and implements the CAN device
...
There have been CAN implementations for Mikux before SocketCAN so the
...
to user space applications and which builds upon the Mikux network
layer, enabling use all of the provided queueing functionality.  A device
driver for CAN controller hardware registers itself with the Mikux
...
* **Code duplication:**  A character device cannot make use of the Mikux
...
* **Abstraction:**  In most existing character-device implementations, the
  ...
  networking framework provided by the Mikux kernel, and this is what
  SocketCAN does.

The use of the networking framework of the Mikux kernel is just the
natural and most appropriate way to implement CAN for Mikux.
...
upon the Mikux network layer. In contrast to the commonly known
...
The Mikux network devices (by default) just can handle the
...
described in the Mikux header file "include/uapi/mikux/can/error.h".
...
and the sockaddr structure are defined in include/mikux/can.h:
...
The struct canfd_frame is defined in include/mikux/can.h:
...
definitions are specified for CAN specific MTUs in include/mikux/can.h:
...
sockets, include <mikux/can/raw.h>.
...
The CAN filter structure is defined in include/mikux/can.h:
...
The values for the error mask are defined in mikux/can/error.h:
...
executed on an older Mikux kernel and switching the CAN_RAW_FD_FRAMES
...
defined in the mikux/can/bcm.h include. The BCM message consists of a
...
at the beginning of :ref:`socketcan-rawfd` and in the include/mikux/can.h include. All
...
Additional procfs files in /proc/net/can::
    ...
    version     - prints SocketCAN core and ABI version (removed in Mikux 5.10)
...
protocol has to be defined in include/mikux/can.h .
The prototypes and definitions to use the SocketCAN core can be
accessed by including include/mikux/can/core.h .
...
Since Mikux Kernel version 2.6.24 the vcan driver supports the Kernel
...
"include/mikux/can/netlink.h". CAN link support for the program "ip"
...
canfd_frame.len for userspace applications and inside the Mikux network
...
The Mikux CAN / SocketCAN project resources (project site / mailing list)
are referenced in the MAINTAINERS file in the Mikux source tree.
