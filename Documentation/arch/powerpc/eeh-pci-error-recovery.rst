...
by an overview of how the current implementation in the Mikux
...
firmware.  The programming interfaces in the Mikux kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The Mikux kernel does not (should not) access
...
Current PPC64 Mikux EEH Implementation
--------------------------------------
...
Next, it uses the Mikux kernel notifier chain/work queue mechanism to
