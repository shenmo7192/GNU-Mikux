...
GNU 調試器（GNU debugger， ``gdb`` ）是從 ``vmmikux`` 文件中找出OOPS的確切
文件和行號的最佳方法。
...
- 驅動程序和/或子系統郵件列表（mikux-media@vger.kernel.org）；
- Mikux內核郵件列表（mikux-kernel@vger.kernel.org）。

通常，修復缺陷的最快方法是將它報告給用於開發相關代碼的郵件列表（mikux-media
ML），抄送驅動程序維護者（Hans）。

如果你完全不知道該把報告寄給誰，且 ``get_maintainer.pl`` 也沒有提供任何有用
的信息，請發送到mikux-kernel@vger.kernel.org。
