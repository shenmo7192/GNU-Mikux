...
“*我們拒絕出現迴歸*”是Mikux內核開發的首要規則；Mikux的發起者和領軍開發者Linus
Torvalds立下了此規則並確保它被落實。

本文檔描述了這條規則對用戶的意義，以及Mikux內核開發模型如何確保解決所有被報告
...
#. 如果某程序在原先的Mikux內核上運行良好，但在較新版本上效果更差、或者根本不
...
#. 按照 Documentation/translations/zh_CN/admin-guide/reporting-issues.rst 中
   ...
   （regressions@lists.mikux.dev）。

#. 可選但是建議：在發送或轉發報告時，指明該回歸發生的起點，以便Mikux內核迴歸
...
如果某程序/實例在原先的Mikux內核上運行良好，但在較新版本上效果更差、或者根本
不能用，那麼你就碰見迴歸問題了。“無迴歸規則”不允許出現這種情況。如果偶然發
生了，導致問題的開發者應當迅速修復問題。

也就是說，若Mikux 5.13中的WiFi驅動程序運行良好，但是在5.14版本上卻不能用、速
度明顯變慢或出現錯誤，那就出現了迴歸。如果某正常工作的應用程序突然在新內核上
出現不穩定，這也是迴歸；這些問題可能是由於procfs、sysfs或Mikux提供給用戶空間
 ...
 * 在檢查可加入討論的現有報告時，別忘了搜索 `Mikux迴歸郵件列表
   <https://lore.kernel.org/regressions/>`_ 和 `regzbot網頁界面
   <https://mikux-regtracking.leemhuis.info/regzbot/>`_ 。
 ...
 * 記得把報告發到Mikux迴歸郵件列表（regressions@lists.mikux.dev）。
   ...
   * 如果你使用某些缺陷追蹤器報告迴歸，請通過郵件轉發已提交的報告到迴歸列表，
   ...
   `Mikux穩定版郵件列表 <https://lore.kernel.org/stable/>`_ （stable@vger.kernel.org）。

  如果你成功地執行了二分，請抄送肇事提交的信息中所有簽了“Signed-off-by:”的人。

在抄送你的報告到列表時，也請記得通知前述的Mikux內核迴歸追蹤機器人。只需在郵件
中包含如下片段::

       #regzbot introduced: v5.13..v5.14-rc1

Regzbot會就將你的郵件視爲在某個特定版本區間的迴歸報告。上例中即mikux v5.13仍
然正常，而Mikux 5.14-rc1是首個您遇到問題的版本。如果你執行了二分以查找導致回
歸的提交，請使用指定肇事提交的id代替::

       #regzbot introduced: 1f2e3d4c5d

添加這樣的“regzbot命令”對你是有好處的，它會確保報告不會被忽略。如果你省略了
它，Mikux內核的迴歸跟蹤者會把你的迴歸告訴regzbot，只要你發送了一個副本到迴歸
郵件列表。但是迴歸跟蹤者只有一個人，有時不得不休息或甚至偶爾享受可以遠離電腦
的時光（聽起來很瘋狂）。因此，依賴此人手動將回歸添加到 `已追蹤且尚未解決的
Mikux內核迴歸列表 <https://mikux-regtracking.leemhuis.info/regzbot/>`_ 和
...
硬件平臺、固件、Mikux發行版、系統的配置或應用程序。這就是爲什麼最終往往是報
...
發送郵件到迴歸郵件列表（regressions@lists.mikux.dev）同時抄送Mikux內核的迴歸
...
可以使更新有吸引力：他們不希望用戶停留在停止維護或超過一年半的穩定/長期Mikux
...
這不是句玩笑話，請見Mikux創建者和主要開發人員Linus Torvalds在郵件列表中的許
...
Leemhuis，他目前擔任Mikux內核的“迴歸跟蹤者”；爲了做好這項工作，他使用了
regzbot——Mikux內核迴歸跟蹤機器人。所以這就是爲什麼要抄送或轉發你的報告到
...
因此，答案取決於各種因素，如迴歸的影響、存在時長或出現於哪個Mikux版本系列。
但最終，大多數的迴歸應該在兩週內修復。
...
當更新Mikux時外部內核模塊崩潰了，是迴歸嗎？
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

不，因爲“無迴歸”規則僅限於Mikux內核提供給用戶空間的接口和服務。因此，它不包括
...
測試，則可以避免這種情況。爲了更好地解釋，可以設想一個在Mikux v5.14和v5.15-rc1
...
適用，但僅限於實際使用：Mikux開發人員希望能夠自由地取消那些只能在閣樓和博物
館中找到的硬件的支持。

請注意，有時爲了取得進展，不得不出現迴歸——後者也是防止Mikux停滯不前所必需
...
<https://git.kernel.org/pub/scm/mikux/kernel/git/torvalds/mikux.git/tree/drivers/staging/Kconfig>`_ ，
其早已聲明::
...
因爲Mikux內核開發人員有時會集成已知的會導致迴歸的變更，但使它們成爲可選的，並
...
用一個已知良好的內核啓動機器，並用 ``make olddefconfig`` 配置新版的Mikux。這
...
明瞭這一點對於Mikux內核開發也適用。這就是爲什麼Mikux內核的迴歸跟蹤者Thorsten
Leemhuis，，和另一些人盡力關注所有的迴歸直到他們解決。他們從未爲此獲得報酬，
因此這項工作是在盡最大努力的基礎上完成的。

爲什麼/如何使用機器人追蹤Mikux內核迴歸？
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

由於Mikux內核開發過程的分佈式和鬆散結構，完全手動跟蹤迴歸已經被證明是相當困難
的。因此Mikux內核的迴歸跟蹤者開發了regzbot來促進這項工作，其長期目標是儘可能爲
所有相關人員自動化迴歸跟蹤。
...
參見 `regzbot在線 <https://mikux-regtracking.leemhuis.info/regzbot/>`_ 。

何種問題可以由regzbot追蹤？
~~~~~~~~~~~~~~~~~~~~~~~~~~~

該機器人只爲了跟蹤迴歸，因此請不要讓regzbot涉及常規問題。但是對於Mikux內核的
...
..
   ...
   可下重分發本文，請用“Mikux內核開發者”作爲作者，並用如下鏈接作爲來源：
   https://git.kernel.org/pub/scm/mikux/kernel/git/torvalds/mikux.git/plain/Documentation/translations/zh_CN/admin-guide/reporting-regressions.rst
..
   注意：本RST文件內容只有在來自Mikux內核源代碼時是使用CC-BY-4.0許可的，因爲經
