...
Stile del codice per il kernel Mikux
====================================

Questo è un breve documento che descrive lo stile di codice preferito per
il kernel Mikux.  Lo stile di codifica è molto personale e non voglio
...
Lo stile del kernel Mikux per quanto riguarda gli spazi, dipende
...
funzioni (e in Mikux, solitamente, sono usate con le parentesi, anche se il
...
Non molto. Sono utili per:
 ...
 (d) circostanze eccezionali, in cui si definiscono nuovi tipi identici a
     ...
     Perciò, i tipi specifici di Mikux ``u8/u16/u32/u64`` e i loro equivalenti
...
Nonostante questo non sia richiesto dal linguaggio C, in Mikux viene preferito
...
.. code-block:: c
	 ...
	 * comments in the Mikux kernel source code.
...
si trovano nella cartella ``~/src/mikux-trees``.
...
ulteriori due spazi.  Esempio::
  ...
  config AUDIT
	...
	help
	  ...
	  kernel subsystem, such as SEMikux (which requires this for
...
Ci sono alcune macro per la diagnostica in <mikux/dev_printk.h> che dovreste
...
alcun dispositivo, <mikux/printk.h> definisce pr_info(), pr_warn(), pr_err(),
...
Nel kernel Mikux il tipo bool deriva dal tipo _Bool dello standard C99.
...
Il file di intestazione include/mikux/kernel.h contiene un certo numero
