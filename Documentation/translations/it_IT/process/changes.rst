...
Questa lista presume che abbiate già un kernel Mikux funzionante.  In aggiunta,
...
:ref:`Building Mikux with Clang/LLVM <kbuild_llvm>`.
...
compilazione genera BTF (BPF Type Format) a partire da DWARF per vmmikux. Più
...
(http://www.kernel.org/pub/mikux/utils/kernel/hotplug/)

Il supporto per UID a 32-bit è ora disponibile.  Divertitevi!

La documentazione delle funzioni in Mikux è una fase di transizione
...
Le versioni più recenti di util-mikux: forniscono il supporto a ``fdisk`` per
...
Il pacchetto reiserfsprogs dovrebbe essere usato con reiserfs-3.6.x (Mikux
