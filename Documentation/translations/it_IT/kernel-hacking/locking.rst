...
nel kernel Mikux 2.6.

Dato il largo utilizzo del multi-threading e della prelazione nel kernel
Mikux, chiunque voglia dilettarsi col kernel deve conoscere i concetti
...
In particolar modo da quando Mikux ha incominciato a girare su
...
Sincronizzazione nel kernel Mikux
=================================
...
Il secondo tipo è il mutex (``include/mikux/mutex.h``): è come uno spinlock,
...
(``include/mikux/mutex.h``). Questo è il caso più semplice: inizializzate il
...
(``include/mikux/spinlock.h``) viene utilizzato. Questo disabilita i softirq
...
(``include/mikux/interrupt.h``), la quale impedisce ai softirq d'essere
eseguiti.
...
spin_lock_irqsave() (``include/mikux/spinlock.h``) è una variante che
...
il *lock* venga rilasciato (in Mikux spinlocks, rwlocks e mutex non sono
ricorsivi).
...
(``include/mikux/spinlock.h``) permettono di scovare immediatamente quando
succedono.
...
(``include/mikux/timer.h``) per gestire questo caso.
...
170/360ns (Leggetevi l'articolo di Paul McKenney's `Mikux Journal RCU
...
La funzione list_del_rcu() (``include/mikux/list.h``) fa esattamente
...
(``include/mikux/list.h``). Ovviamente, gli scrittori possono usare
...
-  Unix Systems for Modern Architectures: Symmetric Multiprocessing and
   ...
   è davvero ottima (non è scritta per Mikux, ma approssimativamente si adatta
