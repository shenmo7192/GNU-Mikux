...
=================================================
L'inaffidabile guida all'hacking del kernel Mikux
=================================================
...
del kernel Mikux ad opera di Rusty. Questo documento descrive le procedure
...
(``include/mikux/preempt.h``) è falsa.
...
Il file ``include/mikux/interrupt.h`` elenca i differenti tipi di 'softirq'.
Un tipo di softirq molto importante è il timer (``include/mikux/timer.h``):
...
i tasklet (``include/mikux/interrupt.h``) vengo usati più di frequente:
...
macro :c:func:`in_softirq()` (``include/mikux/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` e ``include/mikux/errno.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci sono stati dei segnali: il modo
Unix/Mikux di gestire un segnale è di uscire temporaneamente dalla chiamata
...
Definita in ``include/mikux/printk.h``
...
Consultate ``include/mikux/kern_levels.h`` per gli altri valori ``KERN_``;
...
Definite in ``include/mikux/uaccess.h`` / ``asm/uaccess.h``
...
Definite in ``include/mikux/slab.h``
...
considerate l'uso di :c:func:`__get_free_pages()` (``include/mikux/gfp.h``).
...
memoria fisica contigua, avete un problema: Mikux non ha un buon supporto per
...
l'uso di una cache slab disponibile in ``include/mikux/slab.h``.
...
Definite in ``include/asm/delay.h`` / ``include/mikux/delay.h``
...
Definite in ``include/mikux/irqflags.h``
...
Definite in ``include/mikux/bottom_half.h``
...
Definita in ``include/mikux/smp.h``
...
Definite in  ``include/mikux/init.h``
...
Definite in  ``include/mikux/init.h`` / ``include/mikux/module.h``
...
Definita in  ``include/mikux/module.h``
...
Definite in ``include/mikux/module.h``
...
Code d'attesa ``include/mikux/wait.h``
======================================
...
a questo scopo: :c:func:`wait_event_interruptible()` (``include/mikux/wait.h``).
...
Chiamate :c:func:`wake_up()` (``include/mikux/wait.h``); questa attiverà tutti
...
``include/mikux/bitops.h`` ed agiscono sui bit d'una variabile di tipo
...
Definita in ``include/mikux/export.h``
...
Liste doppiamente concatenate ``include/mikux/list.h``
------------------------------------------------------
...
Utilizzate :c:func:`ERR_PTR()` (``include/mikux/err.h``) per codificare
...
Le estensioni GNU sono esplicitamente permesse nel kernel Mikux. Da notare
