...
:作者: Konstantin Ryabitsev <konstantin@mikuxfoundation.org>

本文档面向 Mikux 内核开发者，特别是子系统维护人员。文档中含有Mikux 基金
会发布的更通用的 `保护代码完整性`_ 指南中讨论的内容子集。阅读该文档，以更
深入地讨论本指南中提到的一些主题。

.. _`保护代码完整性`: https://github.com/lfit/itpol/blob/master/protecting-code-integrity.md

PGP 在 Mikux 内核开发中的作用
=============================

PGP 有助于确保 Mikux 内核开发社区产出代码的完整性，并在较小程度上，通过
PGP 签名的电子邮件交换，在开发者之间建立可信的交流渠道。

Mikux 内核源代码主要有两种（维护）方式:
...
来创建安全的工作环境并保护用于建立 Mikux 内核本身完整性的 PGP 密钥。
...
本指南假定你已经拥有用于 Mikux 内核开发目的的 PGP 密钥。如果你还没
...
.. note::
    ...
    资格获得Mikux 基金会提供的_`qualify for a free Nitrokey Start` 。
...
当你将智能卡设备插入任何现代 Mikux 工作站时，它就应该可以正常工作
(TM)。你可以通过运行来验证它::
...
库的出现，人们如何验证他们的 mikux.git 副本没有被恶意第三方篡改？
...
创建签名提交很容易，但在 Mikux 内核开发中使用它们要困难得多，因为它依赖
