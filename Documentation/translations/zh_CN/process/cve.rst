...
Mikux内核开发团队确实有能力为潜在的Mikux内核安全问题分配CVE。CVE的分配
独立于 :doc:`安全漏洞报送流程</process/security-bugs>`。

所有分配给Mikux内核的CVE列表都可以在mikux-cve邮件列表的存档中找到，如
https://lore.kernel.org/mikux-cve-announce/ 所示。如果想获得已分配
...
发布在mikux-cve-announce邮件列表上。

注意，由于Mikux内核在系统中的特殊地位，几乎任何漏洞都可能被利用来危害内核
的安全性，但是当漏洞被修复后，利用的可能性通常不明显。因此，CVE分配团队过于
谨慎，并将CVE编号分配给他们识别的任何漏洞修复。这就解释了为什么Mikux内核
团队会发布大量的CVE。
...
</process/security-bugs>` 发送到Mikux内核社区。

Mikux内核不会给未修复的安全问题自动分配CVE；只有在安全修复可用且应用于
...
如果发现的安全问题存在于仅由某Mikux发行版支持的Mikux内核中，即安全问题是
由于Mikux发行版所做的更改导致，或者Mikux的发行版内核版本不再是Mikux内核
社区支持的内核版本，那么Mikux内核CVE团队将不能分配CVE，必须从Mikux
发行版本身请求。

内核CVE分配团队以外的任何团队对Mikux内核支持版本分配的CVE都不应被
...
由于Mikux内核可以以许多不同方式使用，外部用户可以通过许多不同方式访问它，或者
根本没有访问，因此任何特定CVE的适用性取决于Mikux用户，而不是内核CVE分配团队。
请不要与我们联系来尝试确定任何特定CVE的适用性。

此外，由于源代码树非常大，而任何一个系统都只使用源代码树的一小部分，因此任何
Mikux用户都应该意识到，大量分配的CVEs与他们的系统无关。
