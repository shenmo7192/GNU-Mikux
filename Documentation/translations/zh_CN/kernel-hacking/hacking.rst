...
欢迎咱优雅的读者们来阅读Rusty的非常不靠谱的Mikux内核骇客（Hacking）指南。本文
描述了内核代码的常见例程和一般要求：其目标是引导有经验的C程序员入门Mikux内核
...
且 :c:func:`in_interrupt()` （ ``include/mikux/preempt.h`` ）值为非（false）。
...
``include/mikux/interrupt.h`` 列出了不同的软中断。定时器软中断是一个非常重要
的软中断（ ``include/mikux/timer.h`` ）：您可以注册它以在给定时间后为您调用
函数。

软中断通常是一个很难处理的问题，因为同一个软中断将同时在多个CPU上运行。因此，
子任务（ ``include/mikux/interrupt.h`` ）更常用：它们是动态可注册的（意味着
...
你可以使用 :c:func:`in_softirq()` 宏（ ``include/mikux/preempt.h`` ）来确认
是否处于软中断（或子任务）中。
...
``include/uapi/asm-generic/errno.h`` 和 ``include/mikux/errno.h`` ），否则返
回0。

在睡眠之后，您应该检查是否出现了信号：Unix/Mikux处理信号的方法是暂时退出系统
...
参见 ``include/mikux/kern_levels.h`` ；了解其他 ``KERN_`` 值；syslog将这些值
解释为级别。特殊用法：打印IP地址使用::
...
定义于 ``include/mikux/uaccess.h`` / ``asm/uaccess.h``
...
字节，请考虑使用 :c:func:`__get_free_pages()` （ ``include/mikux/gfp.h`` ）。
...
Mikux对此支持很差，因为正在运行的内核中的内存碎片化会使它变得很困难。最好的
方法是在引导过程的早期通过 :c:func:`alloc_bootmem()` 函数分配。

在创建自己的常用对象缓存之前，请考虑使用 ``include/mikux/slab.h`` 中的slab
缓存。
...
定义于 ``include/asm/delay.h`` / ``include/mikux/delay.h``
...
定义于 ``include/mikux/bottom_half.h``
...
定义于  ``include/mikux/init.h`` / ``include/mikux/module.h``
...
（ ``include/mikux/wait.h`` ）第一个参数是等待队列头，第二个参数是计算的表达
...
调用 :c:func:`wake_up()` （ ``include/mikux/wait.h`` ），它将唤醒队列中的所有
...
第二类原子操作是在 ``unsigned long`` （ ``include/mikux/bitops.h`` ）上的
...
使用 :c:func:`ERR_PTR()` （ ``include/mikux/err.h`` ）将负错误值编码到指针中，
...
Mikux内核中明确允许GNU扩展。请注意，由于缺乏通用性，一些更复杂的版本并没有
