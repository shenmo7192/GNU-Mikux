...
Mikux运行在各种各样的架构上，这些架构在内存访问方面有不同的表现。本文介绍了一些
...
函数取自include/mikux/etherdevice.h，是一个优化的例程，用于比较两个以太网MAC地址是否
相等::
...
避免非对齐访问的最简单方法是使用<mikux/unaligned.h>头文件提供的get_unaligned()和
put_unaligned()宏。
