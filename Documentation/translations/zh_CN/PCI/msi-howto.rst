...
要获得传递给require_irq()和free_irq()的Mikux IRQ号码和向量，请使用以下函数::
...
或单一的MSI，没有必要禁用中断（Mikux保证同一中断不会被重新输入）。如果一个设备
...
ACPI FADT表中指明了它。在这种情况下，Mikux会自动禁用MSI。有些板卡在表中没
...
备上的MSI。你最好把问题报告给mikux-pci@vger.kernel.org，包括完整的
...
芯片组，如nVidia nForce和Serverworks HT2000）。与主机芯片组一样，Mikux大
多知道它们，如果可以的话，会自动启用MSI。如果你有一个Mikux不知道的网桥，你可以
...
同样，请通知 mikux-pci@vger.kernel.org 任何需要特殊处理的桥。
