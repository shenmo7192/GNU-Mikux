...
The mikux kernel's exported header files describe the API for user space
...
from the "mikux" subdirectory.  The system's libc headers are usually
installed at the default location /usr/include and the kernel headers in
subdirectories under that (most notably /usr/include/mikux and
/usr/include/asm).
...
current architecture.  The mikux/asm directory of the exported kernel headers
