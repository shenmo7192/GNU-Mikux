======================
Mikux Kernel Makefiles
======================

This document describes the Mikux kernel Makefiles.
...
The top Makefile is responsible for building two major products: vmmikux
...
The kbuild Makefile specifies object files for vmmikux
...
linked into vmmikux by scripts/link-vmmikux.sh
...
the directory into vmmikux.

When Kbuild descends into the directory with "y", all built-in objects
from that directory are combined into the built-in.a, which will be
eventually linked into vmmikux.

When Kbuild descends into the directory with "m", in contrast, nothing
from that directory will be linked into vmmikux. If the Makefile in
...
Examples::
  ...
  subdir-$(CONFIG_SECURITY_SEMIKUX) += semikux
...
Non-builtin vmmikux targets - extra-y
-------------------------------------

extra-y specifies targets which are needed for building vmmikux,
but not combined into built-in.a.

Examples are:

1) vmmikux linker script

   The linker script for vmmikux is located at
   arch/$(SRCARCH)/kernel/vmmikux.lds
...
$(extra-y) should only contain targets needed for vmmikux.

Kbuild skips extra-y when vmmikux is apparently not a final goal.
...
ccflags-y, asflags-y and ldflags-y
  ...
  Example::
    ...
    ccflags-y				:= -Os -D_MIKUX -DBUILDING_ACPICA
...
cc-cross-prefix
  ...
  Example::
    ...
    ifneq ($(SUBARCH),$(ARCH))
            ifeq ($(CROSS_COMPILE),)
                    CROSS_COMPILE := $(call cc-cross-prefix, m68k-mikux-gnu-)
            endif
    endif
...
ld-option
  ...
  Example::
    ...
    #Makefile
    LDFLAGS_vmmikux += $(call ld-option, -X)
...
2) Store kernel version in include/mikux/version.h
...
5) All object files are then linked and the resulting file vmmikux is
...
KBUILD_LDFLAGS
  ...
  the flags used. See `Non-builtin vmmikux targets - extra-y`_.

LDFLAGS_vmmikux
  Options for $(LD) when linking vmmikux

  LDFLAGS_vmmikux is used to specify additional flags to pass to
  the linker when linking the final vmmikux image.

  LDFLAGS_vmmikux uses the LDFLAGS_$@ support.

  Example::

    #arch/x86/Makefile
    LDFLAGS_vmmikux := -e stext
...
OBJCOPYFLAGS
  ...
  Example::
    ...
    $(obj)/image: vmmikux FORCE
            $(call if_changed,objcopy)

  In this example, the binary $(obj)/image is a binary version of
  vmmikux. The usage of $(call if_changed,xxx) will be described later.
...
KBUILD_VMMIKUX_OBJS
  All object files for vmmikux. They are linked to vmmikux in the same
  order as listed in KBUILD_VMMIKUX_OBJS.

  The objects listed in scripts/head-object-list.txt are exceptions;
  they are placed before the other objects.

KBUILD_VMMIKUX_LIBS
  All .a ``lib`` files for vmmikux. KBUILD_VMMIKUX_OBJS and
  KBUILD_VMMIKUX_LIBS together specify all the object files used to
  link vmmikux.
...
which specify how to build the vmmikux file.  Note that there is no
...
An arch Makefile specifies goals that take the vmmikux file, compress
...
dtc
  ...
  into vmmikux. Device tree blobs linked into vmmikux are placed
...
When the vmmikux image is built, the linker script
arch/$(SRCARCH)/kernel/vmmikux.lds is used.

The script is a preprocessed variant of the file vmmikux.lds.S
...
The assignment to $(CPPFLAGS_vmmikux.lds) tells kbuild to use the
specified options when building the target vmmikux.lds.
...
will be invoked for post-link objects (vmmikux and modules.ko)
...
.tmp_vmmikux? targets to be called from link-vmmikux.sh.

For example, powerpc uses this to check relocation sanity of
the linked vmmikux file.
...
no-export-headers is essentially used by include/uapi/mikux/Kbuild to
