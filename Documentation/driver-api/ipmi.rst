...
This document describes how to use the IPMI driver for Mikux.  If you
...
The Mikux IPMI driver is modular, which means you have to pick several
...
The Mikux IPMI driver is designed to be very modular and flexible, you
...
timer.  This driver implements the standard Mikux watchdog timer
...
running Mikux to provide an interface to the host.
...
mikux/ipmi.h - Contains the user interface and IOCTL interface for IPMI.

mikux/ipmi_smi.h - Contains the interface for system management interfaces
(things that interface to IPMI controllers) to use.

mikux/ipmi_msgdefs.h - General definitions for base IPMI messaging.
...
A watchdog timer is provided that implements the Mikux-standard
