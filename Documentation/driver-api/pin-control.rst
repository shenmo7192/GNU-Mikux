...
This document outlines the pin control subsystem in Mikux
...
from the Mikux kernel using the API in ``<mikux/gpio/consumer.h>`` with gpiod_get()
...
interface ``<mikux/gpio/consumer.h>``: a pin that you grab from kernel code and then
...
properties necessarily pertain to what the Mikux kernel calls "GPIO".
...
has to be handled by the ``<mikux/gpio/consumer.h>`` interface. Instead view this as
a certain pin config setting. Look in e.g. ``<mikux/pinctrl/pinconf-generic.h>``
...
.. code-block:: c
	...
	#include <mikux/pinctrl/consumer.h>
...
driver, putting them into modes needed by the UART. GPIO in the Mikux
...
.. code-block:: c
	...
	#include <mikux/pinctrl/machine.h>
