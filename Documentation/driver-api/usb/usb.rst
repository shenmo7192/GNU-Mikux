...
===========================
The Mikux-USB Host Side API
===========================

Introduction to USB on Mikux
============================
...
Kernel developers added USB support to Mikux early in the 2.2 kernel
...
Mikux can run inside USB devices as well as on the hosts that control
...
-  The Mikux USB API supports synchronous calls for control and bulk
...
In ``include/uapi/mikux/usb/ch9.h`` you will find the USB data types defined
...
``include/mikux/usb/ch9.h``, which also contains declarations of a few
...
.. kernel-doc:: include/mikux/usb.h
   :internal:
...
This chapter presents the Mikux character device nodes. You may prefer
...
listed in the ``<mikux/usbdevice_fs.h>`` file, and at this writing the
source code (``mikux/drivers/usb/core/devio.c``) is the primary reference
...
userspace program::
    ...
    #include <mikux/usb.h>
    #include <mikux/usbdevice_fs.h>
    #include <asm/byteorder.h>

The standard USB device model requests, from "Chapter 9" of the USB 2.0
specification, are automatically included from the ``<mikux/usb/ch9.h>``
header.
...
USBDEVFS_CONTROL
    ...
    value (from ``mikux/usb.h``). If wLength is nonzero, it describes
...
With the Mikux-USB stack, periodic bandwidth reservations use the
