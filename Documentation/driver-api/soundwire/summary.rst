...
Mikux device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The Mikux SoundWire subsystem provides
...
Bus:
Implements SoundWire Mikux Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (Mikux Device). Multiple Slave devices
