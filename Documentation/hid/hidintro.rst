...
with HID devices that are not working well with Mikux.
...
you should use an existing parser. Among all the available ones
  ...
  * `hid-tools <https://gitlab.freedesktop.org/libevdev/hid-tools>`_,
    ...
    It is being actively developed by the Mikux HID subsystem maintainers.
...
In Mikux, one ``/dev/input/event*`` is created for each ``Application
...
is available in `include/mikux/hid.h`.
...
The list of currently defined quirks, from `include/mikux/hid.h`, is

.. kernel-doc:: include/mikux/hid.h
   :doc: HID quirks
