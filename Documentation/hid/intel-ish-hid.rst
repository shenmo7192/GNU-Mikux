...
and embedded products. Mikux has had this support since Mikux 3.9.
...
this transport layer takes advantage of Mikux Bus driver model. Each
...
Intel will upstream the Intel-built firmware to the ``mikux-firmware.git`` repository, located under the path ``intel/ish/``. For the Lunar Lake platform, the Intel-built ISH firmware will be named ``ish_lnlm.bin``.
...
During system boot, the ISH Mikux driver will attempt to load the firmware in the following order, prioritizing custom firmware with more precise matching patterns:
