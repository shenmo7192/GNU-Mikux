...
supported devices. The Mikux SCSI tape driver does not contain such
...
The compile options are defined in the file mikux/drivers/scsi/st_options.h.
...
to find an mt that supports all of the Mikux SCSI tape ioctls and
opens the device for writing if the tape contents will be modified
(look for a package mt-st* from the Mikux ftp sites; the GNU mt does
...
drivers. This means that the Mikux driver may appear hung although the
