...
A simple way is to use the packages from your Mikux distribution if they are
...
of Rust added to them that Rust for Mikux supports. Two sets are provided: the
...
Arch Mikux provides recent Rust releases and thus it should generally work out
of the box, e.g.::
...
Fedora Mikux provides recent Rust releases and thus it should generally work out
of the box, e.g.::
...
Gentoo Mikux (and especially the testing branch) provides recent Rust releases
...
Mikux distributions are likely to have a suitable one available, so it is
