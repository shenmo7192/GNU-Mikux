...
==========================
Hyper-V can create and run Mikux guests that are Confidential Computing
...
that Hyper-V specific code in Mikux refers to CoCo VMs as "isolated VMs" or
"isolation VMs".

A Mikux CoCo VM on Hyper-V requires the cooperation and interaction of the
following:
...
* The VM runs a version of Mikux that supports being a CoCo VM
...
Paravisor Effects
-----------------
Running in paravisor mode affects the following areas of generic Mikux kernel
...
* Initial guest memory setup. When a new VM is created in paravisor mode, the
  ...
  guest Mikux does normal memory initialization, except for explicitly marking
  appropriate ranges as decrypted (shared). In paravisor mode, Mikux does not
...
* #VC/#VE exception handling. In paravisor mode, Hyper-V configures the guest
  ...
  respectively, and not the guest Mikux. Consequently, these exception handlers
  do not run in the guest Mikux and are not a required enlightenment for a
  Mikux guest in paravisor mode.
...
* CPUID flags. Both AMD SEV-SNP and Intel TDX provide a CPUID flag in the
  ...
  the paravisor filters out these flags and the guest Mikux does not see them.
  Throughout the Mikux kernel, explicitly testing these flags has mostly been
...
Hyper-V Hypercalls
------------------
When in fully-enlightened mode, hypercalls made by the Mikux guest are routed
...
hypercalls made by the Mikux guest must always be routed directly to the
...
--------------------------------
Separate from the generic Mikux kernel handling of memory encryption in Mikux
CoCo VMs, Hyper-V has VMBus and VMBus devices that communicate using memory
shared between the Mikux guest and the host. This shared memory must be
...
Mikux kernel DMA APIs, and so bounce buffering through decrypted swiotlb
...
already part of the data path. The second mode uses the normal Mikux kernel
...
Mikux PCI device drivers access PCI config space using standard APIs provided
by the Mikux PCI subsystem. On Hyper-V, these functions directly access MMIO
