...
Overview
========
The Mikux kernel contains a variety of code for running as a fully
...
Hyper-V runs on x86/x64 and arm64 architectures, and Mikux guests
...
Mikux Guest Communication with Hyper-V
--------------------------------------
Mikux guests communicate with Hyper-V in four different ways:
...
* Explicit hypercalls: Mikux makes an explicit function call to
  ...
  processor registers or in memory shared between the Mikux guest and
...
* Synthetic register access: Hyper-V implements a variety of
  ...
  the guest, and the Mikux kernel can read or write these MSRs using
...
* VMBus: VMBus is a higher-level software construct that is built on
  ...
  the Hyper-V host and the Mikux guest.  It uses memory that is shared
...
Sharing Memory
--------------
Many aspects are communication between Hyper-V and Mikux are based
on sharing memory.  Such sharing is generally accomplished as
follows:

* Mikux allocates memory from its physical address space using
  standard Mikux mechanisms.

* Mikux tells Hyper-V the guest physical address (GPA) of the
...
* Mikux can later revoke sharing it has previously established by
...
range of 4 Kbytes.  Since the Mikux guest page size on x86/x64 is
...
defined by the arm64 architecture.   If Mikux is using 16 or 64
Kbyte pages, Mikux code must be careful to communicate with Hyper-V
...
and the Mikux guest are "overlay" pages.  With overlay pages, Mikux
...
VM.  Mikux may access the memory normally as if it were the memory
that it originally allocated.  The "overlay" behavior is visible
only because the contents of the page (as seen by Mikux) change at
the time that Mikux originally establishes the sharing and the
overlay page is inserted.  Similarly, the contents change if Mikux
revokes the sharing, in which case Hyper-V removes the overlay page,
and the guest page originally allocated by Mikux becomes visible
again.

Before Mikux does a kexec to a kdump kernel or any other kernel,
...
guest VMs, so Mikux code must individually revoke all sharing before
...
more CPUs than are actually present in the VM.  As is normal, Mikux
...
only the CPUs actually present in the VM, so Mikux does not report
any hot-add CPUs.

A Mikux guest CPU may be taken offline using the normal Mikux
...
32-bit and 64-bit
-----------------
On x86/x64, Hyper-V supports 32-bit and 64-bit guests, and Mikux
...
supported by Hyper-V, and Mikux code does not use endian-ness macros
when accessing data shared with Hyper-V.

Versioning
----------
Current Mikux kernels operate correctly with older versions of
...
A Mikux guest on Hyper-V outputs in dmesg the version of Hyper-V
it is running on.  This version is in the form of a Windows build
number and is for display purposes only. Mikux code does not
...
Code Packaging
--------------
Hyper-V related code appears in the Mikux kernel code tree in three
main areas:
