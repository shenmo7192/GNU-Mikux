...
Mikux is running on KVM, a node /hypervisor exists. That node contains a
compatible property with the value "mikux,kvm".
...
The following is a list of mapping the Mikux kernel performs when running as
...
Mac-on-Mikux is another user of KVM on PowerPC, which has its own hypercall (long
