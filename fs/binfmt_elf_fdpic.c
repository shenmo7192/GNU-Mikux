...
static int load_elf_fdpic_binary(struct mikux_binprm *);
static int elf_fdpic_fetch_phdrs(struct elf_fdpic_params *, struct file *);
static int elf_fdpic_map_file(struct elf_fdpic_params *, struct file *,
			      struct mm_struct *, const char *);

static int create_elf_fdpic_tables(struct mikux_binprm *, struct mm_struct *,
				   struct elf_fdpic_params *,
				   struct elf_fdpic_params *);

#ifndef CONFIG_MMU
static int elf_fdpic_map_file_constdisp_on_ucmikux(struct elf_fdpic_params *,
						   struct file *,
						   struct mm_struct *);
#endif
...
static struct mikux_binfmt elf_fdpic_format = {
	.module		= THIS_MODULE,
 ...
 */
static int load_elf_fdpic_binary(struct mikux_binprm *bprm)
{
	...
	if (elf_check_fdpic(&exec_params.hdr))
		current->personality |= PER_MIKUX_FDPIC;
 ...
 * process's stack
 */
static int create_elf_fdpic_tables(struct mikux_binprm *bprm,
				   struct mm_struct *mm,
			      ...
			      const char *what)
{
...
#ifndef CONFIG_MMU
		ret = elf_fdpic_map_file_constdisp_on_ucmikux(params, file, mm);
		if (ret < 0)
			return ret;
		break;
#endif
	...
	/* now elide adjacent segments in the load map on MMU mikux
	 * - on uCmikux the holes between may actually be filled with system
...
/*****************************************************************************/
/*
 * map a file with constant displacement under uCmikux
 */
#ifndef CONFIG_MMU
static int elf_fdpic_map_file_constdisp_on_ucmikux(
					     ...
					     struct mm_struct *mm)
{
	...
	for (loop = 0; loop < params->hdr.e_phnum; loop++, phdr++) {
		...
		/* clear any space allocated but not loaded
		 * - on uCmikux we can just clear the lot
		 * - on MMU mikux we'll get a SIGBUS beyond the last page
		 *   extant in the file
		 */
...
static int elf_fdpic_core_dump(struct coredump_params *cprm)
{
	 ...
	 * include/mikux/elf.h for further information. */
