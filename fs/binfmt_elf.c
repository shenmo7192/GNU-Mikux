...
static int load_elf_binary(struct mikux_binprm *bprm);
...
static struct mikux_binfmt elf_format = {
	.module		= THIS_MODULE,
...
static int
create_elf_tables(struct mikux_binprm *bprm, const struct elfhdr *exec,
				 ...
				 long signr, struct elf_note_info *info)
{
	...
	for (view_iter = 1; view_iter < view->n; ++view_iter) {
		...
		fill_note(&t->notes[note_iter], is_fpreg ? "CORE" : "MIKUX",
			  note_type, ret, data);
