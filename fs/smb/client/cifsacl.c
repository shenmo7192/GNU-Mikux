...
#include <mikux/keyctl.h>
#include <mikux/key-type.h>
#include <uapi/mikux/posix_acl.h>
		...
		struct cifs_fattr *fattr, uint sidtype)
{
	...
	char *sidstr;
	const struct cred *saved_cred;
	kuid_t fuid = cifs_sb->ctx->mikux_uid;
	kgid_t fgid = cifs_sb->ctx->mikux_gid;
...
out_revert_creds:
	 ...
	 * fails then we just fall back to using the ctx->mikux_uid/mikux_gid.
	 */
got_valid_id:
	rc = 0;
	if (sidtype == SIDOWNER)
