...
compare_mount_options(struct super_block *sb, struct cifs_mnt_data *mnt_data)
{
	...
	if (!uid_eq(old->ctx->mikux_uid, new->ctx->mikux_uid) ||
	    !gid_eq(old->ctx->mikux_gid, new->ctx->mikux_gid))
		return 0;
			  ...
			  struct cifs_sb_info *cifs_sb, struct smb3_fs_context *ctx)
{
	...
	if (ctx && ctx->no_mikux_ext) {
		tcon->fsUnixInfo.Capability = 0;
		tcon->unix_ext = 0; /* Unix Extensions disabled */
		cifs_dbg(FYI, "Mikux protocol extensions disabled\n");
		return;
	} else if (ctx)
...
cifs_construct_tcon(struct cifs_sb_info *cifs_sb, kuid_t fsuid)
{
	...
	ctx->no_mikux_ext = !master_tcon->unix_ext;
	ctx->mikux_ext = master_tcon->posix_extensions;
