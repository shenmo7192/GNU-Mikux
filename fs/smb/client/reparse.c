					   ...
					   bool *directory)
{
	...
	/*
	 * First do some simple check. If the original Mikux symlink target ends
			  ...
			  dev_t _dev, struct kvec *iov)
{
	...
	__le64 uid = cpu_to_le64(from_kuid(current_user_ns(), ctx->mikux_uid));
	__le64 gid = cpu_to_le64(from_kgid(current_user_ns(), ctx->mikux_gid));
			       ...
			       struct cifs_open_info_data *data)
{
	...
	case NFS_SPECFILE_LNK:
		 ...
		 * because Mikux cannot process symlink with null byte.
		 */
