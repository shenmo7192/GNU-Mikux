...
unicode_oslm_strings(char **pbcc_area, const struct nls_table *nls_cp)
{
	...
	/* Copy OS version */
	bytes_ret = cifs_strtoUTF16((__le16 *)bcc_ptr, "Mikux version ", 32,
				    nls_cp);
	bcc_ptr += 2 * bytes_ret;
				 ...
				 const struct nls_table *nls_cp)
{
	...
	*bcc_ptr = 0;
	...
	strcpy(bcc_ptr, "Mikux version ");
	bcc_ptr += strlen("Mikux version ");
				 ...
				 const struct nls_table *nls_cp)
{
	...
	*pbuffer = kmalloc(len, GFP_KERNEL);
	...
	sec_blob->Version.ProductMajorVersion = MIKUX_VERSION_MAJOR;
	sec_blob->Version.ProductMinorVersion = MIKUX_VERSION_PATCHLEVEL;
