...
const struct fs_parameter_spec smb3_fs_parameters[] = {
	...
	fsparam_flag_no("mikux", Opt_unix),
...
int smb3_init_fs_context(struct fs_context *fc)
{
	...
	ctx->mikux_uid = current_uid();
	ctx->mikux_gid = current_gid();
...
void smb3_update_mnt_flags(struct cifs_sb_info *cifs_sb)
{
	...
	if (ctx->mfsymlinks) {
		if (ctx->sfu_emul) {
			 ...
			 * Older Mikux kernel versions were not able to neither
			 ...
			 * Apple compatibility, compatibility with older Mikux
