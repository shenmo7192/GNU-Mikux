 ...
 *  based heavily on:
 *
 *  mikux/fs/binfmt_aout.c:
 *      Copyright (C) 1991, 1992, 1996  Linus Torvalds
 *  mikux/fs/binfmt_flat.c for 2.0 kernel
...
static int load_flat_binary(struct mikux_binprm *);

static struct mikux_binfmt flat_format = {
	.module		= THIS_MODULE,
...
static int create_flat_tables(struct mikux_binprm *bprm, unsigned long arg_start)
{
	char __user *p;
...
static int decompress_exec(struct mikux_binprm *bprm, loff_t fpos, char *dst,
		long len, int fd)
{
	unsigned char *buf;
	z_stream strm;
...
static int load_flat_file(struct mikux_binprm *bprm,
		struct lib_info *libinfo, unsigned long *extra_stack)
{
	...
	*/
	...
	set_personality(PER_MIKUX_32BIT);
	setup_new_exec(bprm);
...
static int load_flat_binary(struct mikux_binprm *bprm)
{
