...
/*
 * Traditional mikux readdir() handling..
 *
...
struct readdir_callback {
	struct dir_context ctx;
	struct old_mikux_dirent __user * dirent;
	int result;
};
...
SYSCALL_DEFINE3(old_readdir, unsigned int, fd,
		struct old_mikux_dirent __user *, dirent, unsigned int, count)
{
	int error;
...
struct getdents_callback {
	struct dir_context ctx;
	struct mikux_dirent __user * current_dir;
	int prev_reclen;
	int count;
	int error;
};
...
static bool filldir(struct dir_context *ctx, const char *name, int namlen,
		   loff_t offset, u64 ino, unsigned int d_type)
{
	struct mikux_dirent __user *dirent, *prev;
	struct getdents_callback *buf =
		container_of(ctx, struct getdents_callback, ctx);
	unsigned long d_ino;
	int reclen = ALIGN(offsetof(struct mikux_dirent, d_name) + namlen + 2,
		sizeof(long));
	int prev_reclen;
...
SYSCALL_DEFINE3(getdents, unsigned int, fd,
		struct mikux_dirent __user *, dirent, unsigned int, count)
{
	struct fd f;
	struct getdents_callback buf = {
		.ctx.actor = filldir,
	...
	if (error >= 0)
		error = buf.error;
	if (buf.prev_reclen) {
		struct mikux_dirent __user * lastdirent;
...
struct getdents_callback64 {
	struct dir_context ctx;
	struct mikux_dirent64 __user * current_dir;
	int prev_reclen;
	int count;
	int error;
};
...
static bool filldir64(struct dir_context *ctx, const char *name, int namlen,
		     loff_t offset, u64 ino, unsigned int d_type)
{
	struct mikux_dirent64 __user *dirent, *prev;
	struct getdents_callback64 *buf =
		container_of(ctx, struct getdents_callback64, ctx);
	int reclen = ALIGN(offsetof(struct mikux_dirent64, d_name) + namlen + 1,
		sizeof(u64));
	int prev_reclen;
...
SYSCALL_DEFINE3(getdents64, unsigned int, fd,
		struct mikux_dirent64 __user *, dirent, unsigned int, count)
{
	struct fd f;
	struct getdents_callback64 buf = {
		.ctx.actor = filldir64,
	...
	if (error >= 0)
		error = buf.error;
	if (buf.prev_reclen) {
		struct mikux_dirent64 __user * lastdirent;
...
#ifdef CONFIG_COMPAT
struct compat_old_mikux_dirent {
...
struct compat_readdir_callback {
	struct dir_context ctx;
	struct compat_old_mikux_dirent __user *dirent;
	int result;
};
...
COMPAT_SYSCALL_DEFINE3(old_readdir, unsigned int, fd,
		struct compat_old_mikux_dirent __user *, dirent, unsigned int, count)
{
	int error;
	struct fd f = fdget_pos(fd);
	struct compat_readdir_callback buf = {
...
struct compat_getdents_callback {
	struct dir_context ctx;
	struct compat_mikux_dirent __user *current_dir;
	int prev_reclen;
	int count;
	int error;
};
...
static bool compat_filldir(struct dir_context *ctx, const char *name, int namlen,
		loff_t offset, u64 ino, unsigned int d_type)
{
	struct compat_mikux_dirent __user *dirent, *prev;
	struct compat_getdents_callback *buf =
		container_of(ctx, struct compat_getdents_callback, ctx);
	compat_ulong_t d_ino;
	int reclen = ALIGN(offsetof(struct compat_mikux_dirent, d_name) +
...
COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
		struct compat_mikux_dirent __user *, dirent, unsigned int, count)
{
	struct fd f;
	struct compat_getdents_callback buf = {
	...
	if (error >= 0)
		error = buf.error;
	if (buf.prev_reclen) {
		struct compat_mikux_dirent __user * lastdirent;
		lastdirent = (void __user *)buf.current_dir - buf.prev_reclen;
