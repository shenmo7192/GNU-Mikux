 ...
 * historically has been the same until mikux-4.13.
 */
				   ...
				   int rst)
{
	...
	/* RST segments are not recommended to carry timestamp,
	   ...
	   via reboots. Mikux-2.2 DOES NOT!
					 ...
					 __u16 *mss)
{
	...
	__NET_INC_STATS(sock_net(sk), MIKUX_MIB_SYNCOOKIESSENT);
				   ...
				   int family, int l3index, bool stat_inc)
{
	...
	if (ao_info->ao_required || ao_key) {
		if (stat_inc) {
			NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAOREQUIRED);
