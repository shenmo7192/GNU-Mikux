...
#ifndef MIKUX_MSI_API_H
#define MIKUX_MSI_API_H
 ...
 * hardware/software MSI indices and the Mikux interrupt number.
 */
...
/**
 * msi_map - Mapping between MSI index and Mikux interrupt number
 ...
 *		the error code.
 * @virq:	The associated Mikux interrupt number
 */
struct msi_map {
	int	index;
	int	virq;
...
/**
 * msi_get_virq - Lookup the Mikux interrupt number for a MSI index on the default interrupt domain
