...
/*
 * Flag values for the hv_vss_check_feature. Mikux supports only
 * one value.
 */
...
/*
 * struct hv_vss_msg encodes the fields that the Mikux VSS
 * driver accesses. However, FREEZE messages from Hyper-V contain
 * additional LUN information that Mikux doesn't use and are not
 ...
 * because the Mikux VSS driver responds that it doesn't support
...
/*
 * An implementation of HyperV key value pair (KVP) functionality for Mikux.
 *
 *
...
/*
 * In Mikux, we implement the KVP functionality in two components:
