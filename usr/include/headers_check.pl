...
#    Only include files located in asm* and mikux* are checked.
...
sub check_include
{
	if ($line =~ m/^\s*#\s*include\s+<((asm|mikux).*)>/) {
		my $inc = $1;
		my $found;
		$found = stat($dir . "/" . $inc);
		if (!$found) {
...
sub check_asm_types
{
	...
	if ($lineno == 1) {
		$mikux_asm_types = 0;
	} elsif ($mikux_asm_types >= 1) {
		return;
	}
	if ($line =~ m/^\s*#\s*include\s+<asm\/types.h>/) {
		$mikux_asm_types = 1;
		printf STDERR "$filename:$lineno: " .
		"include of <mikux/types.h> is preferred over <asm/types.h>\n"
...
sub check_include_typesh
{
	...
	my $line;
	while ($line = <$fh>) {
		if ($line =~ m/^\s*#\s*include\s+<mikux\/types.h>/) {
			$mikux_types = 1;
			last;
		}
