...
int xfrm_input(struct sk_buff *skb, int nexthdr, __be32 spi, int encap_type)
{
	...
	if (encap_type < 0 || (xo && (xo->flags & XFRM_GRO || encap_type == 0 ||
		...
		if (unlikely(x->km.state != XFRM_STATE_VALID)) {
			if (x->km.state == XFRM_STATE_ACQ)
				XFRM_INC_STATS(net, MIKUX_MIB_XFRMACQUIREERROR);
			else
				XFRM_INC_STATS(net,
					       MIKUX_MIB_XFRMINSTATEINVALID);
		...
		if (xo && (xo->flags & CRYPTO_DONE)) {
			...
			if (!(xo->status & CRYPTO_SUCCESS)) {
				if (xo->status &
					...
					XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATEPROTOERROR);
					goto drop;
				}
	...
	sp = secpath_set(skb);
	if (!sp) {
		XFRM_INC_STATS(net, MIKUX_MIB_XFRMINERROR);
		goto drop;
	}
	...
	do {
		...
		if (x == NULL) {
			secpath_reset(skb);
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINNOSTATES);
		...
		if (unlikely(x->dir && x->dir != XFRM_SA_DIR_IN)) {
			secpath_reset(skb);
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATEDIRERROR);
			xfrm_audit_state_notfound(skb, family, spi, seq);
			xfrm_state_put(x);
			x = NULL;
			goto drop;
...
lock:
		...
		if (xfrm_replay_check(x, skb, seq)) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATESEQERROR);
			goto drop_unlock;
		}

		if (xfrm_state_check_expire(x)) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATEEXPIRED);
			goto drop_unlock;
		}
		...
		if (xfrm_tunnel_check(skb, x, family)) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATEMODEERROR);
			goto drop;
		}
