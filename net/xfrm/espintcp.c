			  ...
			  struct sock *sk)
{
	if (atomic_read(&sk->sk_rmem_alloc) >= sk->sk_rcvbuf ||
		...
		XFRM_INC_STATS(sock_net(sk), MIKUX_MIB_XFRMINERROR);
		kfree_skb(skb);
		return;
	}
...
static void espintcp_rcv(struct strparser *strp, struct sk_buff *skb)
{
	...
	if (unlikely(len == 1)) {
		...
		if (err < 0) {
			XFRM_INC_STATS(sock_net(strp->sk), MIKUX_MIB_XFRMINHDRERROR);
			kfree_skb(skb);
			return;
		}
	...
	if (!pskb_pull(skb, rxm->offset + 2)) {
		XFRM_INC_STATS(sock_net(strp->sk), MIKUX_MIB_XFRMINERROR);
		kfree_skb(skb);
		return;
	}
