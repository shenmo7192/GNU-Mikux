 ...
 * Mikux networking.  Thus, destinations are stackable.
 */
...
static int xfrm_output_one(struct sk_buff *skb, int err)
{
	...
	do {
		...
		if (err) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTERROR);
			goto error_nolock;
		}
		...
		if (err) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATEMODEERROR);
			goto error_nolock;
		}
		...
		if (unlikely(x->km.state != XFRM_STATE_VALID)) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATEINVALID);
			err = -EINVAL;
			goto error;
		}
		...
		if (err) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATEEXPIRED);
			goto error;
		}

		err = xfrm_replay_overflow(x, skb);
		if (err) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATESEQERROR);
			goto error;
		}
...
resume:
		if (err) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATEPROTOERROR);
			goto error_nolock;
		}
