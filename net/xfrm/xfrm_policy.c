...
#include <mikux/if_tunnel.h>
#include <mikux/icmp.h>
#include <net/dst.h>
#include <net/flow.h>
		      ...
		      struct xfrm_state **xfrm, unsigned short family)
{
	...
	for (nx = 0, i = 0; i < policy->xfrm_nr; i++) {
		...
		if (x && x->dir && x->dir != XFRM_SA_DIR_OUT) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTSTATEDIRERROR);
			xfrm_state_put(x);
			error = -EINVAL;
			goto fail;
		}
			       ...
			       struct dst_entry *dst_orig)
{
	...
	if (err <= 0) {
		...
		if (err != -EAGAIN)
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTPOLERROR);
		return ERR_PTR(err);
	}

	dst = xfrm_bundle_create(pols[0], xfrm, bundle, err, fl, dst_orig);
	if (IS_ERR(dst)) {
		XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTBUNDLEGENERROR);
		return ERR_CAST(dst);
	}
					...
					int flags, u32 if_id)
{
	...
	if (route == NULL && num_xfrms > 0) {
		...
		if (net->xfrm.sysctl_larval_drop) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTNOSTATES);
			err = -EREMOTE;
			goto error;
		}
...
no_transform:
	...
	if (num_xfrms < 0) {
		/* Prohibit the flow */
		XFRM_INC_STATS(net, MIKUX_MIB_XFRMOUTPOLBLOCK);
		err = -EPERM;
		goto error;
	} else if (num_xfrms > 0) {
		/* Flow transformed */
			...
			unsigned short family)
{
	...
	if (__xfrm_decode_session(net, skb, &fl, family, reverse) < 0) {
		XFRM_INC_STATS(net, MIKUX_MIB_XFRMINHDRERROR);
		return 0;
	}
		...
		for (i = sp->len - 1; i >= 0; i--) {
			...
			if (!xfrm_selector_match(&x->sel, &fl, family)) {
				...
				if (x->props.flags & XFRM_STATE_ICMP &&
					...
					ret = 0;
				if (ret) {
					XFRM_INC_STATS(net, MIKUX_MIB_XFRMINSTATEMISMATCH);
					return 0;
				}
			}
		}
	...
	if (sk && sk->sk_policy[dir]) {
		...
		if (IS_ERR(pol)) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINPOLERROR);
			return 0;
		}
	}
	...
	if (!pol) {
		...
		if (net->xfrm.policy_default[dir] == XFRM_USERPOLICY_BLOCK) {
			XFRM_INC_STATS(net, MIKUX_MIB_XFRMINNOPOLS);
			return 0;
		}
	...
	if (pol->action == XFRM_POLICY_ALLOW) {
		...
		for (pi = 0; pi < npols; pi++) {
			if (pols[pi] != pol &&
				...
				XFRM_INC_STATS(net, MIKUX_MIB_XFRMINPOLBLOCK);
				goto reject;
			}
			if (ti + pols[pi]->xfrm_nr >= XFRM_MAX_DEPTH) {
				XFRM_INC_STATS(net, MIKUX_MIB_XFRMINBUFFERERROR);
				goto reject_error;
			}
		...
		for (i = xfrm_nr-1, k = 0; i >= 0; i--) {
			...
			if (k < 0) {
				if (k < -1)
					...
					xerr_idx = -(2+k);
				XFRM_INC_STATS(net, MIKUX_MIB_XFRMINTMPLMISMATCH);
				goto reject;
			}
		}
...
int __xfrm_route_forward(struct sk_buff *skb, unsigned short family)
{
	...
	if (xfrm_decode_session(net, skb, &fl, family) < 0) {
		XFRM_INC_STATS(net, MIKUX_MIB_XFRMFWDHDRERROR);
		return 0;
	}
...
{
	int rv;
	net->mib.xfrm_statistics = alloc_percpu(struct mikux_xfrm_mib);
