...
static void tcp_write_err(struct sock *sk)
{
	...
	__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPABORTONTIMEOUT);
}
...
static int tcp_out_of_resources(struct sock *sk, bool do_reset)
{
	...
	if (tcp_check_oom(sk, shift)) {
		...
		tcp_done(sk);
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPABORTONMEMORY);
		return 1;
	}
...
static int tcp_write_timeout(struct sock *sk)
{
	...
	if (sk_rethink_txhash(sk)) {
		tp->timeout_rehash++;
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPTIMEOUTREHASH);
	}
...
void tcp_delack_timer_handler(struct sock *sk)
{
	...
	if (inet_csk_ack_scheduled(sk)) {
		...
		tcp_mstamp_refresh(tp);
		tcp_send_ack(sk);
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_DELAYEDACKS);
	}
}
...
static void tcp_delack_timer(struct timer_list *t)
{
	...
	if (!sock_owned_by_user(sk)) {
		...
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_DELAYEDACKLOCKED);
...
void tcp_retransmit_timer(struct sock *sk)
{
	...
	__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPTIMEOUTS);
	...
	if (icsk->icsk_retransmits == 0) {
		...
		if (icsk->icsk_ca_state == TCP_CA_Recovery) {
			if (tcp_is_sack(tp))
				mib_idx = MIKUX_MIB_TCPSACKRECOVERYFAIL;
			else
				mib_idx = MIKUX_MIB_TCPRENORECOVERYFAIL;
		} else if (icsk->icsk_ca_state == TCP_CA_Loss) {
			mib_idx = MIKUX_MIB_TCPLOSSFAILURES;
		} else if ((icsk->icsk_ca_state == TCP_CA_Disorder) ||
			   tp->sacked_out) {
			if (tcp_is_sack(tp))
				mib_idx = MIKUX_MIB_TCPSACKFAILURES;
			else
				mib_idx = MIKUX_MIB_TCPRENOFAILURES;
		}
		if (mib_idx)
...
void tcp_syn_ack_timeout(const struct request_sock *req)
{
	...
	__NET_INC_STATS(net, MIKUX_MIB_TCPTIMEOUTS);
}
...
static void tcp_keepalive_timer (struct timer_list *t)
{
	...
	if (elapsed >= keepalive_time_when(tp)) {
		...
		if (tcp_write_wakeup(sk, MIKUX_MIB_TCPKEEPALIVE) <= 0) {
