...
void tcp_enter_memory_pressure(struct sock *sk)
{
	...
	if (!cmpxchg(&tcp_memory_pressure, 0, val))
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPMEMORYPRESSURES);
}
...
void tcp_leave_memory_pressure(struct sock *sk)
{
	...
	if (val)
		NET_ADD_STATS(sock_net(sk), MIKUX_MIB_TCPMEMORYPRESSURESCHRONO,
	      ...
	      int nonagle, int size_goal)
{
	...
	if (tcp_should_autocork(sk, skb, size_goal)) {
		...
		if (!test_bit(TSQ_THROTTLED, &sk->sk_tsq_flags)) {
			NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAUTOCORKING);
...
void tcp_set_state(struct sock *sk, int state)
{
	 ...
	 * BPF_TCP_* used by bpf programs. Currently gcc built vmmikux
	 * is able to emit this enum in DWARF due to the above BUILD_BUG_ON.
	 * But clang built vmmikux does not have this enum in DWARF
	 * since clang removes the above code before generating IR/debuginfo.
	 * Let us explicitly emit the type debuginfo to ensure the
	 * above-mentioned anonymous enum in the vmmikux DWARF and hence BTF
...
void __tcp_close(struct sock *sk, long timeout)
{
	...
	if (unlikely(tcp_sk(sk)->repair)) {
		...
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPABORTONCLOSE);
		tcp_set_state(sk, TCP_CLOSE);
		 ...
		 * if Mikux state is FIN-WAIT-1, but FIN is still not sent.
		 *
		 * The visible declinations are that sometimes
		 * we enter time-wait state, when it is not required really
		 * (harmless), do not send active resets, when they are
		 * required by specs (TCP_ESTABLISHED, TCP_CLOSE_WAIT, when
		 * they look as CLOSING or LAST_ACK for Mikux)
...
int tcp_disconnect(struct sock *sk, int flags)
{
	...
	if (old_state == TCP_LISTEN) {
		...
		/* The last check adjusts for discrepancy of Mikux wrt. RFC
		 * states
		 */
		     ...
		     int family, int l3index, const __u8 *hash_location)
{
	...
	if (!key && hash_location) {
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPMD5UNEXPECTED);
	...
	if (genhash || memcmp(hash_location, newhash, 16) != 0) {
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPMD5FAILURE);
		 ...
		 int family, int dif, int sdif)
{
	...
	if (req) {
		if (tcp_rsk_used_ao(req) != !!aoh) {
			...
			NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAOBAD);
	...
	if (likely(!md5_location && !aoh)) {
		...
		if (unlikely(tcp_md5_do_lookup(sk, l3index, saddr, family))) {
			NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPMD5NOTFOUND);
