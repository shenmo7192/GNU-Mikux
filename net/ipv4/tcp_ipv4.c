 ...
 *		code split from:
 *		mikux/ipv4/tcp.c
 *		mikux/ipv4/tcp_input.c
 *		mikux/ipv4/tcp_output.c
 *
...
#include <mikux/jhash.h>
#include <mikux/init.h>
#include <mikux/times.h>
...
void tcp_req_err(struct sock *sk, u32 seq, bool abort)
{
	...
	if (seq != tcp_rsk(req)->snt_isn) {
		__NET_INC_STATS(net, MIKUX_MIB_OUTOFWINDOWICMPS);
	} else if (abort) {
		/*
...
int tcp_v4_err(struct sk_buff *skb, u32 info)
{
	...
	if (sock_owned_by_user(sk)) {
		if (!(type == ICMP_DEST_UNREACH && code == ICMP_FRAG_NEEDED))
			__NET_INC_STATS(net, MIKUX_MIB_LOCKDROPPEDICMPS);
	}
	...
	if (static_branch_unlikely(&ip4_min_ttl)) {
		...
		if (unlikely(iph->ttl < READ_ONCE(inet_sk(sk)->min_ttl))) {
			__NET_INC_STATS(net, MIKUX_MIB_TCPMINTTLDROP);
			goto out;
		}
	}
	...
	case ICMP_DEST_UNREACH:
		...
		if (code == ICMP_FRAG_NEEDED) { /* PMTU discovery (RFC1191) */
			 ...
			 * (SYN-ACKs send out by Mikux are always <576bytes so
	 ...
	 * their original sense (even Mikux sends invalid PORT_UNREACHs)
	 *
...
no_coalesce:
	...
	if (unlikely(sk_add_backlog(sk, skb, limit))) {
		...
		*reason = SKB_DROP_REASON_SOCKET_BACKLOG;
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPBACKLOGDROP);
		return true;
	}
	return false;
}
