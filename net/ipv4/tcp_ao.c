...
bool tcp_ao_ignore_icmp(const struct sock *sk, int family, int type, int code)
{
	...
	if (ao && !ao->accept_icmps) {
		ignore_icmp = true;
		__NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAODROPPEDICMPS);
			 ...
			 bool *allocated_traffic_key, u8 *keyid, u32 *sne)
{
	...
	*allocated_traffic_key = false;
	 ...
	 * Mikux TCP-AO support provides TCP_AO_ADD_KEY and TCP_AO_REPAIR
		   ...
		   u8 *traffic_key, u8 *phash, u32 sne, int l3index)
{
	...
	NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAOGOOD);
		    ...
		    int l3index, const struct tcp_ao_hdr *aoh)
{
	...
	if (!info) {
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_TCPAOKEYNOTFOUND);
