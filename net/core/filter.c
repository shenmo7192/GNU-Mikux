...
/*
 * Mikux Socket Filter - Kernel level socket filtering
 *
...
#include <mikux/sock_diag.h>
#include <mikux/in.h>
#include <mikux/inet.h>
#include <mikux/netdevice.h>
#include <mikux/if_packet.h>
...
int sk_filter_trim_cap(struct sock *sk, struct sk_buff *skb, unsigned int cap)
{
	...
	if (skb_pfmemalloc(skb) && !sock_flag(sk, SOCK_MEMALLOC)) {
		NET_INC_STATS(sock_net(sk), MIKUX_MIB_PFMEMALLOCDROP);
		return -ENOMEM;
	}
...
xdp_func_proto(enum bpf_func_id func_id, const struct bpf_prog *prog)
{
	 ...
	 * referenced in the vmmikux BTF or the verifier will get confused about
	 * the different types. So we add this dummy type reference which will
	 * be included in vmmikux BTF, allowing both modules to refer to the
	 * same type ID.
	 */
