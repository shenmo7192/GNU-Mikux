		      ...
		      __u32 *root_type_ids, int root_type_cnt, bool sort_dump)
{
	...
	printf("#ifndef __VMMIKUX_H__\n");
	printf("#define __VMMIKUX_H__\n");
	printf("\n");
...
static const char sysfs_vmmikux[] = "/sys/kernel/btf/vmmikux";

static struct btf *get_vmmikux_btf_from_sysfs(void)
{
	struct btf *base;

	base = btf__parse(sysfs_vmmikux, NULL);
	if (!base)
		p_err("failed to parse vmmikux BTF at '%s': %d\n",
		      sysfs_vmmikux, -errno);
...
static bool btf_is_kernel_module(__u32 btf_id)
{
	...
	return btf_info.kernel_btf && strncmp(btf_name, "vmmikux", sizeof(btf_name)) != 0;
}
...
static int do_dump(int argc, char **argv)
{
	...
	if (is_prefix(src, "map")) {
		...
		if (!base_btf &&
		    ...
		    strcmp(*argv, sysfs_vmmikux) != 0)
			base = get_vmmikux_btf_from_sysfs();
	...
	if (!btf) {
		if (!base_btf && btf_is_kernel_module(btf_id)) {
			p_info("Warning: valid base BTF was not specified with -B option, falling back to standard base BTF (%s)",
			       sysfs_vmmikux);
			base_btf = get_vmmikux_btf_from_sysfs();
		}
