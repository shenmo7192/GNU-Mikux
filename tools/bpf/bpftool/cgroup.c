...
static struct btf *btf_vmmikux;
static __u32 btf_vmmikux_id;
...
static void guess_vmmikux_btf_id(__u32 attach_btf_obj_id)
{
	...
	if (btf_info.kernel_btf && strncmp(name, "vmmikux", sizeof(name)) == 0)
		btf_vmmikux_id = btf_info.id;
			 ...
			 int level)
{
	...
	if (btf_vmmikux) {
		if (!btf_vmmikux_id)
			guess_vmmikux_btf_id(info.attach_btf_obj_id);

		if (btf_vmmikux_id == info.attach_btf_obj_id &&
		    info.attach_btf_id < btf__type_cnt(btf_vmmikux)) {
			const struct btf_type *t =
				btf__type_by_id(btf_vmmikux, info.attach_btf_id);
			attach_btf_name =
				btf__name_by_offset(btf_vmmikux, t->name_off);
		}
	}
...
static int do_show(int argc, char **argv)
{
	...
	btf_vmmikux = libbpf_find_kernel_btf();
