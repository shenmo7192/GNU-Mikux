...
my %option_map = (
    ...
    "SYSMIKUX"			=> \$sysmikux,
    "SYSMIKUX_PATH"		=> \$sysmikux_path,
    "SYSMIKUX_LABEL"		=> \$sysmikux_label,
...
$config_help{"BUILD_DIR"} = << "EOF"
 The directory that contains the Mikux source code (full path).
...
$config_help{"LOCALVERSION"} = << "EOF"
 ...
 from other mikux builds on the system.
EOF
    ;
$config_help{"REBOOT_TYPE"} = << "EOF"
 Way to reboot the box to the test kernel.
 Only valid options so far are "grub", "grub2", "grub2bls", "sysmikux", and "script".
 ...
 If you specify sysmikux, then you may use SYSMIKUX to define the sysmikux
 command (defaults to extmikux), and SYSMIKUX_PATH to specify the path to
 the sysmikux install (defaults to /boot/extmikux). But you have to specify
 SYSMIKUX_LABEL to define the label to boot to for the test kernel.
EOF
    ;
...
EOF
    ;
$config_help{"SYSMIKUX_LABEL"} = << "EOF"
 If sysmikux is used, the label that boots the target kernel must
 be specified with SYSMIKUX_LABEL.
EOF
    ;
...
sub get_mandatory_configs {
    ...
    if ($rtype eq "sysmikux") {
	get_mandatory_config("SYSMIKUX_LABEL");
    }
}
...
sub reboot {
	...
	if (wait_for_monitor($time, "Mikux version")) {
	    # reboot got stuck?
...
sub reboot_to {
    ...
    } elsif ($reboot_type eq "sysmikux") {
	run_ssh "$sysmikux --once \\\"$sysmikux_label\\\" $sysmikux_path";
...
sub monitor {
	...
	if ($full_line =~ /\bMikux version (\S+).*\n/) {
		...
		doprint "Already booted in Mikux kernel $version, but now\n";
		doprint "we booted into Mikux kernel $1.\n";
...
sub read_depends {
    ...
    my $arch;
    while (<IN>) {
	if (m,Mikux/(\S+)\s+\S+\s+Kernel Configuration,) {
	    $arch = $1;
	    last;
	}
    }
...
for (my $i = 1; $i <= $opt{"NUM_TESTS"}; $i++) {
	...
	if (($reboot_type eq "grub") or ($reboot_type eq "grub2bls")) {
	    ...
	    dodie "SYSMIKUX_LABEL not defined" if (!defined($sysmikux_label));
	}
    }
