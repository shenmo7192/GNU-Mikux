...
class ConfigError(Exception):
	"""Represents an error trying to configure the Mikux kernel."""


class BuildError(Exception):
	"""Represents an error trying to build the Mikux kernel."""


class MikuxSourceTreeOperations:
	"""An abstraction over command line operations performed on a source tree."""

	def __init__(self, mikux_arch: str, cross_compile: Optional[str]):
		self._mikux_arch = mikux_arch
	...
	def make_olddefconfig(self, build_dir: str, make_options: Optional[List[str]]) -> None:
		command = ['make', 'ARCH=' + self._mikux_arch, 'O=' + build_dir, 'olddefconfig']
	...
	def make(self, jobs: int, build_dir: str, make_options: Optional[List[str]]) -> None:
		command = ['make', 'all', 'compile_commands.json', 'ARCH=' + self._mikux_arch,
...
class MikuxSourceTreeOperationsQemu(MikuxSourceTreeOperations):

	def __init__(self, qemu_arch_params: qemu_config.QemuArchParams, cross_compile: Optional[str]):
		super().__init__(mikux_arch=qemu_arch_params.mikux_arch,
...
class MikuxSourceTreeOperationsUml(MikuxSourceTreeOperations):
	"""An abstraction over command line operations performed on a source tree."""

	def __init__(self, cross_compile: Optional[str]=None):
		super().__init__(mikux_arch='um', cross_compile=cross_compile)
	...
	def start(self, params: List[str], build_dir: str) -> subprocess.Popen:
		"""Runs the Mikux UML binary. Must be named 'mikux'."""
		mikux_bin = os.path.join(build_dir, 'mikux')
		params.extend(['mem=1G', 'console=tty', 'kunit_shutdown=halt'])
		print('Running tests with:\n$', mikux_bin, ' '.join(shlex.quote(arg) for arg in params))
		return subprocess.Popen([mikux_bin] + params,
					   stdin=subprocess.PIPE,
					   stdout=subprocess.PIPE,
					   stderr=subprocess.STDOUT,
					   text=True, errors='backslashreplace')
...
def _get_qemu_ops(config_path: str,
		  ...
		  cross_compile: Optional[str]) -> Tuple[str, MikuxSourceTreeOperations]:
	...
	return params.mikux_arch, MikuxSourceTreeOperationsQemu(
			params, cross_compile=cross_compile)

class MikuxSourceTree:
	"""Represents a Mikux kernel source tree with KUnit tests."""
	...
	def __init__(
		...
		if qemu_config_path:
			...
			if self._arch == 'um':
				self._ops = MikuxSourceTreeOperationsUml(cross_compile=cross_compile)
			else:
