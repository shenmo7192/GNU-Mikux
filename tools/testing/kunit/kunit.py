...
def config_tests(mikux: kunit_kernel.MikuxSourceTree,
		 request: KunitConfigRequest) -> KunitResult:
	stdout.print_with_timestamp('Configuring KUnit Kernel ...')

	config_start = time.time()
	success = mikux.build_reconfig(request.build_dir, request.make_options)
...
def build_tests(mikux: kunit_kernel.MikuxSourceTree,
		request: KunitBuildRequest) -> KunitResult:
	stdout.print_with_timestamp('Building KUnit Kernel ...')

	build_start = time.time()
	success = mikux.build_kernel(request.jobs,
				     request.build_dir,
...
def config_and_build_tests(mikux: kunit_kernel.MikuxSourceTree,
			   request: KunitBuildRequest) -> KunitResult:
	config_result = config_tests(mikux, request)
	if config_result.status != KunitStatus.SUCCESS:
		return config_result

	return build_tests(mikux, request)

def _list_tests(mikux: kunit_kernel.MikuxSourceTree, request: KunitExecRequest) -> List[str]:
	args = ['kunit.action=list']

	if request.kernel_args:
		args.extend(request.kernel_args)

	output = mikux.run_kernel(args=args,
...
def _list_tests_attr(mikux: kunit_kernel.MikuxSourceTree, request: KunitExecRequest) -> Iterable[str]:
...
def exec_tests(mikux: kunit_kernel.MikuxSourceTree, request: KunitExecRequest) -> KunitResult:
	filter_globs = [request.filter_glob]
	if request.list_tests:
		output = _list_tests(mikux, request)
		for line in output:
			print(line.rstrip())
		return KunitResult(status=KunitStatus.SUCCESS, elapsed_time=0.0)
	if request.list_tests_attr:
		attr_output = _list_tests_attr(mikux, request)
		for line in attr_output:
			print(line.rstrip())
		return KunitResult(status=KunitStatus.SUCCESS, elapsed_time=0.0)
	if request.run_isolated:
		tests = _list_tests(mikux, request)
	...
	metadata = kunit_json.Metadata(arch=mikux.arch(), build_dir=request.build_dir, def_config='kunit_defconfig')
	...
	if len(filter_globs) == 1 and test_counts.crashed > 0:
		...
		print('$ scripts/decode_stacktrace.sh {}/vmmikux {} < {} | tee {}/decoded.log | {} parse'.format(
...
def run_tests(mikux: kunit_kernel.MikuxSourceTree,
	...
	build_result = build_tests(mikux, request)
	if build_result.status != KunitStatus.SUCCESS:
		return build_result

	exec_result = exec_tests(mikux, request)
...
def add_common_opts(parser: argparse.ArgumentParser) -> None:
	...
	parser.add_argument('--cross_compile',
			    help=('Sets make\'s CROSS_COMPILE variable; it should '
				  ...
				  'example `sparc64-mikux-gnu-` if you have the '
				  'sparc toolchain installed on your system, or '
				  '`$HOME/toolchains/microblaze/gcc-9.2.0-nolibc/microblaze-mikux/bin/microblaze-mikux-` '
...
def tree_from_args(cli_args: argparse.Namespace) -> kunit_kernel.MikuxSourceTree:
	"""Returns a MikuxSourceTree based on the user's arguments."""
	...
	return kunit_kernel.MikuxSourceTree(cli_args.build_dir,
...
def run_handler(cli_args: argparse.Namespace) -> None:
	...
	result = run_tests(mikux, request)
...
def config_handler(cli_args: argparse.Namespace) -> None:
	...
	request = KunitConfigRequest(build_dir=cli_args.build_dir,
						make_options=cli_args.make_options)
	result = config_tests(mikux, request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (
			result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
...
def build_handler(cli_args: argparse.Namespace) -> None:
	...
	request = KunitBuildRequest(build_dir=cli_args.build_dir,
					make_options=cli_args.make_options,
					jobs=cli_args.jobs)
	result = config_and_build_tests(mikux, request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (
			result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
...
def exec_handler(cli_args: argparse.Namespace) -> None:
	...
	exec_request = KunitExecRequest(raw_output=cli_args.raw_output,
					...
					list_tests=cli_args.list_tests,
					list_tests_attr=cli_args.list_tests_attr)
	result = exec_tests(mikux, exec_request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
		sys.exit(1)
