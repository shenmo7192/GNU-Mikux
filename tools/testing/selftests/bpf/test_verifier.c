...
#include "bpf_util.h"
#include "test_btf.h"
#include "../../../include/mikux/filter.h"
...
static struct btf *btf__load_testmod_btf(struct btf *vmmikux)
{
	...
	while (true) {
		...
		btf = btf__load_from_kernel_by_id_split(id, vmmikux);
		if (!btf) {
			close(fd);
			break;
		}
			      ...
			      struct kfunc_btf_id_pair *fixup_kfunc_btf_id)
{
	...
	while (fixup_kfunc_btf_id->kfunc) {
		...
		vmmikux_btf = vmmikux_btf ?: btf__load_vmmikux_btf();
		if (vmmikux_btf) {
			btf_id = btf__find_by_name_kind(vmmikux_btf,
		...
		if (!btf_id) {
			testmod_btf = testmod_btf ?: btf__load_testmod_btf(vmmikux_btf);
			if (testmod_btf) {
			   ...
			   int *passes, int *errors)
{
	...
	if ((prog_type == BPF_PROG_TYPE_TRACING ||
		...
		attach_btf_id = libbpf_find_vmmikux_btf_id(test->kfunc,
