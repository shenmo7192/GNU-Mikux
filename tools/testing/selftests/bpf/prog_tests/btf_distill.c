...
/* create split reference BTF from vmmikux + split BTF with a few type references;
 ...
 */
static void test_distilled_base_vmmikux(void)
{
	struct btf *split_btf = NULL, *vmmikux_btf = btf__load_vmmikux_btf();
	struct btf *split_dist = NULL, *base_dist = NULL;
	__s32 int_id, myint_id;

	if (!ASSERT_OK_PTR(vmmikux_btf, "load_vmmikux"))
		return;
	int_id = btf__find_by_name_kind(vmmikux_btf, "int", BTF_KIND_INT);
	if (!ASSERT_GT(int_id, 0, "find_int"))
		goto cleanup;
	split_btf = btf__new_empty_split(vmmikux_btf);
...
void test_btf_distill(void)
{
	...
	if (test__start_subtest("distilled_base_vmmikux"))
		test_distilled_base_vmmikux();
