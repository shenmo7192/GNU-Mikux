...
{
	struct btf *vmmikux_btf = NULL, *mod_btf = NULL;
	int err, i;
	int btf_fd[2] = {};
	int prog_fd[2] = {};
	int link_fd[2] = {};
	...
	if (!ASSERT_OK_PTR(vmmikux_btf, "load_vmmikux_btf"))
		return;
	...
	mod_btf = btf_get_from_fd(btf_fd[1], vmmikux_btf);
	if (!ASSERT_OK_PTR(mod_btf, "btf_get_from_fd"))
		goto out;

	btf_id[0] = btf__find_by_name_kind(vmmikux_btf, symbol_name, BTF_KIND_FUNC);
	...
	for (i = 0; i < 2; i++) {
		 ...
		 * vmmikux and the bpf_testmod functions, this will fail on
