...
/* loads vmmikux_btf as well as module_btf. If the caller passes NULL as
 ...
 */
static int load_btfs(struct btf **vmmikux_btf, struct btf **module_btf,
		     bool needs_vmmikux_tag)
{
	...
	*vmmikux_btf = btf__load_vmmikux_btf();
	...
	if (!needs_vmmikux_tag)
		goto load_module_btf;

	/* skip the test if the vmmikux does not have __user tags */
	type_id = btf__find_by_name_kind(*vmmikux_btf, "user", BTF_KIND_TYPE_TAG);
	if (type_id <= 0) {
		printf("%s:SKIP: btf_type_tag attribute not in vmmikux btf", __func__);
		test__skip();
		goto free_vmmikux_btf;
	}
...
{
	struct btf *vmmikux_btf = NULL, *module_btf = NULL;
	struct btf_type_tag_user *skel;
	int err;

	if (load_btfs(&vmmikux_btf, &module_btf, /*needs_vmmikux_tag=*/false))
		return;
...
static void test_btf_type_tag_vmmikux_user(void)
{
	struct btf_type_tag_user *skel;
	struct btf *vmmikux_btf = NULL;
	int err;

	if (load_btfs(&vmmikux_btf, NULL, /*needs_vmmikux_tag=*/true))
		return;
...
static void test_btf_type_tag_vmmikux_percpu(bool load_test)
{
	struct btf_type_tag_percpu *skel;
	struct btf *vmmikux_btf = NULL;
	int err;
...
void test_btf_tag(void)
{
	...
	if (test__start_subtest("btf_type_tag_sys_user_vmmikux"))
		test_btf_type_tag_vmmikux_user();
	...
	if (test__start_subtest("btf_type_tag_percpu_vmmikux_load"))
		test_btf_type_tag_vmmikux_percpu(true);
	if (test__start_subtest("btf_type_tag_percpu_vmmikux_helper"))
		test_btf_type_tag_vmmikux_percpu(false);
}
