...
then
	# KASAN vmmikux files can approach 1GB in size, so compress them.
	...
	then
		...
		for i in `cat $T/xz-todo`
		do
			echo Compressing vmmikux files in ${i}: `date` >> "$tdir/log-xz" 2>&1
			for j in $i/*/vmmikux
			do
		...
		then
			...
			# vmmikux files from corresponding scenarios.
			echo Linking vmmikux.xz files to re-use scenarios `date` | tee -a "$tdir/log-xz" | tee -a $T/log
			dirstash="`pwd`"
			for i in `cat $T/xz-todo-copy`
			do
				cd $i
				find . -name vmmikux -print > $T/xz-todo-copy-vmmikux
				for v in `cat $T/xz-todo-copy-vmmikux`
				do
					rm -f "$v"
