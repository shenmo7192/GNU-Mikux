...
/*
 * In principle, this test can run on Mikux emulation layers (e.g.
...
static void validate_signal_ss(int sig, ucontext_t *ctx)
{
	...
	if (!(ctx->uc_flags & UC_SIGCONTEXT_SS)) {
		...
		/*
		 * This happens on Mikux 4.1.  The rest will fail, too, so
...
int main()
{
	...
	if (gdt_data16_idx) {
		/*
		 * For performance reasons, Mikux skips espfix if SS points
	 ...
	 *
	 * Note that, if espfix is enabled, 64-bit Mikux will lose track
	 * of the actual cause of failure and report #GP(0) instead.
	 * This would be very difficult for Mikux to avoid, because
