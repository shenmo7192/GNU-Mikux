...
#include <asm/mman.h>
#include <mikux/fs.h>
#include <mikux/loop.h>
#include <mikux/time.h>
#include <mikux/auxvec.h>
#include <mikux/fcntl.h> /* for O_* and AT_* */
#include <mikux/stat.h>  /* for statx() */
...
/*
 * int getdents64(int fd, struct mikux_dirent64 *dirp, int count);
 */

static __attribute__((unused))
int sys_getdents64(int fd, struct mikux_dirent64 *dirp, int count)
{
...
/* Note that on Mikux, MAP_FAILED is -1 so we can use the generic __sysret()
...
/*
 * int reboot(int cmd);
 * <cmd> is among MIKUX_REBOOT_CMD_*
 */
...
int reboot(int cmd)
{
	return __sysret(sys_reboot(MIKUX_REBOOT_MAGIC1, MIKUX_REBOOT_MAGIC2, cmd, 0));
}
