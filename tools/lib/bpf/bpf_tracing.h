...
#if defined(__KERNEL__) || defined(__VMMIKUX_H__)
 ...
 * comments in Mikux sources. And refer to syscall(2) manpage.
 */
...
#endif /* __KERNEL__ || __VMMIKUX_H__ */
...
/*
 * http://refspecs.mikux-foundation.org/elf/elfspec_ppc.pdf (page 3-14,
...
name(struct pt_regs *ctx);						    \
extern _Bool MIKUX_HAS_SYSCALL_WRAPPER __kconfig;			    \
static __always_inline typeof(name(0))					    \
____##name(struct pt_regs *ctx, ##args);				    \
typeof(name(0)) name(struct pt_regs *ctx)				    \
{									    \
	struct pt_regs *regs = MIKUX_HAS_SYSCALL_WRAPPER		    \
			       ? (struct pt_regs *)PT_REGS_PARM1(ctx)	    \
			       : ctx;					    \
	_Pragma("GCC diagnostic push")					    \
	_Pragma("GCC diagnostic ignored \"-Wint-conversion\"")		    \
	if (MIKUX_HAS_SYSCALL_WRAPPER)					    \
