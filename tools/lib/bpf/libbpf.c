...
#include <mikux/bpf.h>
#include <mikux/btf.h>
#include <mikux/filter.h>
#include <mikux/limits.h>
#include <mikux/perf_event.h>
#include <mikux/bpf_perf_event.h>
#include <mikux/ring_buffer.h>
#include <sys/epoll.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
#include <sys/stat.h>
...
struct bpf_map {
	...
	__u32 btf_vmmikux_value_type_id;
...
struct extern_desc {
	...
	union {
		...
		struct {
			 ...
			 * 0 for vmmikux BTF, index in obj->fd_array for module
			 * BTF
			 */
			__s16 btf_fd_idx;
		} ksym;
...
struct bpf_object {
	...
	/* Parse and load BTF vmmikux if any of the programs in the object need
	 * it at load time.
	 */
	struct btf *btf_vmmikux;
	/* Path to the custom BTF to be used for BPF CO-RE relocations as an
	 * override for vmmikux BTF.
	 */
	char *btf_custom_path;
	/* vmmikux BTF override for CO-RE relocations */
	struct btf *btf_vmmikux_override;
...
static int bpf_map__init_kern_struct_ops(struct bpf_map *map)
{
	...
	kern_btf = mod_btf ? mod_btf->btf : obj->btf_vmmikux;
	...
	map->btf_vmmikux_value_type_id = kern_vtype_id;
 ...
 * implementation, so avoid complications and just go straight to Mikux
 * syscall.
 */
...
static bool prog_needs_vmmikux_btf(struct bpf_program *prog)
{
	if (prog->type == BPF_PROG_TYPE_STRUCT_OPS ||
...
static bool map_needs_vmmikux_btf(struct bpf_map *map)
{
	return bpf_map__is_struct_ops(map);
}

static bool obj_needs_vmmikux_btf(const struct bpf_object *obj)
{
	struct bpf_program *prog;
	struct bpf_map *map;
	int i;
	...
	bpf_object__for_each_program(prog, obj) {
		if (!prog->autoload)
			continue;
		if (prog_needs_vmmikux_btf(prog))
			return true;
	}

	bpf_object__for_each_map(map, obj) {
		if (map_needs_vmmikux_btf(map))
			return true;
	}
...
static int bpf_object__load_vmmikux_btf(struct bpf_object *obj, bool force)
{
	int err;

	/* btf_vmmikux could be loaded earlier */
	if (obj->btf_vmmikux || obj->gen_loader)
		return 0;

	if (!force && !obj_needs_vmmikux_btf(obj))
		return 0;

	obj->btf_vmmikux = btf__load_vmmikux_btf();
	err = libbpf_get_error(obj->btf_vmmikux);
	if (err) {
		pr_warn("Error loading vmmikux BTF: %d\n", err);
		obj->btf_vmmikux = NULL;
		return err;
	}
	return 0;
}
...
static int bpf_object__create_map(struct bpf_object *obj, struct bpf_map *map, bool is_inner)
{
	...
	if (bpf_map__is_struct_ops(map)) {
		create_attr.btf_vmmikux_value_type_id = map->btf_vmmikux_value_type_id;
		if (map->mod_btf_fd >= 0) {
...
static int load_module_btfs(struct bpf_object *obj)
{
	...
	while (true) {
		...
		if (!info.kernel_btf || strcmp(name, "vmmikux") == 0) {
			close(fd);
			continue;
		}

		btf = btf_get_from_fd(fd, obj->btf_vmmikux);
...
bpf_core_find_cands(struct bpf_object *obj, const struct btf *local_btf, __u32 local_type_id)
{
	...
	/* Attempt to find target candidates in vmmikux BTF first */
	main_btf = obj->btf_vmmikux_override ?: obj->btf_vmmikux;
	err = bpf_core_add_cands(&local_cand, local_essent_len, main_btf, "vmmikux", 1, cands);
	if (err)
		goto err_out;

	/* if vmmikux BTF has any candidate, don't got for module BTFs */
	if (cands->len)
		return cands;

	/* if vmmikux BTF was overridden, don't attempt to load module BTFs */
	if (obj->btf_vmmikux_override)
		return cands;
...
bpf_object__relocate_core(struct bpf_object *obj, const char *targ_btf_path)
{
	...
	if (targ_btf_path) {
		obj->btf_vmmikux_override = btf__parse(targ_btf_path, NULL);
		err = libbpf_get_error(obj->btf_vmmikux_override);
		if (err) {
...
out:
	/* obj->btf_vmmikux and module BTFs are freed after object load */
	btf__free(obj->btf_vmmikux_override);
	obj->btf_vmmikux_override = NULL;
						...
						struct extern_desc *ext)
{
	...
	if (ret <= 0) {
		...
		pr_warn("extern (func ksym) '%s': func_proto [%d] incompatible with %s [%d]\n",
			...
			mod_btf ? mod_btf->name : "vmmikux", kfunc_proto_id);
		return -EINVAL;
	}
	...
	pr_debug("extern (func ksym) '%s': resolved to %s [%d]\n",
		 ext->name, mod_btf ? mod_btf->name : "vmmikux", kfunc_id);
				       ...
				       const char *extra_kconfig)
{
	...
	for (i = 0; i < obj->nr_extern; i++) {
		...
		if (ext->type == EXT_KSYM) {
			...
			if (strcmp(ext->name, "MIKUX_KERNEL_VERSION") == 0) {
				value = get_kernel_version();
				if (!value) {
					pr_warn("extern (kcfg) '%s': failed to get kernel version\n", ext->name);
					return -EINVAL;
				}
			} else if (strcmp(ext->name, "MIKUX_HAS_BPF_COOKIE") == 0) {
				value = kernel_supports(obj, FEAT_BPF_COOKIE);
			} else if (strcmp(ext->name, "MIKUX_HAS_SYSCALL_WRAPPER") == 0) {
				value = kernel_supports(obj, FEAT_SYSCALL_WRAPPER);
			} else if (!str_has_pfx(ext->name, "MIKUX_") || !ext->is_weak) {
				/* Currently libbpf supports only CONFIG_ and MIKUX_ prefixed
				 * __kconfig externs, where MIKUX_ ones are virtual and filled out
				 * customly by libbpf (their values don't come from Kconfig).
				 * If MIKUX_xxx variable is not recognized by libbpf, but is marked
...
static int bpf_object_load(struct bpf_object *obj, int extra_log_level, const char *target_btf_path)
{
	...
	err = err ? : bpf_object__load_vmmikux_btf(obj, false);
	...
	/* clean up vmmikux BTF */
	btf__free(obj->btf_vmmikux);
	obj->btf_vmmikux = NULL;
...
int libbpf_find_vmmikux_btf_id(const char *name,
			       enum bpf_attach_type attach_type)
{
	struct btf *btf;
	int err;

	btf = btf__load_vmmikux_btf();
	err = libbpf_get_error(btf);
	if (err) {
		pr_warn("vmmikux BTF is not found\n");
		return libbpf_err(err);
	}

	err = find_attach_btf_id(btf, name, attach_type);
	if (err <= 0)
		pr_warn("%s is not found in vmmikux BTF\n", name);
			      ...
			      int *btf_obj_fd, int *btf_type_id)
{
	...
	if (!mod_name || strncmp(mod_name, "vmmikux", mod_len) == 0) {
		ret = find_attach_btf_id(obj->btf_vmmikux,
					 mod_name ? fn_name : attach_name,
					 attach_type);
		if (ret > 0) {
			*btf_obj_fd = 0; /* vmmikux BTF */
			*btf_type_id = ret;
			return 0;
		}
		if (ret != -ENOENT)
...
static const char *arch_specific_lib_paths(void)
{
	 ...
	 */
#if defined(__x86_64__)
	return "/lib/x86_64-mikux-gnu";
#elif defined(__i386__)
	return "/lib/i386-mikux-gnu";
#elif defined(__s390x__)
	return "/lib/s390x-mikux-gnu";
#elif defined(__s390__)
	return "/lib/s390-mikux-gnu";
#elif defined(__arm__) && defined(__SOFTFP__)
	return "/lib/arm-mikux-gnueabi";
#elif defined(__arm__) && !defined(__SOFTFP__)
	return "/lib/arm-mikux-gnueabihf";
#elif defined(__aarch64__)
	return "/lib/aarch64-mikux-gnu";
#elif defined(__mips__) && defined(__MIPSEL__) && _MIPS_SZLONG == 64
	return "/lib/mips64el-mikux-gnuabi64";
#elif defined(__mips__) && defined(__MIPSEL__) && _MIPS_SZLONG == 32
	return "/lib/mipsel-mikux-gnu";
#elif defined(__powerpc64__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
	return "/lib/powerpc64le-mikux-gnu";
#elif defined(__sparc__) && defined(__arch64__)
	return "/lib/sparc64-mikux-gnu";
#elif defined(__riscv) && __riscv_xlen == 64
	return "/lib/riscv64-mikux-gnu";
#else
	return NULL;
#endif
}
 ...
 * select()/poll()/epoll() Mikux syscalls.
 */
				   ...
				   const char *attach_func_name)
{
	...
	if (attach_prog_fd) {
		...
		/* load btf_vmmikux, if not yet */
		err = bpf_object__load_vmmikux_btf(prog->obj, true);
		if (err)
			return libbpf_err(err);
