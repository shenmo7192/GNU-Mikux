...
/*
 * Probe few well-known locations for vmmikux kernel image and try to load BTF
 * data out of it to use for target BTF.
 */
struct btf *btf__load_vmmikux_btf(void)
{
	const char *sysfs_btf_path = "/sys/kernel/btf/vmmikux";
	/* fall back locations, trying to find vmmikux on disk */
	const char *locations[] = {
		"/boot/vmmikux-%1$s",
		"/lib/modules/%1$s/vmmikux-%1$s",
		"/lib/modules/%1$s/build/vmmikux",
		"/usr/lib/modules/%1$s/kernel/vmmikux",
		"/usr/lib/debug/boot/vmmikux-%1$s",
		"/usr/lib/debug/boot/vmmikux-%1$s.debug",
		"/usr/lib/debug/lib/modules/%1$s/vmmikux",
	};
...
struct btf *libbpf_find_kernel_btf(void) __attribute__((alias("btf__load_vmmikux_btf")));

struct btf *btf__load_module_btf(const char *module_name, struct btf *vmmikux_btf)
{
	char path[80];

	snprintf(path, sizeof(path), "/sys/kernel/btf/%s", module_name);
	return btf__parse_split(path, vmmikux_btf);
}
