...
class Family(SpecFamily):
    def __init__(self, file_name, exclude_ops):
        ...
        if 'uapi-header' in self.yaml:
            ...
            self.uapi_header = f"mikux/{self.ident_name}.h"
        if self.uapi_header.startswith("mikux/") and self.uapi_header.endswith('.h'):
...
def render_uapi(family, cw):
    hdr_prot = f"_UAPI_MIKUX_{c_upper(family.uapi_header_name)}_H"
...
def main():
    ...
    try:
        ...
        if parsed.license != '((GPL-2.0 WITH Mikux-syscall-note) OR BSD-3-Clause)':
            print('Spec license:', parsed.license)
            print('License must be: ((GPL-2.0 WITH Mikux-syscall-note) OR BSD-3-Clause)')
            os.sys.exit(1)
    ...
    hdr_prot = f"_MIKUX_{parsed.c_name.upper()}_GEN_H"
    if args.header:
        cw.p('#ifndef ' + hdr_prot)
        cw.p('#define ' + hdr_prot)
        cw.nl()
    ...
    if args.mode == 'kernel':
        ...
        if args.header:
            cw.p('#include <mikux/types.h>')
            if family_contains_bitfield32(parsed):
                cw.p('#include <mikux/netlink.h>')
        else:
            cw.p(f'#include "{hdr_file}"')
            cw.p('#include "ynl.h"')
    ...
    if args.mode == "user":
        if not args.header:
            cw.p("#include <mikux/genetlink.h>")
            cw.nl()
