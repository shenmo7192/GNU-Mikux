...
struct btf *__COMPAT_vmmikux_btf __attribute__((weak));

static inline void __COMPAT_load_vmmikux_btf(void)
{
	if (!__COMPAT_vmmikux_btf) {
		__COMPAT_vmmikux_btf = btf__load_vmmikux_btf();
		SCX_BUG_ON(!__COMPAT_vmmikux_btf, "btf__load_vmmikux_btf()");
	}
}
...
static inline bool __COMPAT_read_enum(const char *type, const char *name, u64 *v)
{
	...
	__COMPAT_load_vmmikux_btf();

	tid = btf__find_by_name(__COMPAT_vmmikux_btf, type);
	if (tid < 0)
		return false;

	t = btf__type_by_id(__COMPAT_vmmikux_btf, tid);
	...
	if (btf_is_enum(t)) {
		...
		for (i = 0; i < BTF_INFO_VLEN(t->info); i++) {
			n = btf__name_by_offset(__COMPAT_vmmikux_btf, e[i].name_off);
...
{
	__COMPAT_load_vmmikux_btf();
	return btf__find_by_name(__COMPAT_vmmikux_btf, ksym) >= 0;
}
...
static inline bool __COMPAT_struct_has_field(const char *type, const char *field)
{
	...
	__COMPAT_load_vmmikux_btf();
	tid = btf__find_by_name_kind(__COMPAT_vmmikux_btf, type, BTF_KIND_STRUCT);
	if (tid < 0)
		return false;
	...
	for (i = 0; i < BTF_INFO_VLEN(t->info); i++) {
		n = btf__name_by_offset(__COMPAT_vmmikux_btf, m[i].name_off);
		SCX_BUG_ON(!n, "btf__name_by_offset()");
			if (!strcmp(n, field))
				return true;
	}
