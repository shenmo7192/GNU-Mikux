...
#ifndef F_MIKUX_SPECIFIC_BASE
# define F_MIKUX_SPECIFIC_BASE	1024
#endif
...
static void trace__load_vmmikux_btf(struct trace *trace __maybe_unused)
{
#ifdef HAVE_LIBBPF_SUPPORT
	if (trace->btf != NULL)
		return;

	trace->btf = btf__load_vmmikux_btf();
	if (verbose > 0) {
		fprintf(trace->output, trace->btf ? "vmmikux BTF loaded\n" :
						    "Failed to load vmmikux BTF\n");
	}
#endif
}
...
static const char *fcntl_mikux_specific_cmds[] = {
	"SETLEASE", "GETLEASE", "NOTIFY", "DUPFD_QUERY", [5] = "CANCELLK", "DUPFD_CLOEXEC",
	"SETPIPE_SZ", "GETPIPE_SZ", "ADD_SEALS", "GET_SEALS",
	"GET_RW_HINT", "SET_RW_HINT", "GET_FILE_RW_HINT", "SET_FILE_RW_HINT",
};

static DEFINE_STRARRAY_OFFSET(fcntl_mikux_specific_cmds, "F_", F_MIKUX_SPECIFIC_BASE);

static struct strarray *fcntl_cmds_arrays[] = {
	&strarray__fcntl_cmds,
	&strarray__fcntl_mikux_specific_cmds,
};
...
static bool syscall_arg__strtoul_btf_type(char *bf, size_t size, struct syscall_arg *arg, u64 *val)
{
	...
	trace__load_vmmikux_btf(arg->trace);
