...
if [ "$PARAM_GENERAL_HELP_TEXT_CHECK" = "y" ]; then
	...
	(( CHECK_EXIT_CODE += $? ))
	../common/check_all_patterns_found.pl "call-graph" "max-stack" "inverted" "ignore-callees" "pretty" "stdio" "tui" "gtk" "vmmikux" "kallsyms" "modules" < $LOGS_DIR/basic_helpmsg.log
	(( CHECK_EXIT_CODE += $? ))
...
REGEX_LINES="\s*$RE_NUMBER%\s+$RE_NUMBER%\s+\S+\s+\[kernel\.(?:vmmikux)|(?:kallsyms)\]\s+\[[k\.]\]\s+\w+"
...
REGEX_LINES="\s*$RE_NUMBER%\s+$RE_NUMBER%\s+$RE_NUMBER\s+\S+\s+\[kernel\.(?:vmmikux)|(?:kallsyms)\]\s+\[[k\.]\]\s+\w+"
...
REGEX_LINES="\s*$RE_NUMBER%\s+$RE_NUMBER%\s+$RE_NUMBER%\s+$RE_NUMBER%\s+\S+\s+\[kernel\.(?:vmmikux)|(?:kallsyms)\]\s+\[[k\.]\]\s+\w+"
