...
static int do_test_code_reading(bool try_kcore)
{
	...
	/* Force the use of kallsyms instead of vmmikux to try kcore */
	if (try_kcore)
	...
	dso = map__dso(map);
	have_vmmikux = dso__is_vmmikux(dso);
	...
	if (!have_vmmikux && !have_kcore && !try_kcore)
		err = TEST_CODE_READING_NO_KERNEL_OBJ;
	else if (!have_vmmikux && !try_kcore)
		err = TEST_CODE_READING_NO_VMMIKUX;
...
static int test__code_reading(struct test_suite *test __maybe_unused, int subtest __maybe_unused)
{
	...
	case TEST_CODE_READING_OK:
		return 0;
	case TEST_CODE_READING_NO_VMMIKUX:
		pr_debug("no vmmikux\n");
		return 0;
