 ...
 *
 *   Arjan van de Ven <arjan@mikux.intel.com>
...
static int perf_top__parse_source(struct perf_top *top, struct hist_entry *he)
{
	...
	if (dso__symtab_type(dso) == DSO_BINARY_TYPE__KALLSYMS && !dso__is_kcore(dso)) {
		pr_err("Can't annotate %s: No vmmikux file was found in the "
		       "path\n", sym->name);
		sleep(1);
		return -1;
	}
...
static void ui__warn_map_erange(struct map *map, struct symbol *sym, u64 ip)
{
	...
	ui__warning("Out of bounds address found:\n\n"
		    ...
		    "Please report to mikux-kernel@vger.kernel.org\n",
...
"Kernel%s samples will not be resolved.\n",
	...
	if (al.sym == NULL && al.map != NULL) {
		 ...
		 * specified vmmikux file is invalid when we actually have a
		 ...
		 * invalid --vmmikux ;-)
		 */
		if (!machine->kptr_restrict_warned && !top->vmmikux_warned &&
			...
			if (symbol_conf.vmmikux_name) {
				...
				ui__warning("The %s file can't be used: %s\n%s",
					    symbol_conf.vmmikux_name, serr, msg);
			} else {
				ui__warning("A vmmikux file was not found.\n%s",
					    msg);
			}

			if (use_browser <= 0)
				sleep(5);
			top->vmmikux_warned = true;
		}
	}
...
int cmd_top(int argc, const char **argv)
{
	...
	OPT_STRING('k', "vmmikux", &symbol_conf.vmmikux_name,
		   "file", "vmmikux pathname"),
	OPT_BOOLEAN(0, "ignore-vmmikux", &symbol_conf.ignore_vmmikux,
		    "don't load vmmikux even if found"),
	...
	symbol_conf.try_vmmikux_path = (symbol_conf.vmmikux_name == NULL);
