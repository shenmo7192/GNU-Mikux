...
/* Initialize symbol maps and path of vmmikux/modules */
...
out:
	if (ret < 0)
		pr_warning("Failed to init vmmikux path.\n");
	return ret;
}
...
static char *find_module_name(const char *module)
{
	 ...
	 * maps to 'struct module' from mikux/module.h. This section contains
	 * actual module name which will be used by kernel after loading it.
	 * But, we cannot use 'struct module' here since mikux/module.h is not
...
static int kernel_get_module_dso(const char *module, struct dso **pdso)
{
	...
	*dso__load_errno(dso) = 0;
	if (vmmikux_name)
		ret = dso__load_vmmikux(dso, map, vmmikux_name, false);
	else
		ret = dso__load_vmmikux_path(dso, map);
found:
	*pdso = dso;
	return ret;
}
					   ...
					   struct map *map, u64 offs)
{
	...
	if (!sym) {
		 ...
		 */
		return (symbol_conf.ignore_vmmikux_buildid) ? 0 : -ENOENT;
	}
					  ...
					  struct probe_trace_event **tevs)
{
	 ...
	 * Perf failed to add kretprobe event with debuginfo of vmmikux which is
	...
	if (ntevs == 0)	{	/* No error but failed to find probe point. */
		...
		if (ntevs == -EBADF)
			pr_warning("Warning: No dwarf info found in the vmmikux - "
