...
/*
 * Returns pointer with encoded error via <mikux/err.h> interface.
 */
#ifdef HAVE_LIBTRACEEVENT
			 ...
			 int err, char *msg, size_t size)
{
	...
	case EACCES:
		...
		if (!sysfs__read_int("fs/semikux/enforce", &enforced)) {
			if (enforced) {
				printed += scnprintf(msg + printed, size - printed,
		...
		return scnprintf(msg + printed, size - printed,
		 ...
		 "without CAP_PERFMON, CAP_SYS_PTRACE or CAP_SYS_ADMIN Mikux capability.\n"
