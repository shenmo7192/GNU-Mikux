...
{
	/*
	 * Usually vmmikux is an ELF file with type ET_EXEC for most
		 ...
		 enum dso_binary_type type)
{
	...
	if (dso__has_build_id(dso) && !symbol_conf.ignore_vmmikux_buildid) {
		  ...
		  struct symsrc *runtime_ss, int kmodule)
{
	...
	if (!syms_ss->symtab) {
		/*
		 * If the vmmikux is stripped, fail so we will fall back
		 * to using kallsyms. The vmmikux runtime symbols aren't
		 * of much use.
		 */
		if (dso__kernel(dso))
			return err;
