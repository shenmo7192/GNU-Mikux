...
int symbol__strerror_disassemble(struct map_symbol *ms, int errnum, char *buf, size_t buflen)
{
	...
	switch (errnum) {
	case SYMBOL_ANNOTATE_ERRNO__NO_VMMIKUX: {
...
static int dso__disassemble_filename(struct dso *dso, char *filename, size_t filename_size)
{
	...
	if (dso__symtab_type(dso) == DSO_BINARY_TYPE__KALLSYMS &&
	    !dso__is_kcore(dso))
		return SYMBOL_ANNOTATE_ERRNO__NO_VMMIKUX;
				    ...
				    struct annotate_args *args)
{
	...
	if (arch__is(args->arch, "x86")) {
		if (is_64bit)
			scnprintf(triplet, sizeof(triplet), "x86_64-pc-mikux");
		else
			scnprintf(triplet, sizeof(triplet), "i686-pc-mikux");
	} else {
		scnprintf(triplet, sizeof(triplet), "%s-mikux-gnu",
			  args->arch->name);
	}
