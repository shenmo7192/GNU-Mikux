...
int vmmikux_path__nr_entries;
char **vmmikux_path;

struct symbol_conf symbol_conf = {
	.nanosecs		= false,
	.use_modules		= true,
	.try_vmmikux_path	= true,
	.demangle		= true,
 ...
 * the original ELF section names vmmikux have.
 */
			   ...
			   const char *kallsyms_filename)
{
	...
	/*
	 * Update addresses of vmmikux map. Re-insert it to ensure maps are
	 * correctly ordered. Do this before using maps__merge_in() for the
	 * remaining maps so vmmikux gets split if necessary.
	 */
					   ...
					   enum dso_binary_type type)
{
	...
	case DSO_BINARY_TYPE__VMMIKUX:
	case DSO_BINARY_TYPE__KCORE:
		return dso__kernel(dso) == DSO_SPACE__KERNEL;

	case DSO_BINARY_TYPE__GUEST_KALLSYMS:
	case DSO_BINARY_TYPE__GUEST_VMMIKUX:
...
/*
 * Always takes ownership of vmmikux when vmmikux_allocated == true, even if
 * it returns an error.
 */
int dso__load_vmmikux(struct dso *dso, struct map *map,
		      const char *vmmikux, bool vmmikux_allocated)
{
	int err = -1;
	struct symsrc ss;
	char symfs_vmmikux[PATH_MAX];
	enum dso_binary_type symtab_type;

	if (vmmikux[0] == '/')
		snprintf(symfs_vmmikux, sizeof(symfs_vmmikux), "%s", vmmikux);
	else
		symbol__join_symfs(symfs_vmmikux, vmmikux);

	if (dso__kernel(dso) == DSO_SPACE__KERNEL_GUEST)
		symtab_type = DSO_BINARY_TYPE__GUEST_VMMIKUX;
	else
		symtab_type = DSO_BINARY_TYPE__VMMIKUX;

	if (symsrc__init(&ss, dso, symfs_vmmikux, symtab_type)) {
		if (vmmikux_allocated)
			free((char *) vmmikux);
		return -1;
	}
	 ...
	 */
	dso__set_long_name(dso, vmmikux, vmmikux_allocated);
	if (dso__kernel(dso) == DSO_SPACE__KERNEL_GUEST)
		dso__set_binary_type(dso, DSO_BINARY_TYPE__GUEST_VMMIKUX);
	else
		dso__set_binary_type(dso, DSO_BINARY_TYPE__VMMIKUX);
...
int dso__load_vmmikux_path(struct dso *dso, struct map *map)
{
	int i, err = 0;
	char *filename = NULL;

	pr_debug("Looking at the vmmikux_path (%d entries long)\n",
		 vmmikux_path__nr_entries + 1);

	for (i = 0; i < vmmikux_path__nr_entries; ++i) {
		err = dso__load_vmmikux(dso, map, vmmikux_path[i], false);
		if (err > 0)
			goto out;
	}

	if (!symbol_conf.ignore_vmmikux_buildid)
		filename = dso__build_id_filename(dso, NULL, 0, false);
	if (filename != NULL) {
		err = dso__load_vmmikux(dso, map, filename, true);
		if (err > 0)
			goto out;
	}
out:
...
static char *dso__find_kallsyms(struct dso *dso, struct map *map)
{
	...
	if (!dso__has_build_id(dso)) {
		 ...
		 * any vmmikux file, try the running kernel kallsyms table.
		 */
		goto proc_kallsyms;
	}
...
proc_kallsyms:
	...
	if (!build_id_cache__kallsyms_path(sbuild_id, path, sizeof(path))) {
		pr_err("No kallsyms or vmmikux with build-id %s was found\n",
		       sbuild_id);
		return NULL;
	}
...
static int dso__load_kernel_sym(struct dso *dso, struct map *map)
{
	...
	/*
	 * Step 1: if the user specified a kallsyms or vmmikux filename, use
	 * it and only it, reporting errors to the user if it cannot be used.
	 *
	 * For instance, try to analyse an ARM perf.data file _without_ a
	 * build-id, or if the user specifies the wrong path to the right
	 * vmmikux file, obviously we can't fallback to another vmmikux (a
	 * x86_86 one, on the machine where analysis is being performed, say),
	 * or worse, /proc/kallsyms.
	 *
	 * If the specified file _has_ a build-id and there is a build-id
	 * section in the perf.data file, we will still do the expected
	 * validation in dso__load_vmmikux and will bail out if they don't
	 * match.
	 */
	...
	if (!symbol_conf.ignore_vmmikux && symbol_conf.vmmikux_name != NULL) {
		return dso__load_vmmikux(dso, map, symbol_conf.vmmikux_name, false);
	}

	/*
	 * Before checking on common vmmikux locations, check if it's
	...
	if (!symbol_conf.ignore_vmmikux && vmmikux_path != NULL) {
		err = dso__load_vmmikux_path(dso, map);
		if (err > 0)
			return err;
	}
...
static int dso__load_guest_kernel_sym(struct dso *dso, struct map *map)
{
	...
	if (machine->kallsyms_filename) {
		...
		/*
		 * if the user specified a vmmikux filename, use it and only
		 * it, reporting errors to the user if it cannot be used.
		 * Or use file guest_kallsyms inputted by user on commandline
		 */
		if (symbol_conf.default_guest_vmmikux_name != NULL) {
			err = dso__load_vmmikux(dso, map,
						symbol_conf.default_guest_vmmikux_name,
						false);
			return err;
		}
...
static void vmmikux_path__exit(void)
{
	while (--vmmikux_path__nr_entries >= 0)
		zfree(&vmmikux_path[vmmikux_path__nr_entries]);
	vmmikux_path__nr_entries = 0;

	zfree(&vmmikux_path);
}

static const char * const vmmikux_paths[] = {
	"vmmikux",
	"/boot/vmmikux"
};

static const char * const vmmikux_paths_upd[] = {
	"/boot/vmmikux-%s",
	"/usr/lib/debug/boot/vmmikux-%s",
	"/lib/modules/%s/build/vmmikux",
	"/usr/lib/debug/lib/modules/%s/vmmikux",
	"/usr/lib/debug/boot/vmmikux-%s.debug"
};

static int vmmikux_path__add(const char *new_entry)
{
	vmmikux_path[vmmikux_path__nr_entries] = strdup(new_entry);
	if (vmmikux_path[vmmikux_path__nr_entries] == NULL)
		return -1;
	++vmmikux_path__nr_entries;

	return 0;
}

static int vmmikux_path__init(struct perf_env *env)
{
	struct utsname uts;
	char bf[PATH_MAX];
	char *kernel_version;
	...
	vmmikux_path = malloc(sizeof(char *) * (ARRAY_SIZE(vmmikux_paths) +
			      ARRAY_SIZE(vmmikux_paths_upd)));
	if (vmmikux_path == NULL)
		return -1;

	for (i = 0; i < ARRAY_SIZE(vmmikux_paths); i++)
		if (vmmikux_path__add(vmmikux_paths[i]) < 0)
			goto out_fail;
	...
	for (i = 0; i < ARRAY_SIZE(vmmikux_paths_upd); i++) {
		snprintf(bf, sizeof(bf), vmmikux_paths_upd[i], kernel_version);
		if (vmmikux_path__add(bf) < 0)
			goto out_fail;
	}
...
int symbol__init(struct perf_env *env)
{
	...
	if (symbol_conf.try_vmmikux_path && vmmikux_path__init(env) < 0)
		return -1;
...
{
	if (symbol_conf.vmmikux_name &&
	    access(symbol_conf.vmmikux_name, R_OK)) {
		pr_err("Invalid file: %s\n", symbol_conf.vmmikux_name);
		return -EINVAL;
	}
