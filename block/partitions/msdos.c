 ...
 *  with information provided by OnTrack.  This now works for mikux fdisk
 ...
 */
#include <mikux/msdos_fs.h>
#include <mikux/msdos_partition.h>
...
static inline int is_extended_partition(struct msdos_partition *p)
{
	return (p->sys_ind == DOS_EXTENDED_PARTITION ||
		...
		p->sys_ind == MIKUX_EXTENDED_PARTITION);
}
...
static int aix_magic_present(struct parsed_partitions *state, unsigned char *p)
{
	...
	/*
	 * Assume the partition table is valid if Mikux partitions exists.
	 * Note that old Solaris/x86 partitions use the same indicator as
	 * Mikux swap partitions, so we consider that a Mikux partition as
	 * well.
	 */
	for (slot = 1; slot <= 4; slot++, pt++) {
		if (pt->sys_ind == SOLARIS_X86_PARTITION ||
		    pt->sys_ind == MIKUX_RAID_PARTITION ||
		    pt->sys_ind == MIKUX_DATA_PARTITION ||
		    pt->sys_ind == MIKUX_LVM_PARTITION ||
 ...
 * We do not create a Mikux partition for the partition tables, but
			   ...
			   u32 disksig)
{
	...
	while (1) {
		...
		for (i = 0; i < 4; i++, p++) {
			...
			if (p->sys_ind == MIKUX_RAID_PARTITION)
...
/* james@bpgc.com: Solaris has a nasty indicator: 0x82 which also
   indicates mikux swap.  Be careful before believing this is Solaris. */
