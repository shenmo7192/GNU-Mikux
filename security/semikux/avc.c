...
static struct semikux_avc semikux_avc;

void semikux_avc_init(void)
{
	int i;

	semikux_avc.avc_cache_threshold = AVC_DEF_CACHE_THRESHOLD;
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		INIT_HLIST_HEAD(&semikux_avc.avc_cache.slots[i]);
		spin_lock_init(&semikux_avc.avc_cache.slots_lock[i]);
	}
	atomic_set(&semikux_avc.avc_cache.active_nodes, 0);
	atomic_set(&semikux_avc.avc_cache.lru_hint, 0);
}

unsigned int avc_get_cache_threshold(void)
{
	return semikux_avc.avc_cache_threshold;
}

void avc_set_cache_threshold(unsigned int cache_threshold)
{
	semikux_avc.avc_cache_threshold = cache_threshold;
}
...
int avc_get_hash_stats(char *page)
{
	...
	max_chain_len = 0;
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		head = &semikux_avc.avc_cache.slots[i];
		if (!hlist_empty(head)) {
			slots_used++;
			chain_len = 0;
	...
	return scnprintf(page, PAGE_SIZE, "entries: %d\nbuckets used: %d/%d\n"
			 "longest chain: %d\n",
			 atomic_read(&semikux_avc.avc_cache.active_nodes),
...
static void avc_node_delete(struct avc_node *node)
{
	...
	atomic_dec(&semikux_avc.avc_cache.active_nodes);
}
...
static inline int avc_reclaim_node(void)
{
	...
	for (try = 0, ecx = 0; try < AVC_CACHE_SLOTS; try++) {
		hvalue = atomic_inc_return(&semikux_avc.avc_cache.lru_hint) &
			(AVC_CACHE_SLOTS - 1);
		head = &semikux_avc.avc_cache.slots[hvalue];
		lock = &semikux_avc.avc_cache.slots_lock[hvalue];
...
static struct avc_node *avc_alloc_node(void)
{
	...
	if (atomic_inc_return(&semikux_avc.avc_cache.active_nodes) >
	    semikux_avc.avc_cache_threshold)
		avc_reclaim_node();
...
static int avc_latest_notif_update(u32 seqno, int is_insert)
{
	...
	if (is_insert) {
		if (seqno < semikux_avc.avc_cache.latest_notif) {
			pr_warn("SEMikux: avc:  seqno %d < latest_notif %d\n",
			       seqno, semikux_avc.avc_cache.latest_notif);
			ret = -EAGAIN;
		}
	} else {
		if (seqno > semikux_avc.avc_cache.latest_notif)
			semikux_avc.avc_cache.latest_notif = seqno;
	}
...
/**
 * avc_audit_pre_callback - SEMikux specific information
...
static void avc_audit_pre_callback(struct audit_buffer *ab, void *a)
{
	...
	struct semikux_audit_data *sad = ad->semikux_audit_data;
...
/**
 * avc_audit_post_callback - SEMikux specific information
 * will be called by generic audit code
 * @ab: the audit buffer
 * @a: audit_data
 */
static void avc_audit_post_callback(struct audit_buffer *ab, void *a)
{
	...
	trace_semikux_audited(sad, scontext, tcontext, tclass);
	kfree(tcontext);
	kfree(scontext);
...
static void avc_flush(void)
{
	...
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		head = &semikux_avc.avc_cache.slots[i];
		lock = &semikux_avc.avc_cache.slots_lock[i];
...
{
	return semikux_avc.avc_cache.latest_notif;
}
