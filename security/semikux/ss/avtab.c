...
int avtab_alloc(struct avtab *h, u32 nrules)
{
	...
	pr_debug("SEMikux: %d avtab hash slots, %d rules.\n", nslot, nrules);
	return 0;
}
...
void avtab_hash_eval(struct avtab *h, const char *tag)
{
	...
	pr_debug("SEMikux: %s:  %d entries and %d/%d buckets used, "
		    ...
		    void *p)
{
	...
	if (vers < POLICYDB_VERSION_AVTAB) {
		...
		if (rc) {
			pr_err("SEMikux: avtab: truncated entry\n");
			return rc;
		}
		items2 = le32_to_cpu(buf32[0]);
		if (items2 > ARRAY_SIZE(buf32)) {
			pr_err("SEMikux: avtab: entry overflow\n");
			return -EINVAL;
		}
		...
		if (key.source_type != val) {
			pr_err("SEMikux: avtab: truncated source type\n");
			return -EINVAL;
		}
		val = le32_to_cpu(buf32[items++]);
		key.target_type = (u16)val;
		if (key.target_type != val) {
			pr_err("SEMikux: avtab: truncated target type\n");
			return -EINVAL;
		}
		val = le32_to_cpu(buf32[items++]);
		key.target_class = (u16)val;
		if (key.target_class != val) {
			pr_err("SEMikux: avtab: truncated target class\n");
			return -EINVAL;
		}
		...
		if (!(val & (AVTAB_AV | AVTAB_TYPE))) {
			pr_err("SEMikux: avtab: null entry\n");
			return -EINVAL;
		}
		if ((val & AVTAB_AV) && (val & AVTAB_TYPE)) {
			pr_err("SEMikux: avtab: entry has both access vectors and types\n");
			return -EINVAL;
		}
		if (val & AVTAB_XPERMS) {
			pr_err("SEMikux: avtab: entry has extended permissions\n");
			return -EINVAL;
		}
		...
		if (items != items2) {
			pr_err("SEMikux: avtab: entry only had %d items, expected %d\n",
			       items2, items);
			return -EINVAL;
		}
		return 0;
	...
	if (!policydb_type_isvalid(pol, key.source_type) ||
		...
		pr_err("SEMikux: avtab: invalid type or class\n");
		return -EINVAL;
	}

	set = hweight16(key.specified & (AVTAB_XPERMS | AVTAB_TYPE | AVTAB_AV));
	if (!set || set > 1) {
		pr_err("SEMikux:  avtab:  more than one specifier\n");
		return -EINVAL;
	}

	if ((vers < POLICYDB_VERSION_XPERMS_IOCTL) &&
	    (key.specified & AVTAB_XPERMS)) {
		pr_err("SEMikux:  avtab:  policy version %u does not "
	...
	if ((key.specified & AVTAB_TYPE) &&
		...
		pr_err("SEMikux: avtab: invalid type\n");
		return -EINVAL;
	}
...
int avtab_read(struct avtab *a, void *fp, struct policydb *pol)
{
	...
	if (rc < 0) {
		pr_err("SEMikux: avtab: truncated table\n");
		goto bad;
	}
	nel = le32_to_cpu(buf[0]);
	if (!nel) {
		pr_err("SEMikux: avtab: table is empty\n");
		rc = -EINVAL;
		goto bad;
	}
	...
	for (i = 0; i < nel; i++) {
		...
		if (rc) {
			if (rc == -ENOMEM)
				pr_err("SEMikux: avtab: out of memory\n");
			else if (rc == -EEXIST)
				pr_err("SEMikux: avtab: duplicate entry\n");
