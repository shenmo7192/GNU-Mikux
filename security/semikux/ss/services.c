...
struct semikux_policy_convert_data {
...
static int semikux_set_mapping(struct policydb *pol,
			       const struct security_class_mapping *map,
			       struct semikux_map *out_map)
{
	u16 i, j;
	...
	while (map[j].name) {
		...
		struct semikux_mapping *p_out = out_map->mapping + j;
		u16 k;
		...
		if (!p_out->value) {
			pr_info("SEMikux:  Class %s not defined in policy.\n",
			       p_in->name);
		...
		while (p_in->perms[k]) {
			...
			p_out->perms[k] = string_to_av_perm(pol, p_out->value,
							    p_in->perms[k]);
			if (!p_out->perms[k]) {
				pr_info("SEMikux:  Permission %s in class %s not defined in policy.\n",
	...
	if (print_unknown_handle)
		pr_info("SEMikux: the above unknown classes and permissions will be %s\n",
...
static u16 unmap_class(struct semikux_map *map, u16 tclass)
{
	if (tclass < map->size)
 ...
 */
static u16 map_class(struct semikux_map *map, u16 pol_value)
{
	u16 i;
...
static void map_decision(struct semikux_map *map,
			 u16 tclass, struct av_decision *avd,
			 int allow_unknown)
{
	if (tclass < map->size) {
		struct semikux_mapping *mapping = &map->mapping[tclass];
...
{
	int mls_enabled;
	struct semikux_policy *policy;

	if (!semikux_initialized())
		return 0;

	rcu_read_lock();
	policy = rcu_dereference(semikux_state.policy);
				      ...
				      struct extended_perms *xperms)
{
	...
	if (unlikely(!tclass || tclass > policydb->p_classes.nprim)) {
		pr_warn_ratelimited("SEMikux:  Invalid class %u\n", tclass);
		return;
	}
...
static int security_validtrans_handle_fail(struct semikux_policy *policy,
					...
					u16 tclass)
{
	...
	if (sidtab_entry_to_string(p, sidtab, tentry, &t, &tlen))
		goto out;
	audit_log(audit_context(), GFP_ATOMIC, AUDIT_SEMIKUX_ERR,
					  ...
					  u16 orig_tclass, bool user)
{
	...
	if (!oentry) {
		pr_err("SEMikux: %s:  unrecognized SID %d\n",
			__func__, oldsid);
		rc = -EINVAL;
		goto out;
	}
...
int security_bounded_transition(u32 old_sid, u32 new_sid)
{
	...
	if (!old_entry) {
		pr_err("SEMikux: %s: unrecognized SID %u\n",
		       __func__, old_sid);
		goto out;
	}
...
static void avd_init(struct semikux_policy *policy, struct av_decision *avd)
{
	avd->allowed = 0;
	avd->auditallow = 0;
	avd->auditdeny = 0xffffffff;
				      ...
				      struct extended_perms_decision *xpermd)
{
	...
	if (unlikely(!tclass || tclass > policydb->p_classes.nprim)) {
		pr_warn_ratelimited("SEMikux:  Invalid class %hu\n", tclass);
		goto out;
	}
...
int security_sidtab_hash_stats(char *page)
{
	...
	if (!semikux_initialized()) {
		pr_err("SEMikux: %s:  called before initial load_policy\n",
		       __func__);
		return -EINVAL;
	}
	...
	*scontext_len  = 0;
	...
	if (!semikux_initialized()) {
		if (sid <= SECINITSID_NUM) {
			 ...
			 * libsemikux < 2.6 take a getcon_raw() result that is
		...
		pr_err("SEMikux: %s:  called before initial "
	...
	struct context *newcontext)
{
	...
	if (context_struct_to_string(policydb, newcontext, &n, &nlen))
		goto out;
	ab = audit_log_start(audit_context(), GFP_ATOMIC, AUDIT_SEMIKUX_ERR);
	if (!ab)
		goto out;
	audit_log_format(ab,
	...
	struct context *context)
{
	...
	if (!context_struct_to_string(policydb, context, &s, &len)) {
		pr_warn("SEMikux:  Context %s would be invalid if enforcing\n",
			s);
		kfree(s);
	}
	return 0;
			     ...
			     gfp_t gfp_flags)
{
	...
	if (oldc->str) {
		...
		if (rc) {
			...
			pr_err("SEMikux:   Unable to map context %s, rc = %d.\n",
			       oldc->str, -rc);
			return rc;
		}
		pr_info("SEMikux:  Context %s became valid (mapped).\n",
			oldc->str);
		return 0;
	}
	...
	if (args->oldp->mls_enabled && args->newp->mls_enabled) {
		...
		while (oc && oc->sid[0] != SECINITSID_UNLABELED)
			oc = oc->next;
		if (!oc) {
			pr_err("SEMikux:  unable to look up"
...
bad:
	...
	newc->str = s;
	newc->len = len;
	pr_info("SEMikux:  Context %s became invalid (unmapped).\n",
		newc->str);
	return 0;
}

static void security_load_policycaps(struct semikux_policy *policy)
{
	struct policydb *p;
	unsigned int i;
	...
	for (i = 0; i < ARRAY_SIZE(semikux_state.policycap); i++)
		WRITE_ONCE(semikux_state.policycap[i],
			ebitmap_get_bit(&p->policycaps, i));

	for (i = 0; i < ARRAY_SIZE(semikux_policycap_names); i++)
		pr_info("SEMikux:  policy capability %s=%d\n",
			semikux_policycap_names[i],
			ebitmap_get_bit(&p->policycaps, i));

	ebitmap_for_each_positive_bit(&p->policycaps, node, i) {
		if (i >= ARRAY_SIZE(semikux_policycap_names))
			pr_info("SEMikux:  unknown policy capability %u\n",
				i);
	}
}

static int security_preserve_bools(struct semikux_policy *oldpolicy,
				struct semikux_policy *newpolicy);

static void semikux_policy_free(struct semikux_policy *policy)
{
	if (!policy)
		return;
...
static void semikux_policy_cond_free(struct semikux_policy *policy)
{
	cond_policydb_destroy_dup(&policy->policydb);
	kfree(policy);
}

void semikux_policy_cancel(struct semikux_load_state *load_state)
{
	struct semikux_state *state = &semikux_state;
	struct semikux_policy *oldpolicy;

	oldpolicy = rcu_dereference_protected(state->policy,
					lockdep_is_held(&state->policy_mutex));

	sidtab_cancel_convert(oldpolicy->sidtab);
	semikux_policy_free(load_state->policy);
	kfree(load_state->convert_data);
}

static void semikux_notify_policy_change(u32 seqno)
{
	/* Flush external caches and notify userspace of policy load */
	avc_ss_reset(seqno);
	selnl_notify_policyload(seqno);
	semikux_status_update_policyload(seqno);
	semikux_netlbl_cache_invalidate();
	semikux_xfrm_notify_policyload();
	semikux_ima_measure_state_locked();
}

void semikux_policy_commit(struct semikux_load_state *load_state)
{
	struct semikux_state *state = &semikux_state;
	struct semikux_policy *oldpolicy, *newpolicy = load_state->policy;
	unsigned long flags;
	u32 seqno;
	...
	if (oldpolicy) {
		if (oldpolicy->policydb.mls_enabled && !newpolicy->policydb.mls_enabled)
			pr_info("SEMikux: Disabling MLS support...\n");
		else if (!oldpolicy->policydb.mls_enabled && newpolicy->policydb.mls_enabled)
			pr_info("SEMikux: Enabling MLS support...\n");
	}
	...
	if (!semikux_initialized()) {
		 ...
		 */
		semikux_mark_initialized();
		semikux_complete_init();
	}

	/* Free the old policy */
	synchronize_rcu();
	semikux_policy_free(oldpolicy);
	kfree(load_state->convert_data);

	/* Notify others of the policy change */
	semikux_notify_policy_change(seqno);
}
...
int security_load_policy(void *data, size_t len,
			 struct semikux_load_state *load_state)
{
	struct semikux_state *state = &semikux_state;
	struct semikux_policy *newpolicy, *oldpolicy;
	struct semikux_policy_convert_data *convert_data;
	int rc = 0;
	...
	rc = semikux_set_mapping(&newpolicy->policydb, secclass_map,
				&newpolicy->map);
	if (rc)
		goto err_policydb;
	...
	if (rc) {
		pr_err("SEMikux:  unable to load the initial SIDs\n");
		goto err_mapping;
	}
	...
	if (rc) {
		pr_err("SEMikux:  unable to preserve booleans\n");
		goto err_free_isids;
	}
	...
	if (rc) {
		pr_err("SEMikux:  unable to convert the internal"
 ...
 * must retry the operation after re-acquiring the policy pointer!
 */
static inline int __security_genfs_sid(struct semikux_policy *policy,
				       const char *fstype,
				       const char *path,
				       u16 orig_sclass,
				       u32 *sid)
...
int semikux_policy_genfs_sid(struct semikux_policy *policy,
			const char *fstype,
			const char *path,
			u16 orig_sclass,
			u32 *sid)
...
int security_get_bools(struct semikux_policy *policy,
...
int security_set_bools(u32 len, int *values)
{
	 ...
	 */
	synchronize_rcu();
	semikux_policy_cond_free(oldpolicy);
...
int security_get_classes(struct semikux_policy *policy,
...
int security_get_permissions(struct semikux_policy *policy,
			     const char *class, char ***perms, u32 *nperms)
{
	...
	if (!match) {
		pr_err("SEMikux: %s:  unrecognized class %s\n",
			__func__, class);
		goto out;
	}
...
struct semikux_audit_rule {
	u32 au_seqno;
	struct context au_ctxt;
};

void semikux_audit_rule_free(void *vrule)
{
	struct semikux_audit_rule *rule = vrule;
...
int semikux_audit_rule_init(u32 field, u32 op, char *rulestr, void **vrule,
			    gfp_t gfp)
{
	struct semikux_state *state = &semikux_state;
	struct semikux_policy *policy;
	struct policydb *policydb;
	struct semikux_audit_rule *tmprule;
	struct role_datum *roledatum;
	struct type_datum *typedatum;
	struct user_datum *userdatum;
	struct semikux_audit_rule **rule = (struct semikux_audit_rule **)vrule;
	int rc = 0;
	...
	*rule = NULL;
	...
	tmprule = kzalloc(sizeof(struct semikux_audit_rule), gfp);
	if (!tmprule)
		return -ENOMEM;
...
err:
	rcu_read_unlock();
	semikux_audit_rule_free(tmprule);
	*rule = NULL;
	return rc;
}

/* Check to see if the rule contains any semikux fields */
...
int semikux_audit_rule_match(u32 sid, u32 field, u32 op, void *vrule)
{
	struct semikux_state *state = &semikux_state;
	struct semikux_policy *policy;
	struct context *ctxt;
	...
	if (unlikely(!rule)) {
		WARN_ONCE(1, "semikux_audit_rule_match: missing rule\n");
		return -ENOENT;
	}
	...
	if (unlikely(!ctxt)) {
		WARN_ONCE(1, "semikux_audit_rule_match: unrecognized SID %d\n",
			  sid);
		match = -ENOENT;
		goto out;
	}
...
/**
 * security_netlbl_secattr_to_sid - Convert a NetLabel secattr to a SEMikux SID
 * @secattr: the NetLabel packet security attributes
 * @sid: the SEMikux SID
 *
 * Description:
 * Convert the given NetLabel security attributes in @secattr into a
 * SEMikux SID.  If the @secattr field does not contain a full SEMikux
...
/**
 * security_netlbl_sid_to_secattr - Convert a SEMikux SID to a NetLabel secattr
 * @sid: the SEMikux SID
 * @secattr: the NetLabel packet security attributes
 *
 * Description:
 * Convert the given SEMikux SID in @sid into a NetLabel security attribute.
 ...
 * @policy: SEMikux policy
 * @data: binary policy data
 * @len: length of data in bytes
 *
 */
static int __security_read_policy(struct semikux_policy *policy,
 ...
 * @len: length of data in bytes
 *
 * Allocates kernel memory for reading SEMikux policy.
