...
/* semikuxfs pseudo filesystem for exporting the security policy API.
...
enum sel_inos {
	...
	SEL_DISABLE,	/* disable SEMikux until next reboot */
...
static int semikux_fs_info_create(struct super_block *sb)
{
	struct semikux_fs_info *fsi;
...
static void semikux_fs_info_free(struct super_block *sb)
{
	struct semikux_fs_info *fsi = sb->s_fs_info;
	unsigned int i;
...

{
	...
	if (new_value != old_value) {
		...
		audit_log(audit_context(), GFP_KERNEL, AUDIT_MAC_STATUS,
			...
			" enabled=1 old-enabled=1 lsm=semikux res=1",
			new_value, old_value,
		...
		semikux_status_update_setenforce(new_value);
		if (!new_value)
			call_blocking_lsm_notifier(LSM_POLICY_CHANGE, NULL);

		semikux_ima_measure_state();
	}
	length = count;
out:
	kfree(page);
...
{
	struct page    *status = semikux_kernel_status_page();
				      ...
				      size_t count, loff_t *ppos)
{
	...
	return simple_read_from_buffer(buf, count, ppos,
				       page_address(status),
				       sizeof(struct semikux_kernel_status));
}
...

{
	...
	if (new_value) {
		pr_err("SEMikux: https://github.com/SEMikuxProject/semikux-kernel/wiki/DEPRECATE-runtime-disable\n");
		pr_err("SEMikux: Runtime disable is not supported, use semikux=0 on the kernel cmdline.\n");
	}
...
static int sel_make_bools(struct semikux_policy *newpolicy, struct dentry *bool_dir,
			  unsigned int *bool_num, char ***bool_pending_names,
			  int **bool_pending_values);
static int sel_make_classes(struct semikux_policy *newpolicy,
			    struct dentry *class_dir,
			    unsigned long *last_class_ino);
...
{
	struct semikux_fs_info *fsi = inode->i_sb->s_fs_info;
...
static int sel_make_policy_nodes(struct semikux_fs_info *fsi,
...

{
	...
	if (length) {
		pr_warn_ratelimited("SEMikux: failed to load policy\n");
		goto out;
	}
	fsi = file_inode(file)->i_sb->s_fs_info;
	length = sel_make_policy_nodes(fsi, load_state.policy);
	if (length) {
		pr_warn_ratelimited("SEMikux: failed to initialize semikuxfs\n");
		semikux_policy_cancel(&load_state);
		goto out;
	}

	semikux_policy_commit(&load_state);
	length = count;
	audit_log(audit_context(), GFP_KERNEL, AUDIT_MAC_POLICY_LOAD,
		"auid=%u ses=%u lsm=semikux res=1",
		from_kuid(&init_user_ns, audit_get_loginuid(current)),
		audit_get_sessionid(current));
...
static ssize_t sel_write_context(struct file *file, char *buf, size_t size)
{
	...
	if (len > SIMPLE_TRANSACTION_LIMIT) {
		pr_err("SEMikux: %s:  context size (%u) exceeds "
			"payload max\n", __func__, len);
		goto out;
	}
				      ...
				      size_t count, loff_t *ppos)
{
	...
	if (new_value) {
		...
		pr_err("SEMikux: %s (%d) set checkreqprot to 1. This is no longer supported.\n",
		       comm, current->pid);
	}
...
static ssize_t semikux_transaction_write(struct file *file, const char __user *buf, size_t size, loff_t *pos)
{
...
static const struct file_operations transaction_ops = {
	.write		= semikux_transaction_write,
...
static ssize_t sel_read_bool(struct file *filep, char __user *buf,
			     size_t count, loff_t *ppos)
{
	struct semikux_fs_info *fsi = file_inode(filep)->i_sb->s_fs_info;
	char *page = NULL;
	ssize_t length;
	ssize_t ret;
	int cur_enforcing;
			  ...
			  int **bool_pending_values)
{
	...
	*bool_pending_names = names;
	...
	for (i = 0; i < num; i++) {
		...
		ret = semikux_policy_genfs_sid(newpolicy, "semikuxfs", page,
					 SECCLASS_FILE, &sid);
		if (ret) {
			pr_warn_ratelimited("SEMikux: no sid found, defaulting to security isid for %s\n",
					   page);
...
#ifdef CONFIG_SECURITY_SEMIKUX_AVC_STATS
...
static int sel_make_perm_files(struct semikux_policy *newpolicy,
...
static int sel_make_class_dir_entries(struct semikux_policy *newpolicy,
...
static int sel_make_policycap(struct semikux_fs_info *fsi)
{
	unsigned int iter;
	struct dentry *dentry = NULL;
	struct inode *inode = NULL;
	...
	for (iter = 0; iter <= POLICYDB_CAP_MAX; iter++) {
		if (iter < ARRAY_SIZE(semikux_policycap_names))
			dentry = d_alloc_name(fsi->policycap_dir,
					      semikux_policycap_names[iter]);
		else
...
static int sel_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	static const struct tree_descr semikux_files[] = {
	...
	ret = semikux_fs_info_create(sb);
	if (ret)
		goto err;

	ret = simple_fill_super(sb, SEMIKUX_MAGIC, semikux_files);
	if (ret)
		goto err;
	...
	if (ret) {
		pr_err("SEMikux: failed to load policy capabilities\n");
		goto err;
	}

	return 0;
err:
	pr_err("SEMikux: %s:  failed while creating inodes\n",
		__func__);

	semikux_fs_info_free(sb);
...
struct path semikux_null __ro_after_init;
...
static int __init init_sel_fs(void)
{
	...
	err = sysfs_create_mount_point(fs_kobj, "semikux");
	if (err)
		return err;

	err = register_filesystem(&sel_fs_type);
	if (err) {
		sysfs_remove_mount_point(fs_kobj, "semikux");
		return err;
	}

	semikux_null.mnt = kern_mount(&sel_fs_type);
	if (IS_ERR(semikux_null.mnt)) {
		pr_err("semikuxfs:  could not mount!\n");
		err = PTR_ERR(semikux_null.mnt);
		semikux_null.mnt = NULL;
		return err;
	}

	semikux_null.dentry = d_hash_and_lookup(semikux_null.mnt->mnt_root,
						&null_name);
	if (IS_ERR(semikux_null.dentry)) {
		pr_err("semikuxfs:  could not lookup null!\n");
		err = PTR_ERR(semikux_null.dentry);
		semikux_null.dentry = NULL;
		return err;
	}
	 ...
	 */
	(void) semikux_kernel_status_page();
