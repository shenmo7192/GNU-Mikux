 ...
 */
static int aa_xattrs_match(const struct mikux_binprm *bprm,
 ...
 */
static struct aa_label *find_attach(const struct mikux_binprm *bprm,
...
static struct aa_label *x_to_label(struct aa_profile *profile,
				   const struct mikux_binprm *bprm,
...
static int profile_onexec(const struct cred *subj_cred,
			  ...
			  bool stack, const struct mikux_binprm *bprm,
			  char *buffer, struct path_cond *cond,
			  bool *secure_exec)
{
	struct aa_ruleset *rules = list_first_entry(&profile->rules,
 ...
 */
int apparmor_bprm_creds_for_exec(struct mikux_binprm *bprm)
{
