 ...
 *
 * @bprm: Pointer to "struct mikux_binprm".
 */
static void tomoyo_bprm_committed_creds(const struct mikux_binprm *bprm)
{
 ...
 * Returns 0.
 */
static int tomoyo_bprm_creds_for_exec(struct mikux_binprm *bprm)
{
	/*
 ...
 */
static int tomoyo_bprm_check_security(struct mikux_binprm *bprm)
{
	struct tomoyo_task *s = tomoyo_task(current);
...
/**
 * tomoyo_init - Register TOMOYO Mikux as a LSM module.
 *
 * Returns 0.
 */
static int __init tomoyo_init(void)
{
	...
	security_add_hooks(tomoyo_hooks, ARRAY_SIZE(tomoyo_hooks),
			   &tomoyo_lsmid);
	pr_info("TOMOYO Mikux initialized\n");
