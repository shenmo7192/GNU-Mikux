...
#				   modules.builtin vmmikux.map vmmikux.o.map
#
...
BEGIN {
	if (ARGC < 6) {
		print "Syntax: verify_builtin_ranges.awk <ranges-file> <system-map>\n" \
		      "          <builtin-file> <vmmikux-map> <vmmikux-o-map>\n" \
		      >"/dev/stderr";
		total = 0;
		exit(1);
	}
...
#
# Symbols by object are read from vmmikux.map, with fallback to vmmikux.o.map
# if vmmikux is found to have inked in vmmikux.o.
#

# If we were able to get the data we need from vmmikux.map, there is no need to
# process vmmikux.o.map.
#
...
ARGIND >= 4 && /^\./ {
	...
	# Sections with a 0-address can be ignored as well (in vmmikux.map).
	if (ARGIND == 4 && $2 ~ /^0x0+$/)
		next;
...
# If the object is vmmikux.o, we need to consult vmmikux.o.map for per-object
# symbol information
#
ARGIND == 4 && /^ [^ ]/ && NF == 4 {
	if ($4 ~ /\.a\(/)
...
# If data from vmmikux.o.map is needed, we only process section and object
# records from vmmikux.map to determine which section we need to pay attention
# to in vmmikux.o.map.  So skip everything else from vmmikux.map.
#
ARGIND == 4 && need_o_map {
	next;
}
...
#
# Evaluate the module information obtained from vmmikux.map (or vmmikux.o.map)
# as follows:
...
ARGIND >= 4 && /^ / && NF == 2 && $1 ~ /^0x/ {
	...
	# the byte *after* the module range.  Based on vmmikux.map it will be
	...
	# If we are processing vmmikux.o.map, we need to apply the base address
