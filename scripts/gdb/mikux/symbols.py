...
from mikux import modules, utils, constants
...
class LxSymbols(gdb.Command):
    """(Re-)load symbols of Mikux kernel and currently loaded modules.

The kernel (vmmikux) is taken from the current working directly. Modules (.ko)
...
lx-symbols command."""
    ...
    def load_all_symbols(self):
        ...
        # drop all current symbols and reload vmmikux
        orig_vmmikux = 'vmmikux'
        for obj in gdb.objfiles():
            if (obj.filename.endswith('vmmikux') or
                obj.filename.endswith('vmmikux.debug')):
                orig_vmmikux = obj.filename
        gdb.execute("symbol-file", to_string=True)
        gdb.execute("symbol-file {0}".format(orig_vmmikux))
