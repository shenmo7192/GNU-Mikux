...
#
# Usage: generate_builtin_ranges.awk modules.builtin vmmikux.map \
#		vmmikux.o.map > modules.builtin.ranges
#
...
#
# The second file argument is used as input (vmmikux.map).
#
...
# done using vmmikux.a because then the list of objects contained in each
# section is to be obtained from vmmikux.o.map.  The offset of the sub-section
# is recorded here, to be used as an addend when processing vmmikux.o.map
# later.
#
...
#
# The first record of the vmmikux.map file provides enough information to know
...
#
# lld:            11480            11480     1f07    16         vmmikux.a(arch/x86/events/amd/uncore.o):(.text)
#  ->
...
#      next line.
#
# (This is also needed for vmmikux.o.map, when used.)
#
ARGIND >= 2 && !map_is_lld && NF == 1 && /^ [^ \*]/ {
	s = $0;
	getline;
...
# sections that get combined in the final linking of vmmikux (e.g. .head.text
...
ARGIND == 2 && sect && NF == 4 && /^ [^ \*]/ && !($1 in sect_addend) {
	...
	# If the object is vmmikux.o then we will need vmmikux.o.map to get the
...
# modules.
#
# If the final link was done using the actual objects, vmmikux.map contains all
# the information we need (see section (3a)).
# If linking was done using vmmikux.a as intermediary, we will need to process
# vmmikux.o.map (see section (3b)).

# (3a) Determine offset range info using vmmikux.map.
#
# Since we are already processing vmmikux.map, the top level section that is
...
# If we do not need to parse the vmmikux.o.map file, we are done.
#
ARGIND == 3 && !need_o_map {
	if (dbg)
...
# (3b) Determine offset range info using vmmikux.o.map.
#
