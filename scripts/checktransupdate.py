...
def valid_locales(locale):
    ...
    mikux_path = os.path.join(script_path, "..")
    if not os.path.isdir(f"{mikux_path}/Documentation/translations/{locale}"):
...
def main():
    ...
    # Get files related to mikux path
    files = args.files
    if len(files) == 0:
        offical_files = list_files_with_excluding_folders(
            os.path.join(mikux_path, "Documentation"), ["translations", "output"], "rst"
        )
        ...
        for file in offical_files:
            ...
            if os.path.isfile(new_file):
                ...
                if args.print_missing_translations:
                    logging.info(os.path.relpath(os.path.abspath(file), mikux_path))
                    logging.info("No translation in the locale of %s\n", args.locale)

    files = list(map(lambda x: os.path.relpath(os.path.abspath(x), mikux_path), files))

    # cd to mikux root directory
    os.chdir(mikux_path)
