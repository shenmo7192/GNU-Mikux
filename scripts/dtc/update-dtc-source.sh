...
# Simple script to update the version of DTC carried by the Mikux kernel
#
# This script assumes that the dtc and the mikux git trees are in the
# same directory. After building dtc in the dtc directory, it copies the
# source files and generated source file(s) into the scripts/dtc directory
# in the kernel and creates a git commit updating them to the new
# version.
#
# Usage: from the top level Mikux source tree, run:
...
DTC_MIKUX_PATH=`pwd`/scripts/dtc
...
# Copy the files into the Mikux tree
cd $DTC_MIKUX_PATH
