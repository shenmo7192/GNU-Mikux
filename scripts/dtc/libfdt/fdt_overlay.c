 ...
 * @name: Name of the property to modify (phandle or mikux,phandle)
					...
					uint32_t delta)
{
	...
	ret = overlay_phandle_add_offset(fdto, node, "mikux,phandle", delta);
	if (ret && ret != -FDT_ERR_NOTFOUND)
		return ret;
						    ...
						    uint32_t fdt_phandle)
{
	...
	php = fdt_getprop(fdto, node, "mikux,phandle", &len);
	if (php && len == sizeof(*php)) {
		ret = fdt_setprop_inplace_u32(fdto, node, "mikux,phandle", fdt_phandle);
		if (ret)
			return ret;
	}
