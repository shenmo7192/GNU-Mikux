				    ...
				    struct node *node)
{
	...
	mikux_phandle = check_phandle_prop(c, dti, node, "mikux,phandle");

	if (!phandle && !mikux_phandle)
		/* No valid phandles; nothing further to check */
		return;

	if (mikux_phandle && phandle && (phandle != mikux_phandle))
		FAIL(c, dti, node, "mismatching 'phandle' and 'mikux,phandle'"
		     " properties");

	if (mikux_phandle && !phandle)
		phandle = mikux_phandle;
					  ...
					  struct node *node)
{
	...
	if (!prop) {
		prop = get_property(node, "mikux,stdout-path");
		if (!prop)
			return;
