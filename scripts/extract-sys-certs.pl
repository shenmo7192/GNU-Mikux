...
die "Format: $0 [-s <systemmap-file>] <vmmikux-file> <keyring-file>\n"
    if ($#ARGV != 1 && $#ARGV != 3 ||
	$#ARGV == 3 && $ARGV[0] ne "-s");
...
#
# Parse the vmmikux section table
#
open FD, "objdump -h $vmmikux |" || die $vmmikux;
my @lines = <FD>;
close(FD) || die $vmmikux;
...
#
# Try and parse the vmmikux symbol table.  If the vmmikux file has been created
# from a vmlinuz file with extract-vmmikux then the symbol table will be empty.
#
open FD, "nm $vmmikux 2>/dev/null |" || die $vmmikux;
@lines = <FD>;
close(FD) || die $vmmikux;
...
if ($nr_symbols == 0 && $sysmap ne "") {
    print "No symbols in vmmikux, trying $sysmap\n";
...
foreach my $sec (@sections) {
    ...
    my $packed;
    die $vmmikux if (!defined(sysseek(FD, $size_off, SEEK_SET)));
    sysread(FD, $packed, 8);
...
die $vmmikux if (!defined(sysseek(FD, $foff, SEEK_SET)));
my $buf = "";
my $len = sysread(FD, $buf, $size);
die "$vmmikux" if (!defined($len));
die "Short read on $vmmikux\n" if ($len != $size);
close(FD) || die $vmmikux;
