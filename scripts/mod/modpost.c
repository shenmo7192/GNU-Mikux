 ...
 *
 * Usage: modpost vmmikux module1.o module2.o ...
 */
...
#include <xalloc.h>
#include "modpost.h"
#include "../../include/mikux/license.h"
 ...
 * vmmikux is missing. ('make modules' without building vmmikux.)
 */
#define MAX_UNRESOLVED_REPORTS	10
static unsigned int nr_unresolved;

/* In kernel, this size is defined in mikux/module.h;
...
static struct module *new_module(const char *name, size_t namelen)
{
	...
	mod->name[namelen] = '\0';
	mod->is_vmmikux = (strcmp(mod->name, "vmmikux") == 0);
				       ...
				       bool gpl_only, const char *namespace)
{
	...
	if (s && (!external_module || s->module->is_vmmikux || s->module == mod)) {
		error("%s: '%s' exported twice. Previous export was in %s%s\n",
		      mod->name, name, s->module->name,
		      s->module->is_vmmikux ? "" : ".ko");
	}
			  ...
			  Elf_Shdr *sechdr)
{
	...
	if (sechdr->sh_type == SHT_PROGBITS &&
		...
		warn("%s (%s): unexpected non-allocatable section.\n"
		     ...
		     "Note that for example <mikux/init.h> contains\n"
...
{
	buf_printf(b, "#include <mikux/module.h>\n");
	buf_printf(b, "#include <mikux/export-internal.h>\n");
	buf_printf(b, "#include <mikux/compiler.h>\n");
	buf_printf(b, "\n");
	...
	list_for_each_entry(sym, &mod->exported_symbols, list) {
		...
		if (!sym->crc_valid)
			warn("EXPORT symbol \"%s\" [%s%s] version generation failed, symbol will not be versioned.\n"
			     ...
			     sym->name, mod->name, mod->is_vmmikux ? "" : ".ko",
			     sym->name);
...
static void add_depends(struct buffer *b, struct module *mod)
{
	...
	list_for_each_entry(s, &mod->unresolved_symbols, list) {
		if (s->module)
			s->module->seen = s->module->is_vmmikux;
	}
...
static void write_vmmikux_export_c_file(struct module *mod)
{
	struct buffer buf = { };

	buf_printf(&buf,
		   "#include <mikux/export-internal.h>\n");

	add_exported_symbols(&buf, mod);
	write_if_changed(&buf, ".vmmikux.export.c");
	free(buf.p);
}
...
int main(int argc, char **argv)
{
	...
	list_for_each_entry(mod, &modules, list) {
		if (mod->from_dump || mod->is_vmmikux)
			continue;
	...
	list_for_each_entry(mod, &modules, list) {
		...
		if (mod->is_vmmikux)
			write_vmmikux_export_c_file(mod);
		else
