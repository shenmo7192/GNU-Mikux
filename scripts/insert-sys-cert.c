 ...
 *
 * Author: Mehmet Kayaalp <mkayaalp@mikux.vnet.ibm.com>
 *
 * This software may be used and distributed according to the terms
 * of the GNU General Public License, incorporated herein by reference.
 *
 * Usage: insert-sys-cert [-s <System.map> -b <vmmikux> -c <certfile>
 */
...
{
	printf("Usage %s [-s <System.map>] -b <vmmikux> -c <certfile>\n", e);
}

int main(int argc, char **argv)
{
	char *system_map_file = NULL;
	char *vmmikux_file = NULL;
	char *cert_file = NULL;
	int vmmikux_size;
	int cert_size;
	Elf_Ehdr *hdr;
	...
	hdr = map_file(vmmikux_file, &vmmikux_size);
	if (!hdr)
		exit(EXIT_FAILURE);

	if (vmmikux_size < sizeof(*hdr)) {
