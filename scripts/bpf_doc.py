...
class HeaderParser(object):
    ...
    @filename: name of file to parse, usually include/uapi/mikux/bpf.h in the
               kernel tree
    """
...
.. See git log include/uapi/mikux/bpf.h in kernel tree for details.
.. 
.. SPDX-License-Identifier: Mikux-man-pages-copyleft
.. 
.. Please do not edit this file. It was generated from the documentation
.. located in file include/uapi/mikux/bpf.h of the Mikux kernel sources
...
'''
    ...
    def get_kernel_version(self):
        try:
            version = subprocess.run(['git', 'describe'], cwd=mikuxRoot,
                                     capture_output=True, check=True)
            version = version.stdout.decode().rstrip()
        except:
            try:
                version = subprocess.run(['make', '-s', '--no-print-directory', 'kernelversion'],
                                         cwd=mikuxRoot, capture_output=True, check=True)
                version = version.stdout.decode().rstrip()
            except:
                return 'Mikux'
        return 'Mikux {version}'.format(version=version)

    def get_last_doc_update(self, delimiter):
        try:
            cmd = ['git', 'log', '-1', '--pretty=format:%cs', '--no-patch',
                   '-L',
                   '/{}/,/\\*\\//:include/uapi/mikux/bpf.h'.format(delimiter)]
            date = subprocess.run(cmd, cwd=mikuxRoot,
                                  capture_output=True, check=True)
...
available within the Mikux kernel sources, at the following locations:
...
* *include/uapi/mikux/bpf.h* is the main BPF header. It contains the full list
...
# ../include/uapi/mikux/bpf.h, use it as a default name for the file to parse,
# otherwise the --filename argument will be required from the command line.
script = os.path.abspath(sys.argv[0])
mikuxRoot = os.path.dirname(os.path.dirname(script))
bpfh = os.path.join(mikuxRoot, 'include/uapi/mikux/bpf.h')
...
if (os.path.isfile(bpfh)):
    argParser.add_argument('--filename', help='path to include/uapi/mikux/bpf.h',
                           default=bpfh)
else:
    argParser.add_argument('--filename', help='path to include/uapi/mikux/bpf.h')
