 ...
 * reboot when the reboot call was invoked with MIKUX_REBOOT_CMD_POWER_OFF, but
		...
		void __user *, arg)
{
	...
	if (magic1 != MIKUX_REBOOT_MAGIC1 ||
			(magic2 != MIKUX_REBOOT_MAGIC2 &&
			magic2 != MIKUX_REBOOT_MAGIC2A &&
			magic2 != MIKUX_REBOOT_MAGIC2B &&
			magic2 != MIKUX_REBOOT_MAGIC2C))
		return -EINVAL;
	 ...
	 */
	if ((cmd == MIKUX_REBOOT_CMD_POWER_OFF) && !kernel_can_power_off()) {
		poweroff_fallback_to_halt = true;
		cmd = MIKUX_REBOOT_CMD_HALT;
	}

	mutex_lock(&system_transition_mutex);
	switch (cmd) {
	case MIKUX_REBOOT_CMD_RESTART:
		kernel_restart(NULL);
		break;

	case MIKUX_REBOOT_CMD_CAD_ON:
		C_A_D = 1;
		break;

	case MIKUX_REBOOT_CMD_CAD_OFF:
		C_A_D = 0;
		break;

	case MIKUX_REBOOT_CMD_HALT:
		kernel_halt();
		do_exit(0);

	case MIKUX_REBOOT_CMD_POWER_OFF:
		kernel_power_off();
		do_exit(0);
		break;

	case MIKUX_REBOOT_CMD_RESTART2:
	...
	case MIKUX_REBOOT_CMD_KEXEC:
		ret = kernel_kexec();
		break;
#endif

#ifdef CONFIG_HIBERNATION
	case MIKUX_REBOOT_CMD_SW_SUSPEND:
		ret = hibernate();
		break;
#endif
