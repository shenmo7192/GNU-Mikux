...
/* sets obj->mod if object is not vmmikux and module is found */
				  ...
				  unsigned long sympos, unsigned long *addr)
{
	...
	if (args.addr == 0)
		...
		pr_err("symbol position %lu for symbol '%s' in object '%s' not found\n",
		       sympos, name, objname ? objname : "vmmikux");
	} else {
		*addr = args.addr;
		return 0;
	}
			       ...
			       const char *sec_objname)
{
	...
	bool sym_vmmikux;
	bool sec_vmmikux = !strcmp(sec_objname, "vmmikux");
	...
	for (i = 0; i < relasec->sh_size / sizeof(Elf_Rela); i++) {
		...
		sym_vmmikux = !strcmp(sym_objname, "vmmikux");

		/*
		 * Prevent module-specific KLP rela sections from referencing
		 * vmmikux symbols.  This helps prevent ordering issues with
		 * module special section initializations.  Presumably such
		 * symbols are exported and normal relas can be used instead.
		 */
		if (!sec_vmmikux && sym_vmmikux) {
			pr_err("invalid access to vmmikux symbol '%s' from module-specific livepatch relocation section\n",
			       sym_name);
			return -EINVAL;
		}

		/* klp_find_object_symbol() treats a NULL objname as vmmikux */
		ret = klp_find_object_symbol(sym_vmmikux ? NULL : sym_objname,
 ...
 * reference symbols which live in vmmikux; and those which reference symbols
 ...
 *    write vmmikux-specific klp relocations (.klp.rela.vmmikux.* sections).
 *    These relocations are written to the klp module text to allow the patched
 *    code/data to reference unexported vmmikux symbols.  They're written as
 *    early as possible to ensure that other module init code (.e.g.,
 *    jump_label_apply_nops) can access any unexported vmmikux symbols which
				    ...
				    const char *objname, bool apply)
{
	...
	if (strcmp(objname ? objname : "vmmikux", sec_objname))
		return 0;
				  ...
				  struct klp_object *obj)
{
	...
	if (klp_is_module(obj)) {
		 ...
		 * (.klp.rela.{module}.*).  vmmikux-specific relocations were
...
static int klp_init_object(struct klp_patch *patch, struct klp_object *obj)
{
	...
	name = klp_is_module(obj) ? obj->name : "vmmikux";
	...
	klp_for_each_object(patch, obj) {
		...
		if (ret) {
			pr_warn("pre-patch callback failed for object '%s'\n",
				klp_is_module(obj) ? obj->name : "vmmikux");
			goto err;
		}
...
int klp_module_coming(struct module *mod)
{
	...
	if (!strcmp(mod->name, "vmmikux")) {
		pr_err("vmmikux.ko: invalid module name\n");
		return -EINVAL;
	}
