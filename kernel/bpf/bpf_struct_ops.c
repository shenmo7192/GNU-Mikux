				...
				const char *value_name)
{
	...
	common_value_type = btf_type_by_id(btf_vmmikux,
					   ...
					   void *value, u64 flags)
{
	...
	module_type = btf_type_by_id(btf_vmmikux, st_ops_ids[IDX_MODULE_ID]);
	for_each_member(i, t, member) {
...
static struct bpf_map *bpf_struct_ops_map_alloc(union bpf_attr *attr)
{
	...
	st_ops_desc = bpf_struct_ops_find_value(btf, attr->btf_vmmikux_value_type_id);
	if (!st_ops_desc) {
		ret = -ENOTSUPP;
		goto errout;
	}
...
void bpf_map_struct_ops_info_fill(struct bpf_map_info *info, struct bpf_map *map)
{
	...
	info->btf_vmmikux_id = btf_obj_id(st_map->btf);
}
