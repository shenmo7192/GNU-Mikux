...
#include <uapi/mikux/btf.h>
#include <uapi/mikux/bpf.h>
#include <uapi/mikux/bpf_perf_event.h>
#include <uapi/mikux/types.h>
...
#include <mikux/idr.h>
#include <mikux/sort.h>
#include <mikux/bpf_verifier.h>
#include <mikux/btf.h>
#include <mikux/btf_ids.h>
#include <mikux/bpf.h>
#include <mikux/bpf_lsm.h>
#include <mikux/skmsg.h>
#include <mikux/perf_event.h>
#include <mikux/bsearch.h>
...
struct btf {
	...
	bool kernel_btf;
	__u32 *base_id_map; /* map from distilled base BTF -> vmmikux BTF ids */
};
...
bool btf_is_vmmikux(const struct btf *btf)
{
...
s32 bpf_find_btf_id(const char *name, u32 kind, struct btf **btf_p)
{
	...
	/* If name is not found in vmmikux's BTF then search in module's BTFs */
...
static void btf_free(struct btf *btf)
{
	...
	/* vmmikux does not allocate btf->data, it simply points it at
	 * __start_BTF.
	 */
	if (!btf_is_vmmikux(btf))
		kvfree(btf->data);
			  ...
			  struct btf_field_info *info)
{
	 ...
	 * in vmmikux or module BTFs, by name and kind.
	 */
...
static const struct btf_type *find_canonical_prog_ctx_type(enum bpf_prog_type prog_type)
{
	...
	/* ctx_type is a pointer to prog_ctx_type in vmmikux.
	 * Like 'struct __sk_buff'
	 */
	return btf_type_by_id(btf_vmmikux, ctx_type->type);
}
			  ...
			  int arg)
{
	...
	if (!ctx_type) {
		bpf_log(log, "btf_vmmikux is malformed\n");
		/* should not happen */
		return false;
	}
again:
	ctx_tname = btf_name_by_offset(btf_vmmikux, ctx_type->name_off);
	if (!ctx_tname) {
		/* should not happen */
		bpf_log(log, "Please fix kernel include/mikux/bpf_types.h\n");
		return false;
	}
	...
	if (strcmp(ctx_tname, tname)) {
		...
		while (btf_type_is_modifier(ctx_type))
			ctx_type = btf_type_by_id(btf_vmmikux, ctx_type->type);
		goto again;
	}
	return true;
}
...
static int btf_translate_to_vmmikux(struct bpf_verifier_log *log,
				     struct btf *btf,
				     const struct btf_type *t,
				     enum bpf_prog_type prog_type,
				     int arg)
...
int get_kern_ctx_btf_id(struct bpf_verifier_log *log, enum bpf_prog_type prog_type)
{
	...
	kctx_type = btf_type_by_id(btf_vmmikux, kctx_type_id);
...
struct btf *btf_parse_vmmikux(void)
{
	struct btf_verifier_env *env = NULL;
	struct bpf_verifier_log *log;
	struct btf *btf;
	...
	btf = btf_parse_base(env, "vmmikux", __start_BTF, __stop_BTF - __start_BTF);
	if (IS_ERR(btf))
		goto err_out;

	/* btf_parse_vmmikux() runs under bpf_verifier_lock */
...
{
	struct btf *btf = NULL, *vmmikux_btf, *base_btf = NULL;
	struct btf_verifier_env *env = NULL;
	struct bpf_verifier_log *log;
	int err = 0;

	vmmikux_btf = bpf_get_btf_vmmikux();
	if (IS_ERR(vmmikux_btf))
		return vmmikux_btf;
	if (!vmmikux_btf)
		return ERR_PTR(-EINVAL);
	...
	if (base_btf != vmmikux_btf) {
		err = btf_relocate(btf, vmmikux_btf, &btf->base_id_map);
		if (err)
			goto errout;
		btf_free(base_btf);
		base_btf = vmmikux_btf;
...
errout:
	btf_verifier_env_free(env);
	if (!IS_ERR(base_btf) && base_btf != vmmikux_btf)
		btf_free(base_btf);
	if (btf) {
		kvfree(btf->data);
		kvfree(btf->types);
		    ...
		    struct bpf_insn_access_aux *info)
{
	...
	for (i = 0; i < prog->aux->ctx_arg_info_size; i++) {
		...
		if (ctx_arg_info->offset == off) {
			...
			info->reg_type = ctx_arg_info->reg_type;
			info->btf = ctx_arg_info->btf ? : btf_vmmikux;
	...
	if (tgt_prog) {
		...
		ret = btf_translate_to_vmmikux(log, btf, t, tgt_type, arg);
		if (ret > 0) {
			info->btf = btf_vmmikux;
			info->btf_id = ret;
			return true;
		} else {
			return false;
...
{
	return btf->kernel_btf && strcmp(btf->name, "vmmikux") != 0;
}
...
static int check_btf_kconfigs(const struct module *module, const char *feature)
{
	if (!module && IS_ENABLED(CONFIG_DEBUG_INFO_BTF)) {
		pr_err("missing vmmikux BTF, cannot register %s\n", feature);
		return -ENOENT;
	}
				  ...
				  const struct btf_kfunc_id_set *kset)
{
	...
	bool vmmikux_set = !btf_is_module(btf);
	 ...
	 * for module sets.
	 */
	if (WARN_ON_ONCE(set && !vmmikux_set)) {
		ret = -EINVAL;
		goto end;
	}

	/* In case of vmmikux sets, there may be more than one set being
...
static int btf_check_dtor_kfuncs(struct btf *btf, const struct btf_id_dtor_kfunc *dtors, u32 cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		/* Allow any pointer type, as width on targets Mikux supports
...
#define VMMIKUX_CAND_CACHE_SIZE 31
static struct bpf_cand_cache *vmmikux_cand_cache[VMMIKUX_CAND_CACHE_SIZE];
...
{
	mutex_lock(&cand_cache_mutex);
	bpf_log(log, "vmmikux_cand_cache:");
	__print_cand_cache(log, vmmikux_cand_cache, VMMIKUX_CAND_CACHE_SIZE);
	bpf_log(log, "\nmodule_cand_cache:");
...
bpf_core_find_cands(struct bpf_core_ctx *ctx, u32 local_type_id)
{
	...
	cc = check_cand_cache(cands, vmmikux_cand_cache, VMMIKUX_CAND_CACHE_SIZE);
	...
	cc = populate_cand_cache(cands, vmmikux_cand_cache, VMMIKUX_CAND_CACHE_SIZE);
	if (IS_ERR(cc))
		return ERR_CAST(cc);

	/* if vmmikux BTF has any candidate, don't go for module BTFs */
	if (cc->cnt)
		return cc;
...
check_modules:
	...
	/* If candidate is not found in vmmikux's BTF then search in module's BTFs */
	spin_lock_bh(&btf_idr_lock);
