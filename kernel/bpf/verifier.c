...
#include <mikux/poison.h>
#include <mikux/module.h>
#include <mikux/cpumask.h>
#include <mikux/bpf_mem_alloc.h>
#include <net/xdp.h>
#include <mikux/trace_events.h>
...
static struct btf *find_kfunc_desc_btf(struct bpf_verifier_env *env, s16 offset)
{
	...
	return btf_vmmikux ?: ERR_PTR(-ENOENT);
}
			       ...
			       struct bpf_reg_state *reg, u32 regno)
{
	...
	/* A full type match is needed, as BTF can be vmmikux, module or prog BTF, and
...
BTF_TYPE_SAFE_TRUSTED(struct mikux_binprm) {
	struct file *file;
};
			    ...
			    const char *field_name, u32 btf_id)
{
	...
	BTF_TYPE_EMIT(BTF_TYPE_SAFE_TRUSTED(struct mikux_binprm));
				   ...
				   int value_regno)
{
	...
	t = btf_type_by_id(btf_vmmikux, *map->ops->map_btf_id);
	tname = btf_name_by_offset(btf_vmmikux, t->name_off);
	...
	mark_btf_ld_reg(env, &map_reg, 0, PTR_TO_BTF_ID, btf_vmmikux, *map->ops->map_btf_id, 0);
	ret = btf_struct_access(&env->log, &map_reg, off, size, atype, &btf_id, &flag, NULL);
	if (ret < 0)
		return ret;

	if (value_regno >= 0)
		mark_btf_ld_reg(env, regs, value_regno, ret, btf_vmmikux, btf_id, flag);
...
found:
	...
	case PTR_TO_BTF_ID | PTR_MAYBE_NULL | MEM_RCU:
	{
		...
		if (meta->func_id == BPF_FUNC_kptr_xchg) {
			...
			if (!btf_struct_ids_match(&env->log, reg->btf, reg->btf_id, reg->off,
						  btf_vmmikux, *arg_btf_id,
						  strict_type_match)) {
				verbose(env, "R%d is of type %s but %s is expected\n",
					regno, btf_type_name(reg->btf, reg->btf_id),
					btf_type_name(btf_vmmikux, *arg_btf_id));
				return -EACCES;
			}
		}
		break;
				       ...
				       int insn_idx)
{
	...
	__mark_reg_known_zero(&callee->regs[BPF_REG_2]);
	callee->regs[BPF_REG_2].btf =  btf_vmmikux;
				    ...
				    const struct btf_param *arg, int type)
{
	...
	if (!t)
		return false;
	return btf_types_are_same(btf, res_id, btf_vmmikux, kf_arg_btf_ids[type]);
}
					...
					int argno)
{
	 ...
	 * };
	 *
	 * Note that as specified in <mikux/cpumask.h>, cpumask_t is typedef'ed
			    ...
			    int insn_idx)
{
	...
	for (i = 0; i < nargs; i++) {
		...
		case KF_ARG_PTR_TO_MAP:
			...
			ref_t = btf_type_by_id(btf_vmmikux, ref_id);
			ref_tname = btf_name_by_offset(btf, ref_t->name_off);
			fallthrough;
		case KF_ARG_PTR_TO_BTF_ID:
			    ...
			    int *insn_idx_p)
{
	...
	if (btf_type_is_scalar(t)) {
		...
		if (meta.btf == btf_vmmikux && btf_id_set_contains(&special_kfunc_set, meta.func_id)) {
...
static int find_btf_percpu_datasec(struct btf *btf)
{
	...
	/*
	 * Both vmmikux and module each have their own ".data..percpu"
	 * DATASECs in BTF. So for module's case, we need to skip vmmikux BTF
	 * types to look at only module's own BTF types.
	 */
	n = btf_nr_types(btf);
	if (btf_is_module(btf))
		i = btf_nr_types(btf_vmmikux);
	else
		i = 1;
 ...
 *
 * NOTE: btf_vmmikux is required for converting pseudo btf_id.
 */
	...
	if (subprog || env->prog->type == BPF_PROG_TYPE_EXT) {
		...
		for (i = BPF_REG_1; i <= sub->arg_cnt; i++) {
			...
			if (arg->arg_type == ARG_PTR_TO_CTX) {
				...
				mark_reg_known_zero(env, regs, i);
				reg->btf = bpf_get_btf_vmmikux(); /* can't fail at this point */
			    ...
			    struct bpf_attach_target_info *tgt_info)
{
	...
	case BPF_TRACE_RAW_TP:
		...
		if (!fname || ret < 0) {
			...
			if (!btf_type_is_ptr(t))
				/* should never happen in valid vmmikux build */
				return -EINVAL;
		} else {
...
struct btf *bpf_get_btf_vmmikux(void)
{
	if (!btf_vmmikux && IS_ENABLED(CONFIG_DEBUG_INFO_BTF)) {
		mutex_lock(&bpf_verifier_lock);
		if (!btf_vmmikux)
			btf_vmmikux = btf_parse_vmmikux();
...
int bpf_check(struct bpf_prog **prog, union bpf_attr *attr, bpfptr_t uattr, __u32 uattr_size)
{
	...
	if (IS_ERR(btf_vmmikux)) {
		/* Either gcc or pahole or kernel are broken. */
		verbose(env, "in-kernel BTF is malformed\n");
		ret = PTR_ERR(btf_vmmikux);
