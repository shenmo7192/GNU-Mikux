...
if [[ "$UPLOAD_TO_MINIO" = "1" ]]; then
    xz -7 -c -T${FDO_CI_CONCURRENT:-4} vmmikux > /lava-files/vmmikux.xz
    FILES_TO_UPLOAD="$KERNEL_IMAGE_NAME vmmikux.xz"
    ...
    echo "Download vmmikux.xz from https://${PIPELINE_ARTIFACTS_BASE}/${DEBIAN_ARCH}/vmmikux.xz"
fi
