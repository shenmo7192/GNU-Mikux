...
#define LCD_OMIKUXINO_HEADER_MAGIC	0x4F4CB727
#define LCD_OMIKUXINO_DATA_LEN		256

struct lcd_omikuxino_mode {
	u32 pixelclock;
	u32 hactive;
	u32 hfp;
	u32 hbp;
...
struct lcd_omikuxino_eeprom {
	u32 header;
	u32 id;
	char revision[4];
	u32 serial;
	struct lcd_omikuxino_info info;
	u32 num_modes;
	u8 reserved[180];
	u32 checksum;
} __attribute__((__packed__));

struct lcd_omikuxino {
	...
	struct lcd_omikuxino_eeprom eeprom;
};

static inline struct lcd_omikuxino *to_lcd_omikuxino(struct drm_panel *panel)
{
	return container_of(panel, struct lcd_omikuxino, panel);
}

static int lcd_omikuxino_unprepare(struct drm_panel *panel)
{
	struct lcd_omikuxino *lcd = to_lcd_omikuxino(panel);
...
static int lcd_omikuxino_prepare(struct drm_panel *panel)
{
	struct lcd_omikuxino *lcd = to_lcd_omikuxino(panel);
	int ret;
...
static int lcd_omikuxino_get_modes(struct drm_panel *panel,
				   struct drm_connector *connector)
{
	struct lcd_omikuxino *lcd = to_lcd_omikuxino(panel);
	struct lcd_omikuxino_info *lcd_info = &lcd->eeprom.info;
	struct lcd_omikuxino_mode *lcd_mode;
	struct drm_display_mode *mode;
	u32 i, num = 0;

	for (i = 0; i < lcd->eeprom.num_modes; i++) {
		lcd_mode = (struct lcd_omikuxino_mode *)
...
static const struct drm_panel_funcs lcd_omikuxino_funcs = {
	.unprepare = lcd_omikuxino_unprepare,
	.prepare = lcd_omikuxino_prepare,
	.get_modes = lcd_omikuxino_get_modes,
};

static int lcd_omikuxino_probe(struct i2c_client *client)
{
	struct device *dev = &client->dev;
	struct lcd_omikuxino *lcd;
	u32 checksum, i;
	int ret = 0;
	...
	for (i = 0; i < LCD_OMIKUXINO_DATA_LEN; i += I2C_SMBUS_BLOCK_MAX) {
	...
	/* Check magic header */
	if (lcd->eeprom.header != LCD_OMIKUXINO_HEADER_MAGIC) {
	...
	drm_panel_init(&lcd->panel, dev, &lcd_omikuxino_funcs,
...
static void lcd_omikuxino_remove(struct i2c_client *client)
{
	struct lcd_omikuxino *panel = i2c_get_clientdata(client);

	drm_panel_remove(&panel->panel);
}

static const struct of_device_id lcd_omikuxino_of_ids[] = {
	{ .compatible = "olimex,lcd-omikuxino" },
	{ }
};
MODULE_DEVICE_TABLE(of, lcd_omikuxino_of_ids);

static struct i2c_driver lcd_omikuxino_driver = {
	.driver = {
		.name = "lcd_omikuxino",
		.of_match_table = lcd_omikuxino_of_ids,
	},
	.probe = lcd_omikuxino_probe,
	.remove = lcd_omikuxino_remove,
};

module_i2c_driver(lcd_omikuxino_driver);
