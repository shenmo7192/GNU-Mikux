...
static const struct mikux_logo *logo_mono;

static int drm_panic_setup_logo(void)
{
	const struct mikux_logo *logo = fb_find_logo(1);
	const unsigned char *logo_data;
	struct mikux_logo *logo_dup;

	if (!logo || logo->type != MIKUX_LOGO_MONO)
		return 0;
...
#else
#define logo_mono	((const struct mikux_logo *)NULL)
#endif
