...
static int chv_gpio_set_intr_line(struct intel_pinctrl *pctrl, unsigned int pin)
{
	 ...
	 * But this may also be caused by Mikux deciding to use a pin as
...
/*
 * Certain machines seem to hardcode Mikux IRQ numbers in their ACPI
