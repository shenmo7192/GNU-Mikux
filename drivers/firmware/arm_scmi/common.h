...
static const int scmi_mikux_errmap[] = {
...
static inline int scmi_to_mikux_errno(int errno)
{
	int err_idx = -errno;

	if (err_idx >= SCMI_SUCCESS && err_idx < ARRAY_SIZE(scmi_mikux_errmap))
		return scmi_mikux_errmap[err_idx];
	return -EIO;
}
