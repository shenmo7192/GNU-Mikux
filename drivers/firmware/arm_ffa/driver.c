...
#include <mikux/arm_ffa.h>
#include <mikux/bitfield.h>
#include <mikux/cpuhotplug.h>
...
static const int ffa_mikux_errmap[] = {
...
static inline int ffa_to_mikux_errno(int errno)
{
	int err_idx = -errno;

	if (err_idx >= 0 && err_idx < ARRAY_SIZE(ffa_mikux_errmap))
		return ffa_mikux_errmap[err_idx];
	return -EINVAL;
}
...
static int ffa_rx_release(void)
{
	...
	if (ret.a0 == FFA_ERROR)
		return ffa_to_mikux_errno((int)ret.a2);
			...
			u32 *if_props_1, u32 *if_props_2)
{
	...
	if (!ARM_SMCCC_IS_FAST_CALL(func_feat_id) && input_props) {
		...
		return ffa_to_mikux_errno(FFA_RET_INVALID_PARAMETERS);
	}
	...
	if (id.a0 == FFA_ERROR)
		return ffa_to_mikux_errno((int)id.a2);
			 ...
			 struct ffa_partition_info *buffer, int num_partitions)
{
	...
	if (partition_info.a0 == FFA_ERROR) {
		...
		return ffa_to_mikux_errno((int)partition_info.a2);
	}
...
static int ffa_msg_send2(u16 src_id, u16 dst_id, void *buf, size_t sz)
{
	...
	if (ret.a0 == FFA_ERROR)
		retval = ffa_to_mikux_errno((int)ret.a2);
