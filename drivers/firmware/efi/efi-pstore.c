				...
				efi_char16_t *varname)
{
	...
	status = efivar_get_variable(varname, &MIKUX_EFI_CRASH_GUID, NULL,
				     &size, record->buf);
...
static ssize_t efi_pstore_read(struct pstore_record *record)
{
	...
	efi_guid_t guid = MIKUX_EFI_CRASH_GUID;
	...
	for (;;) {
		...
		if (efi_guidcmp(guid, MIKUX_EFI_CRASH_GUID))
			continue;
...
static int efi_pstore_write(struct pstore_record *record)
{
	...
	if (efivar_trylock())
		return -EBUSY;
	status = efivar_set_variable_locked(efi_name, &MIKUX_EFI_CRASH_GUID,
...
static int efi_pstore_erase(struct pstore_record *record)
{
	...
	status = efivar_set_variable(record->priv, &MIKUX_EFI_CRASH_GUID,
