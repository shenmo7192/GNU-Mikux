...
#include <mikux/mailbox_client.h>
...
#include <mikux/pm_opp.h>
#include <mikux/scpi_protocol.h>
...
static int scpi_mikux_errmap[SCPI_ERR_MAX] = {
...
static inline int scpi_to_mikux_errno(int errno)
{
	if (errno >= SCPI_SUCCESS && errno < SCPI_ERR_MAX)
		return scpi_mikux_errmap[errno];
	return -EIO;
}
...
out:
	...
	/* SCPI error codes > 0, translate them to Mikux scale*/
	return ret > 0 ? scpi_to_mikux_errno(ret) : ret;
}
