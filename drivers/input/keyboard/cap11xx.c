 ...
 *
 * (c) 2014 Daniel Mack <mikux@zonque.org>
 */
...
static int cap11xx_init_keys(struct cap11xx_priv *priv)
{
	...
	of_property_read_u32_array(node, "mikux,keycodes",
				   priv->keycodes, priv->model->num_channels);

	/* Disable autorepeat. The Mikux input system has its own handling. */
			     ...
			     struct cap11xx_priv *priv, int num_leds)
{
	...
	for_each_child_of_node(node, child) {
		...
		led->cdev.default_trigger =
			of_get_property(child, "mikux,default-trigger", NULL);
		led->cdev.flags = 0;
...
MODULE_AUTHOR("Daniel Mack <mikux@zonque.org>");
