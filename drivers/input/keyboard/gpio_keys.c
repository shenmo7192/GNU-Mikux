...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	device_for_each_child_node_scoped(dev, child) {
		...
		if (fwnode_property_read_u32(child, "mikux,code",
					     &button->code)) {
		...
		if (fwnode_property_read_u32(child, "mikux,input-type",
					     &button->type))
			button->type = EV_KEY;

		fwnode_property_read_u32(child, "mikux,input-value",
					 (u32 *)&button->value);
		...
		button->can_disable =
			fwnode_property_read_bool(child, "mikux,can-disable");
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	/* Enable auto repeat feature of Mikux input subsystem */
	if (pdata->rep)
