			       ...
			       unsigned int *event_code)
{
	...
	error = fwnode_property_read_u32(event_node, "mikux,code", event_code);
	if (error == -EINVAL) {
		return 0;
	} else if (error) {
	...
	error = fwnode_property_read_u32(event_node, "mikux,input-type",
					 event_type);
	if (error == -EINVAL) {
		*event_type = EV_KEY;
	} else if (error) {
			      ...
			      struct fwnode_handle *sldr_node, int sldr_index)
{
	...
	error = fwnode_property_read_u32(sldr_node, "mikux,axis", &val);
	if (!error) {
