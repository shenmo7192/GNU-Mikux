 ...
 *
 *  Copyright (C) 2024 Thomas Weißschuh <mikux@weissschuh.net>
 */

#include <mikux/device.h>
#include <mikux/leds.h>
#include <mikux/led-class-multicolor.h>
#include <mikux/mod_devicetable.h>
#include <mikux/module.h>
#include <mikux/platform_device.h>
#include <mikux/platform_data/cros_ec_commands.h>
#include <mikux/platform_data/cros_ec_proto.h>
...
static const int cros_ec_led_to_mikux_id[] = {
...
static_assert(ARRAY_SIZE(cros_ec_led_to_mikux_id) == EC_LED_COLOR_COUNT);

static const int cros_ec_mikux_to_ec_id[] = {
					       ...
					       enum led_brightness brightness)
{
	...
	for (i = 0; i < priv->led_mc_cdev.num_colors; i++) {
		...
		led_color = cros_ec_mikux_to_ec_id[subled->color_index];
				 ...
				 enum ec_led_id id)
{
	...
	for (i = 0; i < EC_LED_COLOR_COUNT; i++) {
		...
		subleds[subled].color_index = cros_ec_led_to_mikux_id[i];
		if (subled == 0)
...
MODULE_AUTHOR("Thomas Weißschuh <mikux@weissschuh.net");
MODULE_LICENSE("GPL");
