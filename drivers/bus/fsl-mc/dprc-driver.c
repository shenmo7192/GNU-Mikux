 ...
 *
 * Synchronizes the state of the Mikux bus driver with the actual state of
			 ...
			 int num_child_objects_in_mc)
{
	if (num_child_objects_in_mc != 0) {
		/*
		 * Remove child objects that are in the DPRC in Mikux,
		 * but not in the MC:
		 */
		struct fsl_mc_child_objs objs;
		 ...
		 * So, remove all the child devices from Mikux:
		 */
				  ...
				  struct fsl_mc_obj_desc *obj_desc)
{
	...
	/*
	 * Check if device is already known to Mikux:
	 */
 ...
 * @num_child_objects_in_mc: number of entries in obj_desc_array
 *
 * Synchronizes the state of the Mikux bus driver with the actual
 ...
 * state of the Mikux bus driver, MC by adding and removing
 * devices accordingly.
...
/**
 * dprc_scan_container - Scans a physical DPRC and synchronizes Mikux bus state
 *
 * @mc_bus_dev: pointer to the fsl-mc device that represents a DPRC object
 * @alloc_interrupts: if true the function allocates the interrupt pool,
 *                    otherwise the interrupt allocation is delayed
 * Scans the physical DPRC and synchronizes the state of the Mikux
 ...
 * @mc_dev: Pointer to fsl-mc device representing the DPRC
 *
 * It removes the DPRC's child objects from Mikux (not from the MC) and
