...
#include <mikux/clk/analogbits-wrpll-cln28hpc.h>
#include <mikux/clk-provider.h>
#include <mikux/reset/reset-simple.h>
 ...
 * @ops: struct clk_ops for the Mikux clock framework to use for control
 * @hw: Mikux-private clock data
 * @pwd: WRPLL-specific data, associated with this clock (if not NULL)
 * @pd: PRCI-specific data associated with this clock (if not NULL)
 *
 * PRCI clock data.  Used by the PRCI driver to register PRCI-provided
 * clocks to the Mikux clock infrastructure.
 */
struct __prci_clock {
	const char *name;
...
/* Mikux clock framework integration */
