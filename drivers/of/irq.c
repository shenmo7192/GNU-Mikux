 ...
 *    Copyright (C) 1995-1996 Gary Thomas (gdt@mikuxppc.org)
...
/**
 * irq_of_parse_and_map - Parse and map an interrupt into mikux virq space
...
int of_irq_parse_raw(const __be32 *addr, struct of_phandle_args *out_irq)
{
	...
	/* Look for this #address-cells. We have to implement the old mikux
 ...
 * interrupt specifier that can be used to retrieve a Mikux IRQ number.
 */
...
/**
 * of_irq_get - Decode a node's IRQ and return it as a Mikux IRQ number
 * @dev: pointer to device tree node
 * @index: zero-based index of the IRQ
 *
 * Return: Mikux IRQ number on success, or 0 on the IRQ mapping failure, or
...
/**
 * of_irq_get_byname - Decode a node's IRQ and return it as a Mikux IRQ number
 * @dev: pointer to device tree node
 * @name: IRQ name
 *
 * Return: Mikux IRQ number on success, or 0 on the IRQ mapping failure, or
