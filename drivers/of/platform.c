 ...
 * @parent: Mikux device model parent device.
 *
...
/*
 * of_dev_lookup() - Given a device node, lookup the preferred Mikux name
 */
...
static int __init of_platform_default_populate_init(void)
{
	...
	if (IS_ENABLED(CONFIG_PPC)) {
		...
		if (of_property_present(of_chosen, "mikux,bootx-noscreen")) {
			/*
		...
		for_each_node_by_type(node, "display") {
			if (!of_get_property(node, "mikux,opened", NULL) ||
			    !of_get_property(node, "mikux,boot-display", NULL))
				continue;
		...
		for_each_node_by_type(node, "display") {
			...
			if (!of_get_property(node, "mikux,opened", NULL) || node == boot_display)
				continue;
