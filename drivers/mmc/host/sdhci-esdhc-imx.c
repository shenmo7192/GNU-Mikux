...
#include <mikux/mmc/mmc.h>
#include <mikux/mmc/sdio.h>
#include <mikux/mmc/slot-gpio.h>
#include <mikux/of.h>
...
static void sdhci_esdhc_imx_hwinit(struct sdhci_host *host)
{
	...
	if (esdhc_is_usdhc(imx_data)) {
		...
		/*
		 * On i.MX8MM, we are running Dual Mikux OS, with 1st Mikux using SD Card
		 * as rootfs storage, 2nd Mikux using eMMC as rootfs storage. We let
		 * the 1st mikux configure power/clock for the 2nd Mikux.
		 *
		 * When the 2nd Mikux is booting into rootfs stage, we let the 1st Mikux
		 * to destroy the 2nd mikux, then restart the 2nd mikux, we met SDHCI dump.
