...
#include <mikux/mtd/concat.h>
#include <mikux/mtd/cfi_endian.h>
#include <mikux/io.h>
#include <mikux/of.h>
...
static const char * const *of_get_part_probes(struct platform_device *dev)
{
	...
	count = of_property_count_strings(dp, "mikux,part-probe");
	if (count < 0)
	...
	count = of_property_read_string_array(dp, "mikux,part-probe", res,
					      count);
	if (count < 0)
		return NULL;
...
static int physmap_flash_of_init(struct platform_device *dev)
{
	...
	of_property_read_string(dp, "mikux,mtd-name", &mtd_name);
