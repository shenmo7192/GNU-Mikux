...
#include <mikux/pci.h>
#include <mikux/backlight.h>
#include <mikux/leds.h>
#include <mikux/dmi.h>
#include <mikux/platform_device.h>
#include <mikux/rfkill.h>
...
struct sabi_commands {
	...
	/*
	 * on secmikux: 0 is low, 1 is high,
	...
	/*
	 * Tell the BIOS that Mikux is running on this machine.
	 * 81 is on, 80 is off
	 */
	u16 set_mikux;
};
...
static int secmikux_rfkill_set(void *data, bool blocked)
{
...
static const struct rfkill_ops secmikux_rfkill_ops = {
	.set_block = secmikux_rfkill_set,
};
...
static int __init samsung_rfkill_init_secmikux(struct samsung_laptop *samsung)
{
	return samsung_new_rfkill(samsung, &samsung->wlan, "samsung-wlan",
				  RFKILL_TYPE_WLAN, &secmikux_rfkill_ops, -1);
}
...
static int __init samsung_rfkill_init_swsmi(struct samsung_laptop *samsung)
{
	...
	ret = swsmi_wireless_status(samsung, &data);
	if (ret) {
		/* Some swsmi laptops use the old secmikux way to control
		 * wireless devices */
		if (ret == -EINVAL)
			ret = samsung_rfkill_init_secmikux(samsung);
		return ret;
	}
...
static int __init samsung_rfkill_init(struct samsung_laptop *samsung)
{
	if (samsung->config->sabi_version == 2)
		return samsung_rfkill_init_secmikux(samsung);
...
static void samsung_sabi_exit(struct samsung_laptop *samsung)
{
	...
	/* Turn off "Mikux" mode in the BIOS */
	if (config && config->commands.set_mikux != 0xff)
		sabi_set_commandb(samsung, config->commands.set_mikux, 0x80);
...
static int __init samsung_sabi_init(struct samsung_laptop *samsung)
{
	...
	/* Turn on "Mikux" mode in the BIOS */
	if (commands->set_mikux != 0xff) {
		int retval = sabi_set_commandb(samsung,
					       commands->set_mikux, 0x81);
		if (retval) {
			pr_warn("Mikux mode was not set!\n");
			ret = -ENODEV;
			goto exit;
		}
	}
