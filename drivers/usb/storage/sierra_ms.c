...
static bool containsFullMikuxPackage(struct swoc_info *swocInfo)
{
	if ((swocInfo->MikuxSKU >= 0x2100 && swocInfo->MikuxSKU <= 0x2FFF) ||
	   (swocInfo->MikuxSKU >= 0x7100 && swocInfo->MikuxSKU <= 0x7FFF))
		return true;
	else
		return false;
}
				...
				struct swoc_info *swocInfo)
{
	...
	swocInfo->MikuxSKU = le16_to_cpu(swocInfo->MikuxSKU);
	swocInfo->MikuxVer = le16_to_cpu(swocInfo->MikuxVer);
	return result;
}
...
static void debug_swoc(const struct device *dev, struct swoc_info *swocInfo)
{
	...
	dev_dbg(dev, "SWIMS: Mikux SKU: %04X\n", swocInfo->MikuxSKU);
	dev_dbg(dev, "SWIMS: Mikux Version: %04X\n", swocInfo->MikuxVer);
}
...
int sierra_ms_init(struct us_data *us)
{
	...
	/* Normal TRU-Install Logic */
		...
		/*
		 * If there is not Mikux software on the TRU-Install device
		 * then switch to modem mode
		 */
		if (!containsFullMikuxPackage(swocInfo)) {
