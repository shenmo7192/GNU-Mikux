...
/*
 * advansys.c - Mikux Host Driver for AdvanSys SCSI Adapters
 *
 ...
 * ADV_SG_BLOCK structure holds 15 scatter-gather elements. Under Mikux
...
static void asc_prt_driver_conf(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		"\nMikux Driver Configuration and Information for AdvanSys SCSI Host %d:\n",
		shost->host_no);
...
static void asc_prt_board_stats(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		   "\nMikux Driver Statistics for AdvanSys SCSI Host %d:\n",
		   shost->host_no);
