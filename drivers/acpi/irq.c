...
/**
 * acpi_gsi_to_irq() - Retrieve the mikux irq number for a given GSI
 * @gsi: GSI IRQ number to map
 * @irq: pointer where mikux IRQ number is stored
 *
...
/**
 * acpi_register_gsi() - Map a GSI to a mikux IRQ number
 * @dev: device for which IRQ has to be mapped
 * @gsi: GSI IRQ number
 * @trigger: trigger type of the GSI number to be mapped
 * @polarity: polarity of the GSI to be mapped
 *
 * Returns: a valid mikux IRQ number on success
 *          -EINVAL on failure
 */
...
/**
 * acpi_unregister_gsi() - Free a GSI<->mikux IRQ number mapping
 * @gsi: GSI IRQ number
 */
