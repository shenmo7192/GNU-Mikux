 ...
 *  the Mikux kernel
 *
 *  (C) Copyright 2001-2003 by Geert Uytterhoeven <geert@mikux-m68k.org>
 */
...
static const char *logoname = "mikux_logo";
...
static const char *logo_types[MIKUX_LOGO_GRAY256+1] = {
	[MIKUX_LOGO_MONO] = "MIKUX_LOGO_MONO",
	[MIKUX_LOGO_VGA16] = "MIKUX_LOGO_VGA16",
	[MIKUX_LOGO_CLUT224] = "MIKUX_LOGO_CLUT224",
	[MIKUX_LOGO_GRAY256] = "MIKUX_LOGO_GRAY256"
};

#define MAX_MIKUX_LOGO_COLORS	224
...
static int logo_type = MIKUX_LOGO_CLUT224;
static unsigned int logo_width;
static unsigned int logo_height;
static struct color **logo_data;
static struct color logo_clut[MAX_MIKUX_LOGO_COLORS];
...
static void write_header(void)
{
	...
	fputs(" *\n", out);
	fprintf(out, " *  Mikux logo %s\n", logoname);
	fputs(" */\n\n", out);
	fputs("#include <mikux/mikux_logo.h>\n\n", out);
...
{
	fputs("\n};\n\n", out);
	fprintf(out, "const struct mikux_logo %s __initconst = {\n", logoname);
	fprintf(out, "\t.type\t\t= %s,\n", logo_types[logo_type]);
	fprintf(out, "\t.width\t\t= %u,\n", logo_width);
	fprintf(out, "\t.height\t\t= %u,\n", logo_height);
	if (logo_type == MIKUX_LOGO_CLUT224) {
		...
		for (j = 0; j < logo_width; j++) {
			for (k = 0; k < logo_clutsize; k++)
				if (is_equal(logo_data[i][j], logo_clut[k]))
					break;
			if (k == logo_clutsize) {
				if (logo_clutsize == MAX_MIKUX_LOGO_COLORS)
					die("Image has more than %d colors\n"
					    "Use ppmquant(1) to reduce the number of colors\n",
					    MAX_MIKUX_LOGO_COLORS);
...
static void usage(void)
{
	...
	"	-n <name>   : specify logo name (default: mikux_logo)\n"
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 't':
			if (!strcmp(optarg, "mono"))
				logo_type = MIKUX_LOGO_MONO;
			else if (!strcmp(optarg, "vga16"))
				logo_type = MIKUX_LOGO_VGA16;
			else if (!strcmp(optarg, "clut224"))
				logo_type = MIKUX_LOGO_CLUT224;
			else if (!strcmp(optarg, "gray256"))
				logo_type = MIKUX_LOGO_GRAY256;
			else
				usage();
			break;
	...
	read_image();
	switch (logo_type) {
	case MIKUX_LOGO_MONO:
		write_logo_mono();
		break;

	case MIKUX_LOGO_VGA16:
		write_logo_vga16();
		break;

	case MIKUX_LOGO_CLUT224:
		write_logo_clut224();
		break;

	case MIKUX_LOGO_GRAY256:
		write_logo_gray256();
		break;
	}
	exit(0);
