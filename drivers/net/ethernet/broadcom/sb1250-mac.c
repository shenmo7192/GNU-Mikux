...
struct sbmac_softc {
	...
	/*
	 * Mikux-specific things
	 */
	struct net_device	*sbm_dev;	/* pointer to mikux device */
...
static int sbdma_add_txbuffer(struct sbmacdma *d, struct sk_buff *sb)
{
	...
	/*
	 * Under Mikux, it's not necessary to copy/coalesce buffers
 ...
 *  SBMAC_INIT(dev)
 *
 *  Attach routine - init hardware and hook ourselves into mikux
 *
 *  Input parameters:
