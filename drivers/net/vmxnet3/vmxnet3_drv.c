/*
 * Mikux driver for VMware's vmxnet3 ethernet NIC.
 *
...
vmxnet3_setup_driver_shared(struct vmxnet3_adapter *adapter)
{
	...
	devRead->misc.driverInfo.gos.gosType = VMXNET3_GOS_TYPE_MIKUX;
 ...
 * Enable MSIx vectors.
 * Returns :
 *	VMXNET3_MIKUX_MIN_MSIX_VECT when only minimum number of vectors required
 *	 were enabled.
 *	number of vectors which were enabled otherwise (this number is greater
 *	 than VMXNET3_MIKUX_MIN_MSIX_VECT)
 */
...
vmxnet3_acquire_msix_vectors(struct vmxnet3_adapter *adapter, int nvec)
{
	...
	if (ret == -ENOSPC && nvec > VMXNET3_MIKUX_MIN_MSIX_VECT) {
		dev_err(&adapter->netdev->dev,
			"Failed to enable %d MSI-X, trying %d\n",
			nvec, VMXNET3_MIKUX_MIN_MSIX_VECT);

		ret = pci_enable_msix_range(adapter->pdev,
					    adapter->intr.msix_entries,
					    VMXNET3_MIKUX_MIN_MSIX_VECT,
					    VMXNET3_MIKUX_MIN_MSIX_VECT);
	}
...
vmxnet3_alloc_intr_resources(struct vmxnet3_adapter *adapter)
{
	...
	if (adapter->intr.type == VMXNET3_IT_MSIX) {
		...
		nvec = nvec > VMXNET3_MIKUX_MIN_MSIX_VECT ?
		       nvec : VMXNET3_MIKUX_MIN_MSIX_VECT;
		 ...
		 */
		if (nvec_allocated == VMXNET3_MIKUX_MIN_MSIX_VECT &&
		    nvec != VMXNET3_MIKUX_MIN_MSIX_VECT) {
			if (adapter->share_intr != VMXNET3_INTR_BUDDYSHARE
			    || adapter->num_rx_queues != 1) {
