...
static void gaudi2_init_firmware_loader(struct hl_device *hdev)
{
	...
	fw_loader->mikux_img.image_name = GAUDI2_MIKUX_FW_FILE;
...
static void gaudi2_send_hard_reset_cmd(struct hl_device *hdev)
{
	...
	/*
	 * when Mikux/Bootfit exist this write to the SP can be interpreted in 2 ways:
	...
	/*
	 * When working with preboot (without Mikux/Boot fit) we can
	 * communicate only using the COMMS commands to issue halt/reset.
	 *
	 * For the case in which we are working with Mikux/Bootfit this is a hail-mary
	 ...
	 * trigger the reset as if Mikux wasn't loaded.
	 *
