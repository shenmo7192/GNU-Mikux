...
#define GOYA_MIKUX_FW_FILE	"habanalabs/goya/goya-fit.itb"
...
/*
 * goya_load_firmware_to_device() - Load MIKUX FW code to device.
 * @hdev: Pointer to hl_device structure.
 *
 * Copy MIKUX fw code from firmware file to HBM BAR.
 *
...
static int goya_load_firmware_to_device(struct hl_device *hdev)
{
	...
	dst = hdev->pcie_bar[DDR_BAR_ID] + MIKUX_FW_OFFSET;

	return hl_fw_load_fw_to_device(hdev, GOYA_MIKUX_FW_FILE, dst, 0, 0);
}
...
static void goya_init_firmware_loader(struct hl_device *hdev)
{
	...
	fw_loader->mikux_img.image_name = GOYA_MIKUX_FW_FILE;
...
static int goya_init_cpu(struct hl_device *hdev)
{
	...
	/*
	 * Before pushing u-boot/mikux to device, need to set the ddr bar to
