...
#include <mikux/habanalabs/hl_boot_if.h>
				...
				u32 boot_err1_reg)
{
	...
	if (kernel_ver) {
		dev_info(hdev->dev, "Mikux version %s", kernel_ver);
		kfree(kernel_ver);
	}
...
void hl_fw_ask_hard_reset_without_mikux(struct hl_device *hdev)
{
...
void hl_fw_ask_halt_machine_without_mikux(struct hl_device *hdev)
{
...
static void hl_fw_dynamic_update_mikux_interrupt_if(struct hl_device *hdev)
{
	struct cpu_dyn_regs *dyn_regs =
						...
						u32 img_ld_timeout)
{
	 ...
	 * 2. current FW component is boot-fit and we want to load mikux
	 */
	if (load_fwc == FW_COMP_BOOT_FIT) {
		...
		fw_name = fw_loader->mikux_img.image_name;
	}
					...
					struct fw_load_mgr *fw_loader)
{
	 ...
	 * Note that the CPU_BOOT_STATUS_SRAM_AVAIL is generally set by Mikux
	 * yet there is a debug scenario in which we loading uboot (without Mikux)
...
static int hl_fw_dynamic_wait_for_mikux_active(struct hl_device *hdev,
						struct fw_load_mgr *fw_loader)
{
	struct dynamic_fw_load_mgr *dyn_loader;
	u32 status;
	...
	/* Make sure CPU mikux is running */
	...
	rc = hl_poll_timeout(
		...
		fw_loader->cpu_timeout);
	if (rc) {
		dev_err(hdev->dev, "failed to wait for Mikux (status = %d)\n", status);
		return rc;
	}
...
/**
 * hl_fw_mikux_update_state -	update internal data structures after Mikux
 *				is loaded.
 *				Note: Mikux initialization is comprised mainly
 ...
 * @return 0 on success, otherwise non-zero error code
 */
static void hl_fw_mikux_update_state(struct hl_device *hdev,
						u32 cpu_boot_dev_sts0_reg,
						u32 cpu_boot_dev_sts1_reg)
{
	struct asic_fixed_properties *prop = &hdev->asic_prop;

	hdev->fw_loader.fw_comp_loaded |= FW_TYPE_MIKUX;
					...
					struct fw_load_mgr *fw_loader)
{
	...
	/*
	 * when testing FW load (without Mikux) on PLDM we don't want to
	 * wait until boot fit is active as it may take several hours.
	 * instead, we load the bootfit and let it do all initialization in
	 * the background.
	 */
	if (hdev->pldm && !(hdev->fw_components & FW_TYPE_MIKUX))
		return 0;

	/* Enable DRAM scrambling before Mikux boot and after successful
	 *  UBoot
	 */
	hdev->asic_funcs->init_cpu_scrambler_dram(hdev);

	if (!(hdev->fw_components & FW_TYPE_MIKUX)) {
		dev_dbg(hdev->dev, "Skip loading Mikux F/W\n");
		return 0;
	}
	...
	/* load Mikux image to FW */
	rc = hl_fw_dynamic_load_image(hdev, fw_loader, FW_COMP_MIKUX,
							fw_loader->cpu_timeout);
	if (rc) {
		dev_err(hdev->dev, "failed to load Mikux\n");
		goto protocol_err;
	}

	rc = hl_fw_dynamic_wait_for_mikux_active(hdev, fw_loader);
	if (rc)
		goto protocol_err;

	hl_fw_mikux_update_state(hdev,
				le32_to_cpu(dyn_regs->cpu_boot_dev_sts0),
				le32_to_cpu(dyn_regs->cpu_boot_dev_sts1));

	hl_fw_dynamic_update_mikux_interrupt_if(hdev);
					...
					struct fw_load_mgr *fw_loader)
{
	...
	if (!(hdev->fw_components & FW_TYPE_MIKUX)) {
		dev_info(hdev->dev, "Skip loading Mikux F/W\n");
		rc = 0;
		goto out;
	}
	...
	hl_fw_mikux_update_state(hdev, cpu_boot_dev_status0_reg,
						cpu_boot_dev_status1_reg);
