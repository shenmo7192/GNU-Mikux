...
#define GAUDI_MIKUX_FW_FILE	"habanalabs/gaudi/gaudi-fit.itb"
#define GAUDI_TPC_FW_FILE	"habanalabs/gaudi/gaudi_tpc.bin"

MODULE_FIRMWARE(GAUDI_BOOT_FIT_FILE);
MODULE_FIRMWARE(GAUDI_MIKUX_FW_FILE);
...
static int gaudi_load_firmware_to_device(struct hl_device *hdev)
{
	...
	dst = hdev->pcie_bar[HBM_BAR_ID] + MIKUX_FW_OFFSET;

	return hl_fw_load_fw_to_device(hdev, GAUDI_MIKUX_FW_FILE, dst, 0, 0);
}
...
static void gaudi_init_firmware_loader(struct hl_device *hdev)
{
	...
	fw_loader->mikux_img.image_name = GAUDI_MIKUX_FW_FILE;
...
static void gaudi_pre_hw_init(struct hl_device *hdev)
{
	...
	if (!hdev->asic_prop.fw_security_enabled) {
		/* Set the access through PCI bars (Mikux driver only) as
		 * secured
		 */
...
static int gaudi_hw_init(struct hl_device *hdev)
{
	...
	/*
	 * Before pushing u-boot/mikux to device, need to set the hbm bar to
...
static int gaudi_hw_fini(struct hl_device *hdev, bool hard_reset, bool fw_reset)
{
	...
	/* If mikux is loaded in the device CPU we need to communicate with it
	 * via the GIC. Otherwise, we need to use COMMS or the MSG_TO_CPU
	 * registers in case of old F/Ws
	 */
	if (hdev->fw_loader.fw_comp_loaded & FW_TYPE_MIKUX) {
		irq_handler_offset = hdev->asic_prop.gic_interrupts_enable ?
				mmGIC_DISTRIBUTOR__5_GICD_SETSPI_NSR :
		 ...
		 * reset as if Mikux wasn't loaded.
		 *
		...
		if (hdev->reset_info.curr_reset_cause == HL_RESET_CAUSE_HEARTBEAT) {
			if (hdev->asic_prop.hard_reset_done_by_fw)
				hl_fw_ask_hard_reset_without_mikux(hdev);
			else
				hl_fw_ask_halt_machine_without_mikux(hdev);
		}
	} else {
		if (hdev->asic_prop.hard_reset_done_by_fw)
			hl_fw_ask_hard_reset_without_mikux(hdev);
