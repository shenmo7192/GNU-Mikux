...
{
	char elfheader[256];
	unsigned char *vmlinuz_addr = (unsigned char *)_vmmikux_start;
	unsigned long vmlinuz_size = _vmmikux_end - _vmmikux_start;
	void *addr = 0;
	struct elf_info ei;
	long len;
	...
	if (platform_ops.vmmikux_alloc) {
		addr = platform_ops.vmmikux_alloc(ei.memsize);
	} else {
		/*
...
static struct addr_range prep_initrd(struct addr_range vmmikux, void *chosen,
				     ...
				     unsigned long initrd_size)
{
	...
	setprop_val(chosen, "mikux,initrd-start", (u32)(initrd_addr));
	setprop_val(chosen, "mikux,initrd-end", (u32)(initrd_addr+initrd_size));

	return (struct addr_range){(void *)initrd_addr, initrd_size};
}

#ifdef __powerpc64__
static void prep_esm_blob(struct addr_range vmmikux, void *chosen)
{
	...
	setprop_val(chosen, "mikux,esm-blob-start", (u32)(esm_blob_addr));
	setprop_val(chosen, "mikux,esm-blob-end", (u32)(esm_blob_addr + esm_blob_size));
}
#else
static inline void prep_esm_blob(struct addr_range vmmikux, void *chosen) { }
#endif

/* A buffer that may be edited by tools operating on a zImage binary so as to
 * edit the command line passed to vmmikux (by setting /chosen/bootargs).
...
static void prep_cmdline(void *chosen)
{
	...
	/* Wait-for-input time */
	n = getprop(chosen, "mikux,cmdline-timeout", &v, sizeof(v));
	if (n == sizeof(v))
		getline_timeout = v;
...
void start(void)
{
	struct addr_range vmmikux, initrd;
	...
	vmmikux = prep_kernel();
	initrd = prep_initrd(vmmikux, chosen,
			     loader_info.initrd_addr, loader_info.initrd_size);
	prep_esm_blob(vmmikux, chosen);
	prep_cmdline(chosen);
	...
	kentry = (kernel_entry_t) vmmikux.addr;
	if (ft_addr) {
		if(platform_ops.kentry)
			platform_ops.kentry(ft_addr, vmmikux.addr);
		else
	...
	fatal("Error: Mikux kernel returned to zImage boot wrapper!\n\r");
}
