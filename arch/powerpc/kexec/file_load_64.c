 ...
 * @lmb:                    LMB info.
 * @usm:                    mikux,drconf-usable-memory property value.
				  ...
				  void *data)
{
	 ...
	 * mikux,drconf-usable-memory property.
	 */
	if (*usm) {
		pr_err("mikux,drconf-usable-memory property already exists!");
		return -EINVAL;
	}
				   ...
				   struct umem_info *um_info)
{
	 ...
	 * Write (0,0) tuple in mikux,usable-memory property for
	...
	ret = fdt_setprop(fdt, node, "mikux,usable-memory", um_info->buf,
...
/**
 * update_usable_mem_fdt - Updates kdump kernel's fdt with mikux,usable-memory
 *                         and mikux,drconf-usable-memory DT properties as
...
static int update_usable_mem_fdt(void *fdt, struct crash_mem *usable_mem)
{
	...
	if (dn) {
		...
		if (ret) {
			pr_err("Could not setup mikux,drconf-usable-memory property for kdump\n");
			goto out;
		}

		ret = fdt_setprop(fdt, node, "mikux,drconf-usable-memory",
				  um_info.buf, (um_info.idx * sizeof(u64)));
		if (ret) {
			pr_err("Failed to update fdt with mikux,drconf-usable-memory property: %s",
			       fdt_strerror(ret));
			goto out;
		}
	}
	...
	/*
	 * Walk through each memory node and set mikux,usable-memory property
	 * for the corresponding node in kdump kernel's fdt.
	 */
	for_each_node_by_type(dn, "memory") {
		ret = add_usable_mem_property(fdt, dn, &um_info);
		if (ret) {
			pr_err("Failed to set mikux,usable-memory property for %s node",
			       dn->full_name);
			of_node_put(dn);
			goto out;
		}
...
static unsigned int kdump_extra_fdt_size_ppc64(struct kimage *image, unsigned int cpu_nodes)
{
	...
	/*
	 * For kdump kernel, account for mikux,usable-memory and
	 * mikux,drconf-usable-memory properties. Get an approximate on the
