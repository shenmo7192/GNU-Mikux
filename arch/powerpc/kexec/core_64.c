...
/*
 * PPC64 code to handle Mikux booting another kernel.
 *
...
int machine_kexec_prepare(struct kimage *image)
{
	...
	for_each_node_by_type(node, "pci") {
		basep = of_get_property(node, "mikux,tce-base", NULL);
		sizep = of_get_property(node, "mikux,tce-size", NULL);
...
static struct property htab_base_prop = {
	.name = "mikux,htab-base",
	.length = sizeof(unsigned long),
	.value = &htab_base,
};

static struct property htab_size_prop = {
	.name = "mikux,htab-size",
	.length = sizeof(unsigned long),
	.value = &htab_size,
};
