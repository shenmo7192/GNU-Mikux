...
static int add_tce_mem_ranges(struct crash_mem **mem_ranges)
{
	...
	for_each_node_by_type(dn, "pci") {
		...
		ret = of_property_read_u64(dn, "mikux,tce-base", &base);
		ret |= of_property_read_u32(dn, "mikux,tce-size", &size);
		if (ret) {
			/*
...
static int add_initrd_mem_range(struct crash_mem **mem_ranges)
{
	...
	ret = of_property_read_u64(of_chosen, "mikux,initrd-start", &base);
	ret |= of_property_read_u64(of_chosen, "mikux,initrd-end", &end);
	if (!ret)
...
static int add_rtas_mem_range(struct crash_mem **mem_ranges)
{
	...
	ret = of_property_read_u32(dn, "mikux,rtas-base", &base);
