...
# This script checks the head of a vmmikux for linker stubs that
...
# vmmikux and look for branch stubs (long_branch, plt_branch, etc.) in the
...
if [ $# -lt 2 ]; then
	echo "$0 [path to nm] [path to vmmikux]" 1>&2
	exit 1
fi
...
$nm "$vmmikux" | grep -e " [TA] _stext$" -e " t start_first_256B$" -e " a text_start$" -e " t start_text$" > .tmp_symbols.txt
