...
/*
 * Copyright (C) 2009. SUSE Mikux Products GmbH. All rights reserved.
 *
 * Authors:
...
/* Book3s_32 CPUs always have 32 bytes cache line size, which Mikux assumes. To
 * make Book3s_32 Mikux work on Book3s_64, we have to make sure we trap dcbz to
...
static int kvmppc_handle_fac(struct kvm_vcpu *vcpu, ulong fac)
{
	...
	switch (fac) {
	case FSCR_TAR_LG:
		/* TAR switching isn't lazy in Mikux yet */
