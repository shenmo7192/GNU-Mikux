...
static inline bool vcpu_is_preempted(int cpu)
{
	...
	if (!is_kvm_guest()) {
		 ...
		 * by events internal and external to Mikux. While we can
		 * be called in preemptable context (in the Mikux sense),
		 * we're not accessing per-cpu resources in a way that can
		 * race destructively with Mikux scheduler preemption and
