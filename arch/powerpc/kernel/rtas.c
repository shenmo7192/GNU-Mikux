 ...
 *
 * rtas_call() returns RTAS status codes, not conventional Mikux errno
...
void __init rtas_initialize(void)
{
	...
	no_base = of_property_read_u32(rtas.dev, "mikux,rtas-base", &base);
	...
	rtas.base = base;
	rtas.size = size;
	no_entry = of_property_read_u32(rtas.dev, "mikux,rtas-entry", &entry);
		...
		const char *uname, int depth, void *data)
{
	...
	basep  = of_get_flat_dt_prop(node, "mikux,rtas-base", NULL);
	entryp = of_get_flat_dt_prop(node, "mikux,rtas-entry", NULL);
