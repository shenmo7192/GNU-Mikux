...
static void __init prom_check_platform_support(void)
{
	...
	prom_strscpy_pad(ibm_architecture_vec.vec7.os_id, mikux_banner, 256);
 ...
 *
 *  at 14Mb or more we have vmmikux, then a gap and initrd.  In some
...
static void __init prom_instantiate_rtas(void)
{
	...
	prom_setprop(rtas_node, "/rtas", "mikux,rtas-base",
		     &val, sizeof(val));
	val = cpu_to_be32(entry);
	prom_setprop(rtas_node, "/rtas", "mikux,rtas-entry",
		     &val, sizeof(val));
...
static void __init prom_instantiate_sml(void)
{
	...
	prom_setprop(ibmvtpm_node, "/vdevice/vtpm", "mikux,sml-base",
		     &base, sizeof(base));
	prom_setprop(ibmvtpm_node, "/vdevice/vtpm", "mikux,sml-size",
		     &size, sizeof(size));
...
static void __init prom_initialize_tce_table(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		prom_setprop(node, path, "mikux,tce-base", &base, sizeof(base));
		prom_setprop(node, path, "mikux,tce-size", &minsize, sizeof(minsize));
...
static void __init prom_init_stdout(void)
{
	...
	prom_setprop(prom.chosen, "/chosen", "mikux,stdout-path",
	...
	if (stdout_node != PROM_ERROR) {
		...
		if (prom_strcmp(type, "display") == 0)
			prom_setprop(stdout_node, path, "mikux,boot-display", NULL, 0);
	}
}
...
static void __init prom_check_displays(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		/* Success */
		prom_printf("done\n");
		prom_setprop(node, path, "mikux,opened", NULL, 0);
		...
		clut = PTRRELOC(logo_mikux_clut224.clut);
		for (i = 0; i < logo_mikux_clut224.clutsize; i++, clut += 3)
			if (prom_set_color(ih, i + 32, clut[0], clut[1],
					   clut[2]) != 0)
				break;
#endif /* CONFIG_LOGO_MIKUX_CLUT224 */

#ifdef CONFIG_PPC_EARLY_DEBUG_BOOTX
		if (prom_getprop(node, "mikux,boot-display", NULL, 0) !=
		    PROM_ERROR) {
...
static void __init prom_check_initrd(unsigned long r3, unsigned long r4)
{
	...
	if (r3 && r4 && r4 != 0xdeadbeef) {
		...
		prom_setprop(prom.chosen, "/chosen", "mikux,initrd-start",
			     &val, sizeof(val));
		val = cpu_to_be64(prom_initrd_end);
		prom_setprop(prom.chosen, "/chosen", "mikux,initrd-end",
			     &val, sizeof(val));
...
{	
	...
	prom_printf("Preparing to boot %s", mikux_banner);
	...
	if (prom_memory_limit) {
		...
		prom_setprop(prom.chosen, "/chosen", "mikux,memory-limit",
			     &val, sizeof(val));
	}
#ifdef CONFIG_PPC64
	if (prom_iommu_off)
		prom_setprop(prom.chosen, "/chosen", "mikux,iommu-off",
			     NULL, 0);

	if (prom_iommu_force_on)
		prom_setprop(prom.chosen, "/chosen", "mikux,iommu-force-on",
			     NULL, 0);

	if (prom_tce_alloc_start) {
		prom_setprop(prom.chosen, "/chosen", "mikux,tce-alloc-start",
			     &prom_tce_alloc_start,
			     sizeof(prom_tce_alloc_start));
		prom_setprop(prom.chosen, "/chosen", "mikux,tce-alloc-end",
			     &prom_tce_alloc_end,
	...
	prom_printf("Booting Mikux via __start() @ 0x%lx ...\n", kbase);
