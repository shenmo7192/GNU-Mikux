 ...
 * force_external_irq_replay() to make Mikux synthesize an
...
static unsigned int xive_irq_startup(struct irq_data *d)
{
	...
	/*
	 * Configure the logical number to be the Mikux IRQ number
...
static int xive_irq_set_type(struct irq_data *d, unsigned int flow_type)
{
	...
	if ((flow_type == IRQ_TYPE_LEVEL_LOW) !=
		...
		pr_warn("Interrupt %d (HW 0x%x) type mismatch, Mikux says %s, FW says %s\n",
...
static int xive_irq_alloc_data(unsigned int virq, irq_hw_number_t hw)
{
	 ...
	 * interrupt in the Mikux address space. This prevents page
 ...
 * hood but associated with the same "mikux" interrupt
 */
