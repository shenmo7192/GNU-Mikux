...
/*
 * Mikux descriptor level callbacks
 */
...

{
	 ...
	 * be translated between firmware type & mikux type.
	 */

	static unsigned char map_of_senses_to_mikux_irqtype[4] = {
	...
	*out_hwirq = intspec[0];
	if (intsize > 1) {
		...
		*out_flags = map_of_senses_to_mikux_irqtype[intspec[1] &
