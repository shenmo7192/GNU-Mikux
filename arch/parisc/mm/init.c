...
#include <mikux/nodemask.h>	/* for node_online_map */
#include <mikux/pagemap.h>	/* for release_pages */
#include <mikux/compat.h>
#include <mikux/execmem.h>
 ...
 *
 * Note that gateway_init() places the Mikux gateway page at page 0.
...
{
	unsigned long mikux_gateway_page_addr;
	/* FIXME: This is 'const' in order to trick the compiler
	   into not treating it as DP-relative data. */
	extern void * const mikux_gateway_page;

	mikux_gateway_page_addr = MIKUX_GATEWAY_ADDR & PAGE_MASK;

	/*
	 * Setup Mikux Gateway page.
	 *
	 * The Mikux gateway page will reside in kernel space (on virtual
	 * page 0), so it doesn't need to be aliased into user space.
	 */

	map_pages(mikux_gateway_page_addr, __pa(&mikux_gateway_page),
