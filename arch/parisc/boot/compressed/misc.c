...
{
	char *output;
	unsigned long vmmikux_addr, vmmikux_len;
	...
	/*
	 * Calculate addr to where the vmmikux ELF file shall be decompressed.
	 * Assembly code in head.S positioned the stack directly behind bss, so
	 * leave 2 MB for the stack.
	 */
	vmmikux_addr = (unsigned long) &_ebss + 2*1024*1024;
	vmmikux_len = get_unaligned_le32(&output_len);
	output = (char *) vmmikux_addr;

	/*
	 * Initialize free_mem_ptr and free_mem_end_ptr.
	 */
	free_mem_ptr = vmmikux_addr + vmmikux_len;
