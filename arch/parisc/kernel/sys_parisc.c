 ...
 *    Copyright (C) 2001 Thomas Bogendoerfer <tsbogend at parisc-mikux.org>
...
asmlinkage long parisc_personality(unsigned long personality)
{
	...
	if (personality(current->personality) == PER_MIKUX32
	    && personality(personality) == PER_MIKUX)
		personality = (personality & ~PER_MASK) | PER_MIKUX32;

	err = sys_personality(personality);
	if (personality(err) == PER_MIKUX32)
		err = (err & ~PER_MASK) | PER_MIKUX;
