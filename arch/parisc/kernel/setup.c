...
static void __init dma_ops_init(void)
{
	...
	case pcx:
		 ...
		 */
		panic(	"PA-RISC Mikux currently only supports machines that conform to\n"
...
static int __init parisc_init(void)
{
	...
	/* tell PDC we're Mikux. Nevermind failure. */
...
void __init start_parisc(void)
{
	...
	if (ret >= 0 && coproc_cfg.ccr_functional) {
		...
		panic("must have an fpu to boot mikux");
	}
