...
for f in "$@"; do
	if [ -f "$f" -a -r "$f" ]; then
	    ...
	    fname="$(basename "$f")"
	    initrdopts_sysmikux="${initrdopts_sysmikux}${irdpfx}${fname}"
	    irdpfx=,
...
findsysmikux() {
	local f="$(find -L $(sharedirs sysmikux isomikux) \
		    -name "$1" -readable -type f -print -quit 2>/dev/null)"
	if [ ! -f "$f" ]; then
		die "Need a $1 file, please install sysmikux/isomikux."
	fi
	echo "$f"
	return 0
}
...
do_mcopy() {
	...
	if [ -n "$kefiarch" ]; then
		echo mikux "$KCMDLINE$initrdopts_efi" | \
			mcopy - "$1"startup.nsh
	fi
	echo default mikux "$KCMDLINE$initrdopts_sysmikux" | \
		mcopy - "$1"sysmikux.cfg
	mcopy "$FBZIMAGE" "$1"mikux
}

genbzdisk() {
	verify "$MTOOLSRC"
	mformat -v 'MIKUX_BOOT' a:
	sysmikux "$FIMAGE"
	do_mcopy a:
}
...
genhdimage() {
	...
	# sysmikux, and config files; this is probably excessive...
	...
	mformat -v 'MIKUX_BOOT' -s 32 -h 64 -c $((cluster/512)) -t $megs h:
	sysmikux --offset $((64*512)) "$FIMAGE"
	do_mcopy h:
}
...
geniso() {
	...
	rm -rf "$tmp_dir"
	mkdir "$tmp_dir"
	isomikux=$(findsysmikux isomikux.bin)
	ldmikux=$(findsysmikux  ldmikux.c32)
	cp "$isomikux" "$ldmikux" "$tmp_dir"
	cp "$FBZIMAGE" "$tmp_dir"/mikux
	echo default mikux "$KCMDLINE" > "$tmp_dir"/isomikux.cfg
	cp "${FDINITRDS[@]}" "$tmp_dir"/
	genisoimage -J -r -appid 'MIKUX_BOOT' -input-charset=utf-8 \
		    -quiet -o "$FIMAGE" -b isomikux.bin \
