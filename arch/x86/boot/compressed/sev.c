...
static void __page_state_change(unsigned long paddr, enum psc_op op)
{
	...
	if ((GHCB_RESP_CODE(val) != GHCB_MSR_PSC_RESP) || GHCB_MSR_PSC_RESP_VAL(val))
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_PSC);
 ...
 * - via a setup_data structure, as defined by the Mikux Boot Protocol
 *
...
void sev_enable(struct boot_params *bp)
{
	...
	if (sev_status & MSR_AMD64_SEV_SNP_ENABLED) {
		 ...
		 * guest is running at any other VMPL, this will fail. Mikux SNP guests
		...
		if (ret &&
			...
			sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_NOT_VMPL0);
	}
