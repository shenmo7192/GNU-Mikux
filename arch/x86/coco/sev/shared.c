 ...
 * between the pre-decompression boot code and the running Mikux kernel
 ...
 *   When running under an SVSM, the VMPL that Mikux is executing at must be
...
static void snp_register_ghcb_early(unsigned long paddr)
{
	...
	if ((GHCB_RESP_CODE(val) != GHCB_MSR_REG_GPA_RESP) ||
		...
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_REGISTER);
}
...
static void snp_cpuid_hv(struct ghcb *ghcb, struct es_em_ctxt *ctxt, struct cpuid_leaf *leaf)
{
	if (sev_cpuid_hv(ghcb, ctxt, leaf))
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_CPUID_HV);
}
...
static void __head setup_cpuid_table(const struct cc_blob_sev_info *cc_info)
{
	...
	if (!cc_info || !cc_info->cpuid_phys || cc_info->cpuid_len < PAGE_SIZE)
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_CPUID);
				    ...
				    int ret, u64 svsm_ret)
{
	...
	sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_PVALIDATE);
}
...
static bool __head svsm_setup_ca(const struct cc_blob_sev_info *cc_info)
{
	 ...
	 * Mikux SNP guests only ever run at a single VMPL level so permission mask
	...
	if (!cc_info || !cc_info->secrets_phys || cc_info->secrets_len != PAGE_SIZE)
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_SECRETS_PAGE);

	secrets_page = (struct snp_secrets_page *)cc_info->secrets_phys;
	if (!secrets_page->svsm_size)
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_NO_SVSM);

	if (!secrets_page->svsm_guest_vmpl)
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_SVSM_VMPL0);
	 ...
	 */
	if (caa & (PAGE_SIZE - 1))
		sev_es_terminate(SEV_TERM_SET_MIKUX, GHCB_TERM_SVSM_CAA);
