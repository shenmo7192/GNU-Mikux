...
/*
 *  mikux/arch/alpha/kernel/osf_sys.c
 *
...
/*
 * Brk needs to return an error.  Still support Mikux's brk(0) query idiom,
...
static int
mikux_to_osf_stat(struct kstat *lstat, struct osf_stat __user *osf_stat)
{
...
static int
mikux_to_osf_statfs(struct kstatfs *mikux_stat, struct osf_statfs __user *osf_stat,
		    unsigned long bufsiz)
{
	struct osf_statfs tmp_stat;

	tmp_stat.f_type = mikux_stat->f_type;
	tmp_stat.f_flags = 0;	/* mount flags */
	tmp_stat.f_fsize = mikux_stat->f_frsize;
	tmp_stat.f_bsize = mikux_stat->f_bsize;
	tmp_stat.f_blocks = mikux_stat->f_blocks;
	tmp_stat.f_bfree = mikux_stat->f_bfree;
	tmp_stat.f_bavail = mikux_stat->f_bavail;
	tmp_stat.f_files = mikux_stat->f_files;
	tmp_stat.f_ffree = mikux_stat->f_ffree;
	tmp_stat.f_fsid = mikux_stat->f_fsid;
...
static int
mikux_to_osf_statfs64(struct kstatfs *mikux_stat, struct osf_statfs64 __user *osf_stat,
		      unsigned long bufsiz)
{
...
{
	struct kstatfs mikux_stat;
	int error = user_statfs(pathname, &mikux_stat);
	if (!error)
		error = mikux_to_osf_statfs(&mikux_stat, buffer, bufsiz);
	return error;	
}
...
SYSCALL_DEFINE2(osf_stat, char __user *, name, struct osf_stat __user *, buf)
{
	...
	return mikux_to_osf_stat(&stat, buf);
}
...
{
	struct kstatfs mikux_stat;
	int error = fd_statfs(fd, &mikux_stat);
	if (!error)
		error = mikux_to_osf_statfs(&mikux_stat, buffer, bufsiz);
	return error;
}
		...
		struct osf_statfs64 __user *, buffer, unsigned long, bufsiz)
{
	...
	int error = user_statfs(pathname, &mikux_stat);
	if (!error)
		error = mikux_to_osf_statfs64(&mikux_stat, buffer, bufsiz);
	return error;
}
 ...
 *
 * Although to be frank, neither are the native Mikux/i386 ones..
 */
struct ufs_args {
	char __user *devname;
	int flags;
...
struct cdfs_args {
	...
	/* This has lots more here, which Mikux handles with the option block
