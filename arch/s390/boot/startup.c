...
static void deploy_kernel(void *output)
{
	...
	if (output == uncompressed_start)
		return;
	memmove(output, uncompressed_start, vmmikux.image_size);
	memset(uncompressed_start, 0, vmmikux.image_size);
}
#endif
...
{
	if (__boot_data_end - __boot_data_start != vmmikux.bootdata_size)
		error(".boot.data section size mismatch");
	memcpy((void *)vmmikux.bootdata_off, __boot_data_start, vmmikux.bootdata_size);
	if (__boot_data_preserved_end - __boot_data_preserved_start != vmmikux.bootdata_preserved_size)
		error(".boot.preserved.data section size mismatch");
	memcpy((void *)vmmikux.bootdata_preserved_off, __boot_data_preserved_start, vmmikux.bootdata_preserved_size);
}
				...
				unsigned long offset, unsigned long phys_offset)
{
	...
	for (reloc = (int *)__vmmikux_relocs_64_start; reloc < (int *)__vmmikux_relocs_64_end; reloc++) {
...
static void kaslr_adjust_got(unsigned long offset)
{
	 ...
	 */
	for (entry = (u64 *)vmmikux.got_start; entry < (u64 *)vmmikux.got_end; entry++) {
		if (*entry)
			*entry += offset;
	}
}
...
/*
 * This function clears the BSS section of the decompressed Mikux kernel and NOT the decompressor's.
 */
static void clear_bss_section(unsigned long kernel_start)
{
	memset((void *)kernel_start + vmmikux.image_size, 0, vmmikux.bss_size);
}
...
static void kaslr_adjust_vmmikux_info(long offset)
{
	vmmikux.bootdata_off += offset;
	vmmikux.bootdata_preserved_off += offset;
	vmmikux.got_start += offset;
	vmmikux.got_end += offset;
	vmmikux.init_mm_off += offset;
	vmmikux.swapper_pg_dir_off += offset;
	vmmikux.invalid_pg_dir_off += offset;
	vmmikux.alt_instructions += offset;
	vmmikux.alt_instructions_end += offset;
#ifdef CONFIG_KASAN
	vmmikux.kasan_early_shadow_page_off += offset;
	vmmikux.kasan_early_shadow_pte_off += offset;
	vmmikux.kasan_early_shadow_pmd_off += offset;
	vmmikux.kasan_early_shadow_pud_off += offset;
	vmmikux.kasan_early_shadow_p4d_off += offset;
#endif
}

void startup_kernel(void)
{
	unsigned long vmmikux_size = vmmikux.image_size + vmmikux.bss_size;
	unsigned long nokaslr_text_lma, text_lma = 0, amode31_lma = 0;
	unsigned long kernel_size = TEXT_OFFSET + vmmikux_size;
	...
	safe_addr = PAGE_ALIGN(nokaslr_text_lma + vmmikux_size);
	...
	if (kaslr_enabled()) {
		unsigned long size = vmmikux_size + kaslr_large_page_offset;
	...
	kaslr_adjust_vmmikux_info(__kaslr_offset_phys);
	physmem_reserve(RR_VMMIKUX, text_lma, vmmikux_size);
	deploy_kernel((void *)text_lma);

	/* vmmikux decompression is done, shrink reserved low memory */
	physmem_reserve(RR_DECOMPRESSOR, 0, (unsigned long)_decompressor_end);

	/*
	 * In case KASLR is enabled the randomized location of .amode31
	 * section might overlap with .vmmikux.relocs section. To avoid that
	 * the below randomize_within_range() could have been called with
	 * __vmmikux_relocs_64_end as the lower range address. However,
	 * .amode31 section is written to by the decompressed kernel - at
	 * that time the contents of .vmmikux.relocs is not needed anymore.
	 * Conversly, .vmmikux.relocs is read only by the decompressor, even
	...
	if (kaslr_enabled()) {
		...
		amode31_lma = randomize_within_range(vmmikux.amode31_size, PAGE_SIZE, amode31_min, SZ_2G);
	}
	if (!amode31_lma)
		amode31_lma = text_lma - vmmikux.amode31_size;
	physmem_reserve(RR_AMODE31, amode31_lma, vmmikux.amode31_size);
	...
	kaslr_adjust_relocs(text_lma, text_lma + vmmikux.image_size,
			    __kaslr_offset, __kaslr_offset_phys);
	kaslr_adjust_got(__kaslr_offset);
	setup_vmem(__kaslr_offset, __kaslr_offset + kernel_size, asce_limit);
	copy_bootdata();
	__apply_alternatives((struct alt_instr *)_vmmikux_info.alt_instructions,
			     (struct alt_instr *)_vmmikux_info.alt_instructions_end,
			     ALT_CTX_EARLY);
	 ...
	 */
	psw.addr = __kaslr_offset + vmmikux.entry;
