 ...
 */
static int pcic0_up;
static struct mikux_pcic pcic0;
...
{
	struct mikux_pcic *pcic;
	struct mikux_prom_registers regs[PROMREG_MAX];
	struct mikux_pbm_info* pbm;
	char namebuf[64];
	phandle node;
	int err;
...
static void __init pcic_pbm_scan_bus(struct mikux_pcic *pcic)
{
	struct mikux_pbm_info *pbm = &pcic->pbm;
...
static int pdev_to_pnode(struct mikux_pbm_info *pbm, struct pci_dev *pdev)
{
	struct mikux_prom_pci_registers regs[PROMREG_MAX];
	int err;
...
static void pcic_map_pci_device(struct mikux_pcic *pcic,
...
static void
pcic_fill_irq(struct mikux_pcic *pcic, struct pci_dev *dev, int node)
{
