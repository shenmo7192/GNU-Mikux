...
/*
 *  mikux/arch/sparc/kernel/setup.c
 *
 ...
 */
void __init sparc32_start_kernel(struct mikux_romvec *rp)
{
	prom_init(rp);
...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = prom_getbootargs();
	...
	if((boot_flags & BOOTME_DEBUG) && (mikux_dbvec != NULL) &&
	   ((*(short *)mikux_dbvec) != -1)) {
		printk("Booted under KADB. Syncing trap table.\n");
		(*(mikux_dbvec->teach_debugger))();
	}
