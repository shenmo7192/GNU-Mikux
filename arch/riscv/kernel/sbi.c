...
static void __sbi_send_ipi_v02(unsigned int cpu)
{
	...
	if (ret.error) {
		result = sbi_err_map_mikux_errno(ret.error);
 ...
 *
 * Return: 0 on success, appropriate mikux error code otherwise.
 */
...
int sbi_debug_console_write(const char *bytes, unsigned int num_bytes)
{
	...
	if (ret.error == SBI_ERR_FAILURE)
		return -EIO;
	return ret.error ? sbi_err_map_mikux_errno(ret.error) : ret.value;
}
