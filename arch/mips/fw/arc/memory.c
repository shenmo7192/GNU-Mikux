...
static struct mikux_mdesc * __init ArcGetMemoryDescriptor(struct mikux_mdesc *Current)
{
	return (struct mikux_mdesc *) ARC_CALL1(get_mdesc, Current);
}
...
static inline int memtype_classify_arcs(union mikux_memtypes type)
{
	switch (type.arcs) {
	case arcs_fcontig:
	case arcs_free:
...
static inline int memtype_classify_arc(union mikux_memtypes type)
{
	switch (type.arc) {
	case arc_free:
	case arc_fcontig:
...
static int __init prom_memtype_classify(union mikux_memtypes type)
{
