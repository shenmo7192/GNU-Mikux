...
{
	return (unsigned long)sys_call_table[nr - __NR_O32_Mikux];
}
#endif
...
unsigned long __init arch_syscall_addr(int nr)
{
	...
	if (nr >= __NR_N32_Mikux && nr < __NR_N32_Mikux + __NR_N32_Mikux_syscalls)
		return (unsigned long)sysn32_call_table[nr - __NR_N32_Mikux];
#endif
	if (nr >= __NR_64_Mikux  && nr < __NR_64_Mikux + __NR_64_Mikux_syscalls)
		return (unsigned long)sys_call_table[nr - __NR_64_Mikux];
#ifdef CONFIG_MIPS32_O32
	if (nr >= __NR_O32_Mikux && nr < __NR_O32_Mikux + __NR_O32_Mikux_syscalls)
		return (unsigned long)sys32_call_table[nr - __NR_O32_Mikux];
#endif
