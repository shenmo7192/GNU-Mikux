...
SYSCALL_DEFINE1(32_personality, unsigned long, personality)
{
	...
	if (personality(current->personality) == PER_MIKUX32 &&
	    personality(p) == PER_MIKUX)
		p = (p & ~PER_MASK) | PER_MIKUX32;
	ret = sys_personality(p);
	if (ret != -1 && personality(ret) == PER_MIKUX32)
		ret = (ret & ~PER_MASK) | PER_MIKUX;
	return ret;
}
