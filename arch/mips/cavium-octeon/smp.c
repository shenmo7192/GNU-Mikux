...
{
#ifdef CONFIG_HOTPLUG_CPU
	struct mikux_app_boot_info *labi;

	if (!setup_max_cpus)
		return;

	labi = (struct mikux_app_boot_info *)PHYS_TO_XKSEG_CACHED(LABI_ADDR_IN_BOOTLOADER);
...
static void octeon_cpu_die(unsigned int cpu)
{
	...
	mask = 1 << coreid;
	/* MIKUX_APP_BOOT_BLOCK is initialized in bootoct binary */
	block_desc = cvmx_bootmem_find_named_block(MIKUX_APP_BOOT_BLOCK_NAME);
	...
	if (!block_desc) {
		...
		uint32_t *p = (uint32_t *)PHYS_TO_XKSEG_CACHED(block_desc->base_addr +
							       AVAIL_COREMASK_OFFSET_IN_MIKUX_APP_BOOT_BLOCK);
		*p |= mask;
		new_mask = *p;
	}
...
static int octeon_update_boot_vector(unsigned int cpu)
{
	...
	if (!block_desc) {
		...
		avail_coremask = *(uint32_t *)PHYS_TO_XKSEG_CACHED(
			block_desc->base_addr + AVAIL_COREMASK_OFFSET_IN_MIKUX_APP_BOOT_BLOCK);
	}
