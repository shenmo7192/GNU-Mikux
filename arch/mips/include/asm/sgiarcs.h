...
enum mikux_identifier {
	bogus, ronly, removable, consin, consout, input, output
};

/* A prom device tree component. */
struct mikux_component {
	enum mikux_devclass	class;	/* node class */
	enum mikux_devtypes	type;	/* node type */
	enum mikux_identifier	iflags; /* node flags */
...
typedef struct mikux_component pcomponent;
...
union mikux_memtypes {
    enum arcs_memtypes arcs;
    enum arc_memtypes arc;
};

struct mikux_mdesc {
	union mikux_memtypes type;
	ULONG base;
	ULONG pages;
};
...
struct mikux_finfo {
	struct mikux_bigint   begin;
	struct mikux_bigint   end;
	struct mikux_bigint   cur;
	enum mikux_devtypes   dtype;
...
#define ROMVECTOR ((struct mikux_romvec *) (long)(PROMBLOCK)->romvec)

/* Cache layout parameter block. */
union mikux_cache_key {
	struct param {
#ifdef __MIPSEL__
		unsigned short size;
		unsigned char lsize;
