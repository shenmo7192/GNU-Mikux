...
{
	fprintf(f, "Usage: loongson3-llsc-check /path/to/vmmikux\n");
}
...
{
	int vmmikux_fd, status, err, i;
	const char *vmmikux_path;
	struct stat st;
	Elf64_Ehdr *eh;
	Elf64_Shdr *sh;
	void *vmmikux;
	...
	vmmikux_path = argv[1];
	vmmikux_fd = open(vmmikux_path, O_RDONLY);
	if (vmmikux_fd == -1) {
		perror("Unable to open vmmikux");
		goto out_ret;
	}

	err = fstat(vmmikux_fd, &st);
	if (err) {
		perror("Unable to stat vmmikux");
		goto out_close;
	}

	vmmikux = mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, vmmikux_fd, 0);
	if (vmmikux == MAP_FAILED) {
		perror("Unable to mmap vmmikux");
		goto out_close;
	}

	eh = vmmikux;
	if (memcmp(eh->e_ident, ELFMAG, SELFMAG)) {
		fprintf(stderr, "vmmikux is not an ELF?\n");
		goto out_munmap;
	}

	if (eh->e_ident[EI_CLASS] != ELFCLASS64) {
		fprintf(stderr, "vmmikux is not 64b?\n");
		goto out_munmap;
	}

	if (eh->e_ident[EI_DATA] != ELFDATA2LSB) {
		fprintf(stderr, "vmmikux is not little endian?\n");
		goto out_munmap;
	}

	for (i = 0; i < le16toh(eh->e_shnum); i++) {
		sh = vmmikux + le64toh(eh->e_shoff) + (i * le16toh(eh->e_shentsize));
