 ...
 * all hyp code/data, not yet linked into vmmikux. Hyp section names
 ...
 * the linker of `vmmikux` to populate the array entries with offsets
 ...
 *
 * The linker of vmmikux will populate the position given by `rela` with
 ...
 * will not be known until linking of `vmmikux`, so emit a PREL32
...
static void emit_rela_abs64(Elf64_Rela *rela, const char *sh_orig_name)
{
	...
	/*
	 * Create a PREL32 relocation which instructs the linker of `vmmikux`
