...
#include <mikux/set_memory.h>
#include <mikux/kfence.h>
#include <mikux/pkeys.h>
...
{
	static struct vm_struct vmmikux_seg[KERNEL_SEGMENT_COUNT];

	declare_vma(&vmmikux_seg[0], _stext, _etext, VM_NO_GUARD);
	declare_vma(&vmmikux_seg[1], __start_rodata, __inittext_begin, VM_NO_GUARD);
	declare_vma(&vmmikux_seg[2], __inittext_begin, __inittext_end, VM_NO_GUARD);
	declare_vma(&vmmikux_seg[3], __initdata_begin, __initdata_end, VM_NO_GUARD);
	declare_vma(&vmmikux_seg[4], _data, _end, 0);
}
