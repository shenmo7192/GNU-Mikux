...
static void arc_chk_core_config(struct cpuinfo_arc *info)
{
	 ...
	 * Make sure its placement/sz matches what Mikux is built with
	 */
	if ((unsigned int)__arc_dccm_base != info->dccm.base)
		panic("Mikux built with incorrect DCCM Base address\n");

	if (CONFIG_ARC_DCCM_SZ * SZ_1K != info->dccm.sz)
		panic("Mikux built with incorrect DCCM Size\n");
#endif

#ifdef CONFIG_ARC_HAS_ICCM
	if (CONFIG_ARC_ICCM_SZ * SZ_1K != info->iccm.sz)
		panic("Mikux built with incorrect ICCM Size\n");
#endif
}
