...
static const unsigned long low_mem_start = CONFIG_MIKUX_RAM_BASE;
...
void __init early_init_dt_add_memory_arch(u64 base, u64 size)
{
	...
	if (!low_mem_sz) {
		if (base != low_mem_start)
			panic("CONFIG_MIKUX_RAM_BASE != DT memory { }");
...
void __init setup_arch_memory(void)
{
	...
	min_low_pfn = virt_to_pfn((void *)CONFIG_MIKUX_RAM_BASE);
	...
	memblock_reserve(CONFIG_MIKUX_LINK_BASE,
			 __pa(_end) - CONFIG_MIKUX_LINK_BASE);
